...
from data_utils import load_snapshot, get_all_snapshots, parse_roles, build_reaction_matrix, require_clean_manual_events
...
if ultimo and ultimo.get('participantes'):
    ...
    if is_finalizado:
        ...
        analysis_date_label = f"📅 Dados de **{snap_for_analysis['date']}** (dia do paredão)"
    else:
    ...
    if is_finalizado:
        ...
        print(f'{analysis_date_label} — análise histórica congelada no momento da votação.')
        print(f'</div>\n')
    else:
        print(f"Como os emparedados estão no **queridômetro** ({analysis_date_label.replace('📅 ', '')}).\n")
...
::: {.callout-caution title="ℹ️ Sobre os dados" appearance="minimal" collapse="true"}
Os resultados do Paredão são atualizados após cada eliminação com base em informações oficiais (GShow, transmissão ao vivo). A análise de votos usa os dados do queridômetro mais próximos da data do paredão.
:::
