...
""")
        ...
        if poll:
            ...
            _vote_ctx = 'menos votado p/ salvar' if _is_save_poll else 'com {pct:.1f}% dos votos'
            print(f"\n## 📊 {_poll_label}\n")
            if _is_save_poll:
                print(f'<p class="text-warning small" style="font-weight:bold;">⚠️ Paredão Falso — voto é para SALVAR (mais votado vai ao Quarto Secreto, menos votado sai)</p>\n')
            ...
            _consol_desc = f'apenas {_consol_pct:.1f}% dos votos p/ salvar' if _is_save_poll else f'com {_consol_pct:.1f}% dos votos'
            ...
            print(f'<div class="text-muted">{_consol_desc}</div>')
            print(f'</div>')
            # Model card
            if _model_pred:
                _model_elim = _model_pred['predicao_eliminado']
                _model_pct = _model_pred['prediction'].get(_model_elim, 0)
                _model_desc = f'apenas {_model_pct:.1f}% dos votos p/ salvar' if _is_save_poll else f'com {_model_pct:.1f}% dos votos'
                ...
                print(f'<div class="text-muted">{_model_desc}</div>')
            ...
            def _poll_highlight(val, all_vals, color_best, color_worst='#e74c3c'):
                """Return style for poll value: best/worst depends on save vs eliminate."""
                best = max(all_vals)
                worst = min(all_vals)
                if _is_save_poll:
                    if val == worst:
                        return f'font-weight:bold; color:{color_worst};'
                    elif val == best:
                        return f'font-weight:bold; color:#00bc8c;'
                else:
                    if val == best:
            ...
            if _model_pred:
                ...
                print(f'<summary class="text-muted" style="cursor:pointer;">ℹ️ Pesos do modelo: {" · ".join(_weight_parts)}</summary>')
...
if ultimo and (ultimo.get('status') == 'finalizado' or any(p.get('resultado') for p in ultimo.get('participantes', []))):
        ...
        if poll and poll.get('resultado_real'):
            ...
            def _poll_hl_fin(val, all_vals, color_best, color_worst='#e74c3c'):
                """Highlight for finalized poll: save vs eliminate."""
                best = max(all_vals)
                worst = min(all_vals)
                if _is_save_poll_fin:
                    if val == worst:
                        return f'font-weight:bold; color:{color_worst};'
                    elif val == best:
                        return f'font-weight:bold; color:#00bc8c;'
