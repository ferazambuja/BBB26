...
if _pp_data:
    ...
    _html = '<h4 style="margin-top:1.2rem;">📊 Indicações Recebidas</h4>\n'
    ...
    for _name in _sorted_names:
        ...
        _html += f'<span style="color:#fff; font-weight:bold; font-size:0.8rem;">{_count}×</span>'
        _html += '</div></div></div>\n'
    ...
    if _not_nom:
        ...
        _html += f'🛡️ <strong>Não indicados por ninguém:</strong> {_not_html}</div>\n'
    _html += '</div>\n'
    print(_html)
    ...
    # ── Mutual nominations ──
    if _mutuals:
        _mhtml = '<h4 style="margin-top:1.2rem;">⚔️ Indicações Mútuas</h4>\n'
        _mhtml += '<p class="text-muted" style="font-size:0.82rem;">Pares que querem o outro no paredão — hostilidade recíproca declarada.</p>\n'
        _mhtml += '<div style="display:flex; flex-wrap:wrap; gap:8px;">'
        for _pair in _mutuals:
            _a, _b = _pair[0], _pair[1]
            _mhtml += f'<div style="background:#2a1a1a; border:1px solid #E6194B; border-radius:8px; padding:4px 10px; font-size:0.82rem;">'
            _mhtml += f'{avatar_img(_a, AVATARS, 26)} ↔ {avatar_img(_b, AVATARS, 26)}</div>'
        _mhtml += '</div>\n'
        print(_mhtml)
    ...
    if len(_both) >= 5:
        ...
        _fhtml = '<div style="margin-top:1rem; padding:10px 14px; background:#1a1a2a; border:1px solid #4363d8; border-radius:8px;">'
    ...
    if _prev_picks:
        ...
        if _movers:
            _mvhtml = '<div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:12px;">'
            for _name, _w4, _w6, _delta in _movers:
                ...
                if _delta > 0:
                    ...
                    _badge_color = "#3CB44B"
                    _arrow = f"🔻 {_delta}"
                _mvhtml += f'<div style="background:#1a1a1a; border:1px solid {_badge_color}; border-radius:8px; padding:8px 12px; text-align:center; min-width:110px;">'
                _mvhtml += f'{_av}<br/>'
                _mvhtml += f'<span style="font-size:0.78rem; color:#aaa;">Sem 4: {_w4}×</span><br/>'
                _mvhtml += f'<span style="font-size:0.78rem; color:#aaa;">Sem 6: {_w6}×</span><br/>'
                _mvhtml += f'<span style="font-weight:bold; color:{_badge_color}; font-size:0.9rem;">{_arrow}</span></div>'
            _mvhtml += '</div>\n'
            print(_mvhtml)

        # Insight callouts
        _ihtml = '<div style="display:flex; flex-direction:column; gap:8px; margin-bottom:12px;">'
        ...
        if _gab_w4 >= 2 and _gab_w6 == 0:
            ...
            _ihtml += f'<div style="padding:8px 12px; background:#1a2a1a; border:1px solid #3CB44B; border-radius:6px; font-size:0.82rem;">'
            _ihtml += f'🔍 <strong>Caso Gabriela:</strong> era a mais indicada na sem 4 ({_gab_w4}×), agora 0×. {"; ".join(_detail_parts)}.</div>'

        # Jonas rise
        _jonas_w4 = _prev_received.get("Jonas Sulzbach", 0)
        _jonas_w6 = _received.get("Jonas Sulzbach", 0)
        if _jonas_w6 >= 5 and _jonas_w4 <= 1:
            _ihtml += f'<div style="padding:8px 12px; background:#2a1a1a; border:1px solid #E6194B; border-radius:6px; font-size:0.82rem;">'
            _ihtml += f'🎯 <strong>Ascensão de Jonas:</strong> não participou do Sincerinho da sem 4 (era Líder). Agora é o alvo #1 com {_jonas_w6}× indicações — 3º título consecutivo de Líder o tornou alvo principal.</div>'

        # Loyalty stat
        _loyalty_pct = round(len(_kept) / len(_common) * 100) if _common else 0
        _ihtml += f'<div style="padding:8px 12px; background:#1a1a2a; border:1px solid #4363d8; border-radius:6px; font-size:0.82rem;">'
        _ihtml += f'📊 <strong>Fidelidade:</strong> {len(_kept)}/{len(_common)} ({_loyalty_pct}%) mantiveram o alvo da semana 4 nas suas 3 escolhas.</div>'
        _ihtml += '</div>\n'
        print(_ihtml)

        # Persistent rivalries (expandable)
        if _kept:
            _rhtml = '<details style="margin-top:0.5rem;">\n'
            _rhtml += '<summary style="cursor:pointer; font-weight:bold; font-size:0.95rem;">🎯 Rivalidades persistentes (sem 4 → sem 6)</summary>\n\n'
            _rhtml += '<table class="table table-striped" style="font-size:0.85rem; margin-top:0.5rem;">'
            _rhtml += '<thead><tr><th>Quem</th><th>Alvo (mantido)</th><th>Tipo</th></tr></thead><tbody>'
            for _a, _t in _kept:
                ...
                if _is_mutual and _was_mutual:
                    ...
                    _tipo = '→ Unilateral'
                _rhtml += f'<tr><td>{_av_a} {_a}</td><td>{_av_t} {_t}</td><td>{_tipo}</td></tr>'
            _rhtml += '</tbody></table></details>\n'
            print(_rhtml)
        ...
        print('<div style="margin-top:0.5rem; padding:8px 12px; background:#2a2a1a; border:1px solid #ffe119; border-radius:6px; font-size:0.82rem;">'
              '⚠️ <strong>Lição da semana 4:</strong> Sol Vega escolheu Juliano Floss, mas no Duelo de Risco ambos indicaram <em>Samira</em> em consenso — alguém que nenhum dos dois havia escolhido. A dinâmica pode mudar o alvo original.'
              '</div>')
