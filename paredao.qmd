...
if ultimo and ultimo.get('participantes'):
    ...
    # ── Determine which snapshot to use for "today" analysis ──
    # For finalizado: use paredão-date snapshot (frozen historical view)
    # For em_andamento: use latest snapshot (live current view)
    if is_finalizado:
        # Find the snapshot closest to paredão date
        ...
        analysis_date_label = f"📅 Dados de **{snap_for_analysis['date']}** (dia da formação)"
    else:
        snap_for_analysis = latest
        analysis_date_label = f"📅 Dados de **{latest['label']}** (atual)"

    # Sentimento e negativas no snapshot de análise
    sent_paredao = {}
    neg_paredao = {}
    ...
    # Ranking no snapshot de análise
    ...
    # ── Série histórica (diária) — filtered to paredão date for finalizado ──
    rows = []
    for snap in daily_snapshots:
        date = snap['date']
        # For finalizado paredões, don't include data AFTER the paredão
    ...
    if table_rows:
        ...
        print(f'<tr><td>👎 <strong>{negativas_label}</strong></td><td>{avatar_html(most_neg["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{neg_paredao.get(most_neg["Indicado"], 0)} reações</td></tr>')
        if deltas:
