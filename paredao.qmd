...
# ══════════════════════════════════════════════════════════════════════════════
# LEITURA RÁPIDA — QUERIDÔMETRO DOS INDICADOS (HOJE + HISTÓRICO)
# ══════════════════════════════════════════════════════════════════════════════
...
if ultimo and ultimo.get('participantes'):
    ...
    print("Como os emparedados estão no **queridômetro de hoje** e no **histórico recente**.\n")
    ...
    if table_rows:
        ...
        print('<tbody>')
        print(f'<tr><td>🔴 <strong>Mais rejeitado hoje</strong></td><td>{avatar_html(worst_today["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{worst_today["Sentimento hoje"]} sentimento</td></tr>')
        print(f'<tr><td>👎 <strong>Mais negativas recebidas</strong></td><td>{avatar_html(most_neg["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{neg_hoje.get(most_neg["Indicado"], 0)} reações</td></tr>')
        if deltas:
...
if tem_votos and all_matrices:
    ...
    if len(df_scatter) > 2:
        ...
        if abs(corr_val) < 0.3:
            print(f"> **Correlação fraca (r = {corr_val:.2f}):** Ser odiado no queridômetro "
                  f"**não significa** ser votado. Os votos seguem lógica estratégica.\n")
        elif corr_val > 0.5:
            print(f"> **Correlação forte (r = {corr_val:.2f}):** Quem é mais detestado "
                  f"**tende a ser mais votado**. O jogo está honesto.\n")
        else:
            print(f"> **Correlação moderada (r = {corr_val:.2f}):** Existe alguma relação "
