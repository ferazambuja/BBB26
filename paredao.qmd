...
""")
        ...
        if poll:
            ...
            for nome in participantes_poll:
                avatar = avatar_img(nome, AVATARS, 24)
            ...
            def _poll_highlight(val, all_vals, color_best):
                """Highlight the most-voted participant in platform color."""
                if val == max(all_vals):
                    return f'font-weight:bold; color:{color_best};'
                return ''
            ...
            # 1. Tie detection
            if _is_tie:
                _diff = abs(_sorted_consol[0][1] - _sorted_consol[1][1])
                _curiosities.append(f'🔥 Diferença de apenas <strong>{_diff:.1f} p.p.</strong> entre {_sorted_consol[0][0].split()[0]} e {_sorted_consol[1][0].split()[0]} — empate técnico!')
            # 2. Platform split
        ...
        if poll and poll.get('resultado_real'):
            ...
            if _model_pred_fin and _model_mae_fin is not None:
                if _model_correct_fin:
                    model_badge = '<span style="background:#00bc8c; color:#fff; padding:0.3rem 0.8rem; border-radius:12px; font-weight:bold;">✅ Modelo correto</span>'
                else:
                    model_badge = '<span style="background:#E6194B; color:#fff; padding:0.3rem 0.8rem; border-radius:12px; font-weight:bold;">❌ Modelo errado</span>'
            ...
            def _poll_hl_fin(val, all_vals, color_best):
                """Highlight the most-voted participant in platform color."""
                if val == max(all_vals):
                    return f'font-weight:bold; color:{color_best};'
                return ''
            ...
            for plat in platform_order:
                if plat in plataformas:
                    ...
                    for nome in participantes_poll:
                        val = pdata.get(nome, 0)
                        style = _poll_hl_fin(val, _pfvals, '#f39c12')
                        print(f'<td class="tc" style="{style}">{val:.1f}%</td>')
            ...
            for nome in participantes_poll:
                val = consolidado.get(nome, 0)
                style = _poll_hl_fin(val, _cfvals, '#9b59b6')
                if 'font-weight' not in style:
                    style = 'font-weight:bold;' + style
                print(f'<td class="tc" style="{style}">{val:.1f}%</td>')
            ...
            # Model row
            if _model_pred_fin:
                print(f'<tr style="background:rgba(0,188,140,0.2);"><td style="font-weight:bold;">🎯 Modelo</td>')
                _mfvals = [_model_pred_fin['prediction'].get(n, 0) for n in participantes_poll]
                for nome in participantes_poll:
                    val = _model_pred_fin['prediction'].get(nome, 0)
                    style = _poll_hl_fin(val, _mfvals, '#00bc8c')
                    if 'font-weight' not in style:
                        style = 'font-weight:bold;' + style
                    print(f'<td class="tc" style="{style}">{val:.1f}%</td>')
