...
""")
        ...
        if _duelo and _duelo.get('escolhas'):
            ...
            print(f'<p style="color:#aaa; font-size:0.9em;">Cada participante escolheu quem quer enfrentar no Paredão. Amanhã, um nome será sorteado para o Duelo de Risco.</p>\n')
            ...
            print('<tbody>')
            # Sort by target popularity (most targeted first), then alphabetically
            _target_count = {}
        ...
        if _duelo.get('indicado_consenso') and _duelo.get('resultado') == 'nos_indicamos':
            ...
            print(f"### 📊 Opções pré-duelo — Score combinado\n")
            print(f'<p style="color:#aaa; font-size:0.85em;">Ranking de todos os alvos elegíveis por score combinado ({_sorteado} + {_oponente}). Scores <strong>antes</strong> da indicação. Quanto mais negativo, mais provável como alvo.</p>\n')
            ...
            for t in _dr_eligible:
                ...
                _combined.append({
                    "name": t,
                    "sol_score": s_entry["score"],
                    "jf_score": j_entry["score"],
                    "combined": s_entry["score"] + j_entry["score"],
                    "sol_q": s_entry["queridometro"],
                    "jf_q": j_entry["queridometro"],
                    "is_vip": t in _dr_vip,
                })
            _combined.sort(key=lambda x: x["combined"])

            print('<table class="table table-striped" style="max-width:100%; font-size:0.9rem;">')
            print(f'<thead><tr><th>#</th><th>Alvo</th><th style="text-align:center;">{_sorteado.split()[0]} Score</th><th style="text-align:center;">{_oponente.split()[0]} Score</th><th style="text-align:center;">Combinado</th><th style="text-align:center;">Q {_sorteado.split()[0]}</th><th style="text-align:center;">Q {_oponente.split()[0]}</th><th></th></tr></thead>')
            print('<tbody>')
            for _i, _c in enumerate(_combined):
                ...
                print(f'<tr style="{_row_style}"><td style="color:#888;">{_i+1}</td><td>{_img_c}<strong>{_c["name"]}</strong>{_target_badge}</td><td style="text-align:center;">{_sc(_c["sol_score"])}</td><td style="text-align:center;">{_sc(_c["jf_score"])}</td><td style="text-align:center;">{_sc(_c["combined"])}</td><td style="text-align:center;">{_q_c(_c["sol_q"])}</td><td style="text-align:center;">{_q_c(_c["jf_q"])}</td><td>{_vip_badge}</td></tr>')
            print('</tbody></table>\n')
            ...
            if _target_pos and _target_entry:
                ...
                print(f'<strong style="color:#9b59b6;">💡 Posição:</strong> <span style="color:{_insight_color};">{_consenso_target} era o <strong>#{_target_pos}</strong> alvo mais provável</span> pelo score combinado ({_target_entry["combined"]:+.2f}).')
                if _target_entry["sol_q"] >= 0 and _target_entry["jf_q"] < 0:
                    print(f' O queridômetro de {_sorteado.split()[0]} era <span style="color:#2ecc71;">positivo ({_target_entry["sol_q"]:+.2f})</span> enquanto {_oponente.split()[0]} era <span style="color:#e74c3c;">negativo ({_target_entry["jf_q"]:+.2f})</span> — sugerindo que {_oponente.split()[0]} puxou a decisão.')
                print('</div>\n')
            ...
            def _render_overlap_box(title, color, rgba_bg, items, emoji):
                ...
                for _it in items:
                    ...
                    print(f'<span style="background:rgba(0,0,0,0.3); padding:0.3rem 0.6rem; border-radius:8px; font-size:0.85em;">{_img_it}<strong>{_it["name"]}</strong>{_mark} <span style="color:#888;">({_it["combined"]:+.2f})</span></span>')
                print('</div></div>')
            ...
            # Get last 14 daily snapshots
            _last14 = daily_snapshots[-14:] if len(daily_snapshots) >= 14 else daily_snapshots
            _last14_mat = daily_matrices[-14:] if len(daily_matrices) >= 14 else daily_matrices
            ...
            for _actor, _target_q, _label in _q_pairs:
                ...
                for _snap, _mat in zip(_last14, _last14_mat):
                    _reaction = _mat.get(_actor, {}).get(_target_q, '')
                    if _reaction:
            ...
            _sol_hearts_to_target = sum(1 for _snap, _mat in zip(_last14, _last14_mat)
                                         if _mat.get(_sorteado, {}).get(_consenso_target, '') in POSITIVE)
            _sol_total_days = sum(1 for _snap, _mat in zip(_last14, _last14_mat)
                                  if _mat.get(_sorteado, {}).get(_consenso_target, ''))
