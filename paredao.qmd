...
def load_paredoes():
    """Carrega dados de paredão do arquivo JSON."""
...
<p style="color: #ccc; margin: 0.5rem 0 0 0; font-size: 0.9em;">Participantes detectados da API. Informações adicionais serão preenchidas manualmente.</p>
...
""")
        ...
        if participantes:
            ...
            for p in participantes:
                ...
                como_indicado = p.get('como', '')  # Ex: "Dinâmica", "Líder", "Casa", "API"
                ...
                if como_indicado and not is_auto:
                    ...
                    print(f'<div style="color: #f39c12; font-size: 0.75em; margin-top: 0.5rem;">⏳ aguardando detalhes</div>')
                print('</div>')
        ...
        if tem_votos_casa:
            ...
            if len(ranking) >= 2:
                ...
                if len(ranking) == 2:
                    ...
                    print(f'Divisão da casa: {", ".join(parts_bloc)}.\n')
                ...
                for bloc in blocs:
                    ...
                    print(f'<div style="text-align:center; margin-bottom:0.8rem;">')
        ...
        if True:  # Always show when data available
            ...
            if sentiment_data:
                ...
                for _, row in df_sent.iterrows():
                    if row['is_nominee']:
                        tick_labels.append(f"<b>⚠️ {row['name']}</b>")
                    else:
        ...
        # ── Enquetes do Votalhada (em andamento) ──
        ...
        if poll:
            ...
            print("\n## 📊 Previsão das Enquetes\n")
            print(f'<p class="text-muted small">Dados agregados de <a href="https://votalhada.blogspot.com/" target="_blank">Votalhada</a> — coleta em {poll.get("data_coleta", "?")[:10]} {poll.get("data_coleta", "?")[11:16] if len(poll.get("data_coleta", "")) > 11 else ""}</p>\n')

            print(f'<div class="alert alert-info" style="font-size:0.95rem;">')
            print(f'🔮 <strong>Previsão atual:</strong> {consolidado.get("predicao_eliminado", "—")} com mais votos para sair ({consolidado.get(consolidado.get("predicao_eliminado", ""), 0):.1f}%)')
            print(f'</div>\n')
            ...
            print(f'<tr style="background:rgba(155,89,182,0.2);"><td style="font-weight:bold;">📊 Consolidado</td>')
            for nome in participantes_poll:
        ...
        if _duelo and _duelo.get('indicado_consenso') and _duelo.get('resultado') == 'nos_indicamos':
            ...
            print(f'<div style="background:#9b59b6; color:#fff; padding:0.15rem 0.6rem; border-radius:8px; font-size:0.75em; margin-top:0.2rem;">🎲 Sorteada</div>')
            print('</div>')
            # Arrow + decision
            print(f'<div style="text-align:center; color:#9b59b6; font-size:1.5em; padding:0 0.5rem;">')
            print(f'<div>🤝</div>')
            print(f'<div style="font-size:0.5em; color:#ccc;">Nós indicamos</div>')
            print(f'</div>')
            # Oponente
            print(f'<div style="text-align:center;">')
            if _av_o:
            ...
            print(f'<div style="background:#e67e22; color:#fff; padding:0.15rem 0.6rem; border-radius:8px; font-size:0.75em; margin-top:0.2rem;">⚔️ Oponente</div>')
            print('</div>')
            # Arrow to target
            print(f'<div style="text-align:center; color:#e74c3c; font-size:1.5em; padding:0 0.5rem;">→</div>')
            # Target
            print(f'<div style="text-align:center;">')
            if _av_t:
            ...
            print(f'<div style="background:#e74c3c; color:#fff; padding:0.15rem 0.6rem; border-radius:8px; font-size:0.75em; margin-top:0.2rem;">📌 Emparedada</div>')
            print('</div>')
            print('</div>')
            if _resultado_detail:
                print(f'<p style="text-align:center; color:#aaa; font-size:0.85em; margin-top:0.5rem;">{_resultado_detail}</p>\n')
            ...
            _outcome_map = {
                ('nos_indicamos', 'nos_indicamos'): ('🤝 Consenso', 'Ambos indicam alguém em acordo', '#9b59b6'),
                ('nos_indicamos', 'eu_encaro'): ('⚔️ Divergência', f'{_oponente.split()[0]} vai ao Paredão sozinho', '#e67e22'),
                ('eu_encaro', 'nos_indicamos'): ('⚔️ Divergência', f'{_sorteado.split()[0]} vai ao Paredão sozinho', '#e67e22'),
                ('eu_encaro', 'eu_encaro'): ('💥 Duelo direto', 'Ambos vão ao Paredão', '#e74c3c'),
            }
            ...
            print('<summary style="cursor:pointer; color:#ccc; font-size:0.9em;">📐 <strong>Matriz de decisões possíveis</strong> — o que cada combinação de escolhas produzia</summary>')
            print('<div style="padding:0.8rem 0;">')
            print(f'<p style="color:#aaa; font-size:0.82em;">No Duelo de Risco, cada duelista escolhe secretamente: <strong>"Nós indicamos"</strong> (juntos indicam um terceiro) ou <strong>"Eu encaro"</strong> (vai ao Paredão sozinho). O resultado depende da combinação.</p>')
            print('<table class="table" style="max-width:500px; font-size:0.85rem; margin:0.5rem auto;">')
            print(f'<thead><tr><th></th><th style="text-align:center;">🟣 {_oponente.split()[0]}<br><span style="font-size:0.8em; color:#888;">"Nós indicamos"</span></th><th style="text-align:center;">🟠 {_oponente.split()[0]}<br><span style="font-size:0.8em; color:#888;">"Eu encaro"</span></th></tr></thead>')
            print('<tbody>')
            for s_choice, s_label in [('nos_indicamos', 'Nós indicamos'), ('eu_encaro', 'Eu encaro')]:
                print(f'<tr><td style="vertical-align:middle;"><strong>🟣 {_sorteado.split()[0]}</strong><br><span style="font-size:0.8em; color:#888;">"{s_label}"</span></td>')
                for o_choice in ['nos_indicamos', 'eu_encaro']:
                    ...
                    _check = ' ✅' if _is_actual else ''
                    print(f'<td style="text-align:center; {_border} border-radius:6px; padding:0.5rem;"><span style="color:{_color}; font-weight:bold;">{_title}</span>{_check}<br><span style="font-size:0.8em; color:#aaa;">{_desc}</span></td>')
                print('</tr>')
            print('</tbody></table>')
            ...
            print(f'<thead><tr><th>#</th><th>Alvo</th><th style="text-align:center;">{_sorteado.split()[0]}</th><th style="text-align:center;">{_oponente.split()[0]}</th><th style="text-align:center;">Gargalo</th><th style="text-align:center;">Q {_sorteado.split()[0]}</th><th style="text-align:center;">Q {_oponente.split()[0]}</th><th></th></tr></thead>')
            print('<tbody>')
            for _i, _c in enumerate(_combined):
                ...
                print(f'<tr style="{_row_style}"><td style="color:#888;">{_i+1}</td><td>{_img_c}<strong>{_c["name"]}</strong>{_target_badge}</td><td style="text-align:center;">{_sc(_c["sol_score"])}</td><td style="text-align:center;">{_sc(_c["jf_score"])}</td><td style="text-align:center;">{_consensus_icon} {_sc(_c["bottleneck"])}</td><td style="text-align:center;">{_q_c(_c["sol_q"])}</td><td style="text-align:center;">{_q_c(_c["jf_q"])}</td><td>{_vip_badge}</td></tr>')
            print('</tbody></table>')
            print(f'<p style="color:#888; font-size:0.78em; margin-top:0.3rem;">🔴 = ambos negativos (consenso natural) · 🟡 = um positivo (alguém cede)</p>\n')
            ...
            def _render_overlap_box(title, color, rgba_bg, items, emoji):
                ...
                for _it in items:
                    ...
                    _mark = ' ⭐' if _it["name"] == _consenso_target else ''
            ...
            _render_overlap_box(f"Ambos negativos — consenso natural", "#e74c3c", "rgba(231,76,60,0.08)", _both_neg, "🔴")
            _render_overlap_box(f"Só {_sorteado.split()[0]} negativ{artigo(_sorteado).replace('o','a') if artigo(_sorteado)=='a' else 'o'}", "#e67e22", "rgba(230,126,34,0.08)", _sol_only, "🟠")
            _render_overlap_box(f"Só {_oponente.split()[0]} negativo", "#f1c40f", "rgba(241,196,15,0.08)", _jf_only, "🟡")
            _render_overlap_box(f"Ambos positivos — seguro", "#2ecc71", "rgba(46,204,113,0.08)", _both_pos, "🟢")
            ...
            for _snap, _mat in _last14:
                _d = _snap["date"]
                _d_short = _d[5:]  # MM-DD
                print(f'<th style="text-align:center; font-size:0.75em; color:#888; padding:0.2rem;">{_d_short}</th>')
            print('</tr></thead><tbody>')
            ...
            for _actor, _target_q, _label in _q_pairs:
                ...
                for _snap, _mat in _last14:
                    ...
                    if _reaction:
                        ...
                        print(f'<td style="text-align:center; background:{_bg}; padding:0.2rem;" title="{_reaction}">{_emoji}</td>')
                    else:
                        print(f'<td style="text-align:center; color:#444; padding:0.2rem;">·</td>')
                print('</tr>')
            print('</tbody></table>\n')
        ...
        if _duelo and _duelo.get('escolhas'):
            ...
            if _alvos:
                ...
                for _a in _alvos:
                    ...
                    print(f'<div style="background: rgba(255,255,255,0.05); border: 1px solid {_cor}; border-radius: 10px; padding: 0.6rem 1rem; text-align: center; min-width: 100px;">')
                    if _av:
                    ...
                    print(f'<div style="color:#888; font-size:0.7em;">{_por}</div>')
                    print('</div>')
                print('</div>\n')
            ...
            if _nao_citados:
                ...
                print(f'<div style="color:#ccc; font-size:0.9em;">{" · ".join(_names_html)}</div>')
                print('</div>\n')
...
if ultimo and (ultimo.get('status') == 'finalizado' or any(p.get('resultado') for p in ultimo.get('participantes', []))):
        ...
        if poll and poll.get('resultado_real'):
            ...
            if accuracy and accuracy['predicao_correta']:
                status_html = '<span style="background:#3CB44B; color:#fff; padding:0.3rem 0.8rem; border-radius:12px; font-weight:bold;">✅ Previsão correta</span>'
            else:
                status_html = '<span style="background:#E6194B; color:#fff; padding:0.3rem 0.8rem; border-radius:12px; font-weight:bold;">❌ Previsão errada</span>'

            erro_medio = accuracy['erro_medio'] if accuracy else 0
            print(f'<div style="text-align:center; margin-bottom:1.5rem;">')
            print(f'{status_html}')
            print(f'<span style="margin-left:1rem; color:#888;">Erro médio: <strong>{erro_medio:.1f}</strong> pontos percentuais</span>')
            print(f'</div>\n')
            ...
            for nome in participantes_poll:
                ...
                print(f'<td style="text-align:center; {style}">{val:.1f}%</td>')
            print('<td style="text-align:center; color:#888;">—</td>')
            print('</tr>')
            print('</tbody></table>\n')
            ...
            if paredao_votos:
                ...
                print('<thead><tr><th>Comparação</th><th style="text-align:center;">Enquete</th><th style="text-align:center;">Votação</th><th style="text-align:center;">Erro</th></tr></thead>')
                print('<tbody>')
                ...
                print('<thead><tr><th>#</th><th>Plataforma</th><th style="text-align:center;">Previsão</th><th style="text-align:center;">Erro</th></tr></thead>')
                print('<tbody>')
...
# ══════════════════════════════════════════════════════════════════════
# PAREDÃO PERFEITO (SINCERINHO)
# Shown when week 6 sincerao has paredao_perfeito data.
# Auto-hides when the data doesn't exist.
# ══════════════════════════════════════════════════════════════════════
...
if _me_path.exists():
    ...
    for _we in _me.get("weekly_events", []):
        ...
        if _sinc and isinstance(_sinc, dict) and "paredao_perfeito" in _sinc:
            _pp_data = _sinc
            _pp_week = _we.get("week", "?")
            break

if _pp_data:
    _pp = _pp_data["paredao_perfeito"]
    _stats = _pp_data.get("stats", {})
    _most = _stats.get("most_nominated", [])
    _not_nom = _stats.get("not_nominated", [])
    _mutuals = _stats.get("mutual_nominations", [])

    print("\n## 🗣️ Sincerinho — Paredão Perfeito\n")
    print(f'<p class="text-muted" style="font-size:0.85rem;">Cada participante montou sua berlinda ideal com 3 nomes. Dados usados na dinâmica do Big Fone.</p>\n')
    ...
    print('<h4 style="margin-top:1.2rem;">📊 Indicações Recebidas</h4>\n')
    ...
    for _name in _sorted_names:
        ...
        print(f'<div style="display:flex; align-items:center; margin-bottom:4px;">')
        print(f'  <div style="width:180px; display:flex; align-items:center; gap:6px;">{_av} <span style="font-size:0.85rem;">{_name}</span></div>')
        print(f'  <div style="flex:1; background:#333; border-radius:4px; height:22px; position:relative;">')
        print(f'    <div style="width:{_pct}%; background:{_color}; height:100%; border-radius:4px; display:flex; align-items:center; justify-content:flex-end; padding-right:6px;">')
        print(f'      <span style="color:#fff; font-weight:bold; font-size:0.8rem;">{_count}×</span>')
        print(f'    </div>')
        print(f'  </div>')
        print(f'</div>')
    ...
    if _not_nom:
        ...
        print(f'<div style="margin-top:8px; padding:6px 10px; background:#1a3a1a; border:1px solid #3CB44B; border-radius:6px; font-size:0.85rem;">')
        print(f'  🛡️ <strong>Não indicados por ninguém:</strong> {_not_html}')
        print(f'</div>')
    print('</div>\n')

    # ── Who picked whom (expandable) ──
    print('<details style="margin-top:1rem;">')
    print('<summary style="cursor:pointer; font-weight:bold; font-size:1rem;">🔍 Quem indicou quem (detalhes)</summary>\n')
    print('<table class="table table-striped" style="font-size:0.9rem; margin-top:0.5rem;">')
    print('<thead><tr><th>Participante</th><th>Paredão Perfeito (3 nomes)</th></tr></thead>')
    print('<tbody>')
    for _actor in sorted(_pp.keys()):
        ...
        print(f'<tr><td>{_av} {_actor}</td><td>{_targets_html} <span style="font-size:0.8rem; color:#aaa;">{", ".join(_targets)}</span></td></tr>')
    print('</tbody></table>')
    print('</details>\n')

    # ── Mutual nominations ──
    if _mutuals:
        print('<h4 style="margin-top:1.2rem;">⚔️ Indicações Mútuas</h4>')
        print(f'<p class="text-muted" style="font-size:0.82rem;">Pares que querem o outro no paredão — hostilidade recíproca declarada.</p>')
        print('<div style="display:flex; flex-wrap:wrap; gap:8px;">')
        for _pair in _mutuals:
            _a, _b = _pair[0], _pair[1]
            print(f'<div style="background:#2a1a1a; border:1px solid #E6194B; border-radius:8px; padding:4px 10px; font-size:0.82rem;">')
            print(f'  {avatar_img(_a, AVATARS, 26)} ↔ {avatar_img(_b, AVATARS, 26)}')
            print(f'</div>')
        print('</div>\n')
    ...
    if len(_both) >= 5:
        ...
        print('<div style="margin-top:1rem; padding:10px 14px; background:#1a1a2a; border:1px solid #4363d8; border-radius:8px;">')
        print(f'  <strong>🏛️ Bloco Anti-Jonas/Alberto</strong> — {len(_both)} pessoas nomearam ambos:<br/>')
        print(f'  <div style="margin-top:6px;">{_both_html}</div>')
        print(f'</div>\n')

    # ── Week-over-week comparison with previous Sincerinho ──
    _prev_picks = None
    for _se in _me.get("special_events", []):
        if "sincerinho_picks" in _se:
            _prev_picks = _se["sincerinho_picks"]
            _prev_date = _se.get("date", "?")
            break

    if _prev_picks:
        # Build comparison data
        _common = sorted(set(_prev_picks.keys()) & set(_pp.keys()))
        _gone = sorted(set(_prev_picks.keys()) - set(_pp.keys()))
        _kept = []
        _changed = []
        for _name in _common:
            _old_t = _prev_picks[_name]
            if _old_t in _pp[_name]:
                _kept.append((_name, _old_t))
            else:
                _reason = "saiu" if _old_t in [p for p in _me.get("participants", {}).keys()] and _me["participants"].get(_old_t, {}).get("status") in ("eliminada", "eliminado", "desclassificado", "desistente") else "trocou"
                _changed.append((_name, _old_t, _reason))

        # Previous Sincerinho received counts
        _prev_received = _PPCounter()
        for _a, _t in _prev_picks.items():
            _prev_received[_t] += 1

        print('<h4 style="margin-top:1.5rem;">🔄 Evolução: Sincerinho Semana 4 → Semana 6</h4>')
        print(f'<p class="text-muted" style="font-size:0.82rem;">Na semana 4, cada participante escolheu <strong>1 pessoa</strong> para enfrentar no Paredão. Agora escolheram 3. Comparando as escolhas:</p>\n')

        # Biggest movers (target popularity shift)
        _active_w6 = set(_pp.keys())
        _movers = []
        for _name in _active_w6:
            _w4 = _prev_received.get(_name, 0)
            _w6 = _received.get(_name, 0)
            _delta = _w6 - _w4
            if abs(_delta) >= 2:
                _movers.append((_name, _w4, _w6, _delta))
        _movers.sort(key=lambda x: -x[3])

        if _movers:
            print('<div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:12px;">')
            for _name, _w4, _w6, _delta in _movers:
                _av = avatar_img(_name, AVATARS, 36)
                if _delta > 0:
                    _badge_color = "#E6194B"
                    _arrow = f"🔺 +{_delta}"
                else:
                    _badge_color = "#3CB44B"
                    _arrow = f"🔻 {_delta}"
                print(f'<div style="background:#1a1a1a; border:1px solid {_badge_color}; border-radius:8px; padding:8px 12px; text-align:center; min-width:110px;">')
                print(f'  {_av}<br/>')
                print(f'  <span style="font-size:0.78rem; color:#aaa;">Sem 4: {_w4}×</span><br/>')
                print(f'  <span style="font-size:0.78rem; color:#aaa;">Sem 6: {_w6}×</span><br/>')
                print(f'  <span style="font-weight:bold; color:{_badge_color}; font-size:0.9rem;">{_arrow}</span>')
                print(f'</div>')
            print('</div>\n')

        # Insight callouts
        print('<div style="display:flex; flex-direction:column; gap:8px; margin-bottom:12px;">')

        # Gabriela case
        _gab_w4 = _prev_received.get("Gabriela", 0)
        _gab_w6 = _received.get("Gabriela", 0)
        if _gab_w4 >= 2 and _gab_w6 == 0:
            _old_noms = [a for a, t in _prev_picks.items() if t == "Gabriela"]
            _still_here = [n for n in _old_noms if n in _pp]
            _left = [n for n in _old_noms if n not in _pp]
            _detail_parts = []
            for _n in _left:
                _detail_parts.append(f"{_n} saiu")
            for _n in _still_here:
                _new_t = ", ".join(_pp[_n])
                _detail_parts.append(f"{_n} → {_new_t}")
            print(f'<div style="padding:8px 12px; background:#1a2a1a; border:1px solid #3CB44B; border-radius:6px; font-size:0.82rem;">')
            print(f'  🔍 <strong>Caso Gabriela:</strong> era a mais indicada na sem 4 ({_gab_w4}×), agora 0×. {"; ".join(_detail_parts)}.')
            print(f'</div>')

        # Jonas rise
        _jonas_w4 = _prev_received.get("Jonas Sulzbach", 0)
        _jonas_w6 = _received.get("Jonas Sulzbach", 0)
        if _jonas_w6 >= 5 and _jonas_w4 <= 1:
            print(f'<div style="padding:8px 12px; background:#2a1a1a; border:1px solid #E6194B; border-radius:6px; font-size:0.82rem;">')
            print(f'  🎯 <strong>Ascensão de Jonas:</strong> não participou do Sincerinho da sem 4 (era Líder). Agora é o alvo #1 com {_jonas_w6}× indicações — 3º título consecutivo de Líder o tornou alvo principal.')
            print(f'</div>')

        # Loyalty stat
        _loyalty_pct = round(len(_kept) / len(_common) * 100) if _common else 0
        print(f'<div style="padding:8px 12px; background:#1a1a2a; border:1px solid #4363d8; border-radius:6px; font-size:0.82rem;">')
        print(f'  📊 <strong>Fidelidade:</strong> {len(_kept)}/{len(_common)} ({_loyalty_pct}%) mantiveram o alvo da semana 4 nas suas 3 escolhas.')
        print(f'</div>')

        print('</div>\n')

        # Persistent rivalries (expandable)
        _persistent = [(a, t) for a, t in _kept if t in _pp and a in _pp.get(t, [])]
        _persistent_one_way = [(a, t) for a, t in _kept if t not in _pp or a not in _pp.get(t, [])]

        if _kept:
            print('<details style="margin-top:0.5rem;">')
            print('<summary style="cursor:pointer; font-weight:bold; font-size:0.95rem;">🎯 Rivalidades persistentes (sem 4 → sem 6)</summary>\n')
            print('<table class="table table-striped" style="font-size:0.85rem; margin-top:0.5rem;">')
            print('<thead><tr><th>Quem</th><th>Alvo (mantido)</th><th>Tipo</th></tr></thead>')
            print('<tbody>')
            for _a, _t in _kept:
                _av_a = avatar_img(_a, AVATARS, 28)
                _av_t = avatar_img(_t, AVATARS, 28)
                # Check if mutual now
                _is_mutual = _a in _pp.get(_t, [])
                # Check if was mutual in week 4
                _was_mutual = _prev_picks.get(_t) == _a
                if _is_mutual and _was_mutual:
                    _tipo = '⚔️ Mútua em ambas'
                elif _is_mutual:
                    _tipo = '⚔️ Agora mútua'
                elif _was_mutual:
                    _tipo = '🔓 Era mútua, agora unilateral'
                else:
                    _tipo = '→ Unilateral'
                print(f'<tr><td>{_av_a} {_a}</td><td>{_av_t} {_t}</td><td>{_tipo}</td></tr>')
            print('</tbody></table>')
            print('</details>\n')

        # Week 4 outcome lesson
        print('<div style="margin-top:0.5rem; padding:8px 12px; background:#2a2a1a; border:1px solid #ffe119; border-radius:6px; font-size:0.82rem;">')
        print('  ⚠️ <strong>Lição da semana 4:</strong> Sol Vega escolheu Juliano Floss, mas no Duelo de Risco ambos indicaram <em>Samira</em> em consenso — alguém que nenhum dos dois havia escolhido. A dinâmica pode mudar o alvo original.')
        print('</div>')
...
if _show_prediction:
    ...
    if not _lider_name:
        ...
        html_pred += '🏠 <strong>Sem líder ativo</strong> — previsão de indicação indisponível.</div>'
    else:
        ...
        html_pred += '<h2 id="previsao-indicacao">🎯 Previsão — Indicação do Líder</h2>'
        ...
        html_pred += f'🎯 <strong style="color:#e74c3c;">Previsão de Indicação d{_art} Líder</strong></div>'
        ...
        html_pred += f'<span style="color:#888;">— Líder da Semana {_display_week}</span></span></div>'
        ...
        for _i, (_tname, _tentry) in enumerate(_top3):
            ...
            html_pred += f'<div style="color:#888; font-size:0.75em;">#{_i+1} mais provável</div>'
        ...
        # Anjo note
        if _anjo_name:
            html_pred += f'<div style="color:#3498db; font-size:0.9em;">😇 Anjo: <strong>{_anjo_name}</strong>'
            if _imunizado_nome:
                html_pred += f' — imunizou <strong>{_imunizado_nome}</strong> (bloqueado como alvo)'
            html_pred += '</div>'
        else:
            html_pred += '<div style="color:#888; font-size:0.9em;">😇 Anjo ainda não definido esta semana</div>'
        ...
        for _rank, (_tname, _tentry) in enumerate(_ranked, 1):
            ...
            html_pred += " ".join(_chips) if _chips else '<span style="color:#555;">—</span>'
            html_pred += '</td>'
            ...
            if _tname in _vip_list:
                ...
                html_pred += '<td style="padding:0.4rem; text-align:center; color:#555;">—</td>'
            ...
            if _pair_edges or _hist_fwd:
                ...
                html_pred += f'padding:0.2rem 0;">📋 {len(_pair_edges)} evento(s) · '
                html_pred += f'{len(_hist_fwd)} dia(s) de queridômetro</summary>'
                ...
                if _pair_edges:
                    ...
                    html_pred += '<th style="padding:2px 4px; color:#666; text-align:left;">Direção</th>'
                    ...
                    for _edge in _pair_edges:
                        ...
                        _arrow = f'{_e_actor.split()[0]} → {(_tname if _e_actor == _lider_name else _lider_name).split()[0]}'
                        _detail_parts = []
                ...
                if _hist_fwd:
                    ...
                    html_pred += f'<strong>Queridômetro</strong> (últimos {len(_show_dates)} dias)</div>'
                    html_pred += '<table style="width:100%; border-collapse:collapse; font-size:0.78em;">'
                    html_pred += '<thead><tr style="border-bottom:1px solid #333;">'
                    html_pred += f'<th style="padding:2px 4px; color:#666; text-align:left;">Data</th>'
                    html_pred += f'<th style="padding:2px 4px; color:#666; text-align:center;">{_lider_name.split()[0]}→</th>'
                    html_pred += f'<th style="padding:2px 4px; color:#666; text-align:center;">→{_lider_name.split()[0]}</th>'
                    html_pred += '</tr></thead><tbody>'
        ...
        html_pred += '<strong style="color:#aaa;">📐 Metodologia</strong><br>'
        html_pred += 'A previsão é baseada no <strong>score acumulado do Líder → cada participante</strong>, '
        html_pred += 'calculado pelo sistema de Sentiment Index (queridômetro com memória de streak + '
        html_pred += 'eventos de poder, Sincerão, votos e VIP). O participante com o score mais negativo '
        html_pred += 'é o alvo mais provável de indicação. '
        html_pred += 'Membros do VIP foram escolhidos pelo Líder, indicando aliança — são alvos improváveis. '
        html_pred += '<strong>Isto não é um modelo preditivo dedicado</strong> — é uma leitura dos dados '
        html_pred += 'de relacionamento acumulados até o momento.'
        html_pred += '</div>'
        ...
        if vp_retro and is_finalizado:
            ...
            if vp_ind.get("total", 0) > 0:
                ...
                if all_targets and actual_votes:
                    ...
                    print('<div style="font-size:0.85em; color:#888; margin-bottom:4px;">📊 Previsão do modelo</div>')
                    ...
                    # Actual
                    print('<div style="flex:1; min-width:250px;">')
                    print('<div style="font-size:0.85em; color:#888; margin-bottom:4px;">🗳️ Votação real</div>')
                    for tgt in all_targets[:6]:
    ...
    for nominee in all_nominees:
        ...
        if nominee_indicators:
            for ir in nominee_indicators:
                ...
                print(f'<thead><tr><th>Direção</th><th style="text-align:center;">Reação</th></tr></thead>')
                print(f'<tbody>')
                print(f'<tr><td>{avatar_html(actor, AVATARS, 28)} → {nome}</td><td style="text-align:center; font-size:1.3em;">{a2t}</td></tr>')
                print(f'<tr><td>{avatar_html(nome, AVATARS, 28)} → {actor}</td><td style="text-align:center; font-size:1.3em;">{t2a}</td></tr>')
                print(f'</tbody></table>\n')
        ...
        if n_votes > 0:
            ...
            print('<thead><tr><th>Votante</th><th style="text-align:center;">Deu</th><th style="text-align:center;">Recebeu</th><th>Tipo</th><th>Histórico</th>', end="")
            if has_vp:
            ...
            for v in votes_for_nominee:
                ...
                if has_vp:
                    ...
                    if not pred:
                        print(f'<td style="text-align:center; color:#666;">—</td>')
                    elif pred.get('predicted') == nome:
                        conf = pred.get('confidence', '')
                        print(f'<td style="text-align:center; color:#3CB44B; font-size:0.85em;">✅ {conf}</td>')
                    else:
                        pred_target = pred.get('predicted', '?')
                        short_name = pred_target.split()[0]
                        print(f'<td style="text-align:center; color:#E6194B; font-size:0.85em;">❌→{short_name}</td>')
                print(f'</tr>')
            print('</tbody></table>\n')
