...
""")
        ...
        if poll:
            ...
            # Color scheme: green=best, red=worst, white=middle
            def _poll_highlight(val, all_vals, color_best):
                """Color-code: best(green) · worst(red) · middle(white)."""
                if val == max(all_vals):
                    return f'font-weight:bold; color:#2ecc71;'
                if val == min(all_vals):
                    return f'font-weight:bold; color:#e74c3c;'
                return 'color:#ddd;'
...
if ultimo and ultimo.get('participantes'):
    ...
    for nominee in all_nominees:
        ...
        if n_votes > 0:
            ...
            print(f"*{nome} não recebeu votos da casa — chegou ao paredão por indicação direta.*\n")
