...
""")
        ...
        if tem_votos_casa:
            ...
            for alvo, n in ranking:
                ...
                votantes_html = ' '.join([avatar_img(v, 42) for v in votantes])
                print(f'<tr><td>{avatar_html(alvo, 42)}</td><td style="text-align:center; font-weight:bold; font-size:1.1em;">{n}</td><td>{votantes_html}</td></tr>')
            print('</tbody></table>\n')
        elif n_indicados >= esperado_indicados:
            print(f"\n### 🗳️ Votação da Casa\n")
...
if ultimo and ultimo.get('participantes'):
    ...
    if table_rows:
        ...
        print('<tbody>')
        print(f'<tr><td>🔴 <strong>{rejected_label}</strong></td><td>{avatar_html(worst_paredao["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{worst_paredao["Sentimento"]} sentimento</td></tr>')
        print(f'<tr><td>👎 <strong>{negativas_label}</strong></td><td>{avatar_html(most_neg["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{negs_recebidas.get(most_neg["Indicado"], 0)} reações</td></tr>')
        if deltas:
            biggest_drop = min(deltas.items(), key=lambda x: x[1])
            biggest_rise = max(deltas.items(), key=lambda x: x[1])
            print(f'<tr><td>📉 <strong>Maior queda (3 dias)</strong></td><td>{avatar_html(biggest_drop[0], 28)}</td><td style="text-align:center; font-weight:bold; color:#E6194B;">{biggest_drop[1]:+.1f}</td></tr>')
            print(f'<tr><td>📈 <strong>Maior recuperação (3 dias)</strong></td><td>{avatar_html(biggest_rise[0], 28)}</td><td style="text-align:center; font-weight:bold; color:#4CAF50;">{biggest_rise[1]:+.1f}</td></tr>')
        print(f'<tr><td>⬇️ <strong>Mais dias no Bottom 5</strong></td><td>{avatar_html(most_bottom["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{most_bottom["Dias no Bottom 5"]} dias</td></tr>')
        if most_top['Dias no Top 5'] > 0:
            print(f'<tr><td>⬆️ <strong>Mais dias no Top 5</strong></td><td>{avatar_html(most_top["Indicado"], 28)}</td><td style="text-align:center; font-weight:bold;">{most_top["Dias no Top 5"]} dias</td></tr>')
        print('</tbody></table>')
```
...
if tem_votos and all_matrices:
    ...
    if vote_examples["contraditorio"]:
        ...
        for votante, alvo, label in vote_examples["contraditorio"]:
            print(f"<li>{avatar_html(votante, 24)} votou em {avatar_html(alvo, 24)} apesar de dar ❤️.</li>")
        print("</ul>\n")
    ...
    if ind_examples["alinhado"]:
        ...
        for origem, ator, alvo, label in ind_examples["alinhado"]:
            print(f"<li>{origem}: {avatar_html(ator, 24)} já dava negativa para {avatar_html(alvo, 24)}.</li>")
        print("</ul>\n")
    ...
    if has_temporal_insights and n_daily >= 3:
        ...
        if always_friends_voted:
            ...
            for v in always_friends_voted:
                print(f'<tr><td>{avatar_html(v["votante"], 32)}</td><td>{avatar_html(v["alvo"], 32)}</td><td style="text-align:center; font-weight:bold;">{v["days_as_friends"]} dias de ❤️</td></tr>')
            print('</tbody></table>\n')
    ...
    if lider and indicado:
        ...
        if worst_for_leader and worst_for_leader != indicado and worst_w < 0:
            ...
            if n_daily >= 2 and hist_indicado['pattern'] != 'sem_dados':
                ...
                print(f'<thead><tr><th>Comparação</th><th style="text-align:center;">{avatar_html(indicado, 28)}</th><th style="text-align:center;">{avatar_html(worst_for_leader, 28)}</th></tr></thead>')
                print('<tbody>')
    ...
    if len(df_scatter) > 2:
        ...
        if most_hated is not None and most_voted_name:
            if most_hated['nome'] == most_voted_name:
                print(f'<p style="font-size: 1.05rem;">✅ <strong>Alinhado:</strong> {avatar_html(most_hated["nome"], 28)} é quem recebeu mais reações negativas '
                ...
                if most_voted_data is not None:
                    print(f'<p style="font-size: 1.05rem;">⚠️ <strong>Parcialmente alinhado:</strong> O mais votado foi {avatar_html(most_voted_name, 28)} '
                          f'({int(most_voted_data["votos_recebidos"])} votos, {int(most_voted_data["neg_recebidas"])} negativas), '
                          f'mas quem recebeu mais negatividade no queridômetro é {avatar_html(most_hated["nome"], 28)} '
        ...
        if dinamica and dinamica.get('indicaram') and dinamica.get('indicado'):
            ...
            print(f'{", ".join([avatar_html(n, 28) for n in indicaram])} escolheram indicar {avatar_html(indicado, 28)} ao paredão.')
            print(f'</p>\n')
            ...
            if all_gave_negative:
                print(f'<p style="font-size: 1.05rem;">✅ <strong>Escolha coerente:</strong> Quem decidiu já dava reações negativas para {avatar_html(indicado, 28)} no queridômetro.</p>\n')
            else:
