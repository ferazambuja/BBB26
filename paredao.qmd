...
""")
        ...
        if participantes:
            print(render_nominee_cards_em_andamento(participantes, esperado_indicados, AVATARS, MEMBER_OF))
        ...
        if poll:
            ...
            print(f'<strong style="color:#00bc8c;">🧮 Nosso Modelo</strong> usa as mesmas enquetes, mas dá <em>mais peso para quem acertou mais</em> nos paredões anteriores.')
            print(f'</div>')
            print(f'</div>\n')
            ...
            if _model_pred:
                ...
                if len(_sorted_model) >= 2 and abs(_sorted_model[0][1] - _sorted_model[1][1]) < 2.0:
                    _m_tie_tag = '<span style="background:#f39c12;color:#000;font-size:0.65em;padding:0.15em 0.5em;border-radius:8px;margin-left:0.3em;">EMPATE TÉCNICO</span>'
            ...
            if _model_pred:
                ...
                _diff_m = abs(_sorted_model_c[0][1] - _sorted_model_c[1][1])
                if _diff_m < 2.0:
                ...
                if _model_elim != _consol_elim:
                    ...
                    _curiosities.append(f'🤝 Ambos os modelos apontam <strong>{_consol_elim.split()[0]}</strong>, mas a margem é mínima')
            # 2. Platform split
            ...
            if _min_pct < 10:
                _curiosities.append(f'📉 {_min_voter.split()[0]} tem apenas <strong>{_min_pct:.1f}%</strong> — distância enorme para os outros dois')
            # 4. Time series trend (lead changes)
            ...
            if len(_serie) >= 2:
                ...
                if _lead_changes >= 1:
                    _curiosities.append(f'🔄 Liderança trocou <strong>{_lead_changes}×</strong> desde o início da votação — disputa acirrada!')
            # 5. Total votes milestone
            if total_votos > 1_000_000:
                _m = total_votos / 1_000_000
                _curiosities.append(f'🗳️ Já são <strong>{_m:.1f} milhões</strong> de votos contabilizados')
            # 6. Historical accuracy (best/worst platform + model track record)
            if _prec and _prec.get('sufficient'):
                ...
                if _rmse_hist:
                    ...
                    _curiosities.append(f'🏆 Historicamente, <strong>{_plat_names_h[_best_plat]}</strong> é a fonte mais precisa (erro {_rmse_hist[_best_plat]:.1f} p.p.) e <strong>{_plat_names_h[_worst_plat]}</strong> a menos ({_rmse_hist[_worst_plat]:.1f} p.p.)')
                ...
                if _bt:
                    ...
                    _curiosities.append(f'📊 Em <strong>{_n_finalized} paredões</strong>: Votalhada acertou {_consol_correct}/{_n_finalized} (erro médio {_consol_avg:.1f} p.p.) · Nosso Modelo acertou {_model_correct_n}/{_n_finalized} (erro médio {_model_avg:.1f} p.p.)')
            ...
            if _model_pred:
                ...
                print(f'<p style="margin:0 0 0.5rem 0;">O Votalhada soma votos de todas as fontes — quem tem mais votos pesa mais (Sites ~70%). Mas ter muitos votos não significa acertar o resultado.</p>')
                ...
                for p, w in sorted(_wu.items(), key=lambda x: -x[1]):
                    ...
                    print(f'<tr><td>{_plat_labels_d.get(p,p)}</td><td class="tc">{rmse_val:.1f} p.p.</td><td class="tc">{_vol_approx.get(p,0)}%</td><td class="tc" style="font-weight:bold;">{w*100:.0f}%</td></tr>')
                print(f'</table>')
                print(f'<p style="margin:0.5rem 0 0 0;">Resultado: o erro médio cai de 9,8 para 4,3 p.p. comparado ao consolidado do Votalhada.</p>')
                print(f'</div></details>')
            print()
