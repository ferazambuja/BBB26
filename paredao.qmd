...
# ══════════════════════════════════════════════════════════════════════════════
# LEITURA RÁPIDA — QUERIDÔMETRO DOS INDICADOS (PAREDÃO DATE + HISTÓRICO)
# ══════════════════════════════════════════════════════════════════════════════
# CRITICAL: For finalizado paredões, we MUST use the paredão-date snapshot,
...
if ultimo and ultimo.get('participantes'):
    ...
    # Load precomputed quick insights
    ...
    negs_recebidas = qi.get("negs_recebidas", {})
    ...
    # Build table_rows from precomputed stats
    table_rows = []
    for stat in indicados_stats:
        nome = stat["nome"]
        table_rows.append({
            'Indicado': nome,
            'Grupo': MEMBER_OF.get(nome, '?'),
            'Sentimento': f"{stat['sentimento']:+.1f}",
            'Rank': stat.get('rank', '—'),
            'Δ 3 dias': stat.get('delta_3d', '—'),
            'Dias no Top 5': stat.get('dias_top5', 0),
            'Dias no Bottom 5': stat.get('dias_bottom5', 0),
            'Dias negativos': stat.get('dias_negativos', '—'),
        })

    print("## 🔍 Leitura Rápida dos Indicados\n")
    ...
    if is_finalizado:
        ...
        print(f"Como os emparedados estavam no **queridômetro** ({analysis_date_label.replace('📅 ', '')}).\n")
    ...
    print('<th style="text-align:center;">Rank</th><th style="text-align:center;">Δ 3 dias</th>')
    ...
    for row in table_rows:
        ...
        delta = row['Δ 3 dias']
        delta_color = '#4CAF50' if '▲' in delta or '+' in delta else ('#E6194B' if '▼' in delta else '#888')
        print(f'<tr>')
        print(f'<td>{avatar_html(nome, AVATARS, 32)}</td>')
    ...
    if table_rows:
        ...
        for r in table_rows:
            ...
            if d == '—':
                continue
            try:
                val = float(d.replace('▲', '').replace('▼', '').replace('≈', '').strip())
                deltas[r['Indicado']] = val
            except ValueError:
                pass

        print("\n### 📊 Destaques\n")

        rejected_label = "Mais rejeitado (paredão)" if is_finalizado else "Mais rejeitado hoje"
        negativas_label = "Mais negativas recebidas"

        print('<table class="table" style="max-width: 750px; font-size: 1.05rem;">')
        print('<thead><tr><th>Métrica</th><th>Participante</th><th style="text-align:center;">Valor</th></tr></thead>')
        print('<tbody>')
        print(f'<tr><td>🔴 <strong>{rejected_label}</strong></td><td>{avatar_html(worst_paredao["Indicado"], AVATARS, 28)}</td><td style="text-align:center; font-weight:bold;">{worst_paredao["Sentimento"]} sentimento</td></tr>')
        print(f'<tr><td>👎 <strong>{negativas_label}</strong></td><td>{avatar_html(most_neg["Indicado"], AVATARS, 28)}</td><td style="text-align:center; font-weight:bold;">{negs_recebidas.get(most_neg["Indicado"], 0)} reações</td></tr>')
        if deltas:
            biggest_drop = min(deltas.items(), key=lambda x: x[1])
            biggest_rise = max(deltas.items(), key=lambda x: x[1])
            print(f'<tr><td>📉 <strong>Maior queda (3 dias)</strong></td><td>{avatar_html(biggest_drop[0], AVATARS, 28)}</td><td style="text-align:center; font-weight:bold; color:#E6194B;">{biggest_drop[1]:+.1f}</td></tr>')
            print(f'<tr><td>📈 <strong>Maior recuperação (3 dias)</strong></td><td>{avatar_html(biggest_rise[0], AVATARS, 28)}</td><td style="text-align:center; font-weight:bold; color:#4CAF50;">{biggest_rise[1]:+.1f}</td></tr>')
        print(f'<tr><td>⬇️ <strong>Mais dias no Bottom 5</strong></td><td>{avatar_html(most_bottom["Indicado"], AVATARS, 28)}</td><td style="text-align:center; font-weight:bold;">{most_bottom["Dias no Bottom 5"]} dias</td></tr>')
        if most_top['Dias no Top 5'] > 0:
            print(f'<tr><td>⬆️ <strong>Mais dias no Top 5</strong></td><td>{avatar_html(most_top["Indicado"], AVATARS, 28)}</td><td style="text-align:center; font-weight:bold;">{most_top["Dias no Top 5"]} dias</td></tr>')
        print('</tbody></table>')
```
...
# ══════════════════════════════════════════════════════════════════════════════
# ANÁLISE ESTRATÉGICA DE VOTOS — Baseada em Categorias de Relacionamento
#
# Classifica cada voto de acordo com o tipo de relacionamento entre votante e alvo:
#
# CATEGORIAS DE RELACIONAMENTO (votante → alvo):
# ─────────────────────────────────────────────────────────────────────────────
# 1. INIMIGOS DECLARADOS (mútuo) — Ambos se dão reações negativas
#    → Voto esperado. Ambos se dão reações negativas (mesmo sem saber).
#
# 2. FALSOS AMIGOS — Votante deu ❤️ ao alvo publicamente, mas votou contra
#    → Traição. O alvo achava que tinha um amigo, mas foi apunhalado.
#
# 3. PONTO CEGO — Alvo dá ❤️ ao votante, mas votante dá negativo ao alvo
#    → O votante ataca alguém que o considera amigo. Alvo nem percebe o perigo.
#
# 4. ALIADOS MÚTUOS — Ambos se dão ❤️
#    → Proteção esperada. Se votou contra, é traição grave.
#
# 5. HOSTILIDADE UNILATERAL — Votante dá negativo, alvo também (mas não mútuo)
#    → Voto coerente com a reação dada no queridômetro.
# ══════════════════════════════════════════════════════════════════════════════
...
if tem_votos and all_matrices:
    ...
    analysis_date = paredao.get('data_formacao') or paredao.get('data')

    # Encontrar snapshot mais próximo à data do paredão
    closest_idx = 0
    for i, snap in enumerate(snapshots):
    ...
    # Lista de indicações/contragolpes/dinâmicas (ator -> alvo)
    indicacoes = []
    ...
    def classify_relationship(votante, alvo, matrix):
        ...
        if votante_positive and alvo_positive:
            return 'aliados_mutuos', '💔 Traição de Aliado', '#9b59b6'

        # Votante gave heart but voted against = Falso Amigo
        if votante_positive and not alvo_positive:
            return 'falso_amigo', '🎭 Falso Amigo', '#E6194B'

        # Mutual negativity = Declared enemies
        if votante_negative and alvo_negative:
            return 'inimigos_declarados', '⚔️ Hostilidade Mútua', '#3CB44B'

        # Alvo gave heart but votante gave negative = Blind spot for alvo
        if votante_negative and alvo_positive:
            return 'ponto_cego', '🎯 Ponto Cego do Alvo', '#f39c12'

        # One-sided hostility (votante hostile, alvo neutral/mild)
        if votante_strong_neg:
            return 'hostilidade_forte', '🐍 Hostilidade Forte', '#3CB44B'

        if votante_negative:
            return 'hostilidade_leve', '🌱 Hostilidade Leve', '#FF9800'

        return 'neutro', '❓ Neutro', '#999'

    # ══════════════════════════════════════════════════════════════════════════
    # ANÁLISE TEMPORAL — COMO OS RELACIONAMENTOS EVOLUÍRAM AO LONGO DO TEMPO
    # ══════════════════════════════════════════════════════════════════════════

    def analyze_relationship_history(votante, alvo, daily_matrices, daily_snapshots, analysis_date):
        """
        Analisa a evolução do relacionamento entre votante e alvo ao longo do tempo.

        Retorna um dict com:
        - history: lista de (date, rxn_votante_to_alvo, rxn_alvo_to_votante)
        - pattern: 'sempre_amigos', 'sempre_inimigos', 'ex_amigos', 'recém_inimigos', 'instável'
        - days_as_friends: quantos dias foram amigos mútuos
        - days_as_enemies: quantos dias foram inimigos
        - change_date: data da última mudança significativa (se houver)
        - narrative: texto descritivo da evolução
        """
        history = []
        days_positive = 0
        days_negative = 0
        ...
        for i, mat in enumerate(daily_matrices):
            ...
            if snap_date > analysis_date:
                break  # Don't look past paredão date
        ...
        if total_days == 0:
            return {
                ...
                'change_date': None, 'narrative': 'Sem dados históricos'
            }
        ...
        if days_positive == total_days:
            ...
            narrative = f"Sempre deu ❤️ ({days_positive} dias). Amizade consistente."
        elif days_negative == total_days:
            pattern = 'sempre_inimigos'
            narrative = f"Inimigos desde o início ({days_negative} dias)."
            ...
            if days_since_change <= 2:
                ...
                narrative = f"Eram amigos por {days_positive} dias, mudou há {days_since_change} dia(s)!"
            else:
                pattern = 'ex_amigos'
            ...
            pattern = 'instavel'
            narrative = f"Relacionamento instável: {days_positive}d ❤️, {days_negative}d negativo."
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NARRATIVA PRINCIPAL — O QUE ACONTECEU NESTE PAREDÃO
    # ══════════════════════════════════════════════════════════════════════════

    print(f"## 📖 A História do {paredao['numero']}º Paredão\n")
    ...
    # Opening narrative
    if n_traicoes >= 3:
        print(f"Este paredão foi marcado por **traições**. Nada menos que **{n_traicoes} votos** "
              f"vieram de pessoas que davam ❤️ ao alvo no queridômetro.\n")
    elif n_pontos_cegos >= 3:
        print(f"Este paredão revelou **pontos cegos** perigosos. **{n_pontos_cegos} participantes** "
              f"foram votados por pessoas que consideravam amigas.\n")
    elif n_esperados >= total_votes * 0.5:
        print(f"Este paredão seguiu a lógica das **hostilidades mútuas**. "
              f"A maioria dos votos ({n_esperados}/{total_votes}) veio de quem já dava reações negativas ao alvo.\n")
    else:
        print(f"Este paredão foi **estratégico**. Os votos não seguiram simplesmente as emoções do queridômetro.\n")

    # ══════════════════════════════════════════════════════════════════════════
    # PAREDÃO: COERÊNCIA SOCIAL (votos + indicações vs queridômetro)
    # ══════════════════════════════════════════════════════════════════════════
    ...
    vote_align = Counter()
    vote_examples = {"contraditorio": [], "alinhado": []}
    for votante, alvo in votos.items():
        label = matrix.get((votante, alvo), '')
        status = alignment_from_reaction(label)
        vote_align[status] += 1
        if status in vote_examples and len(vote_examples[status]) < 4:
            vote_examples[status].append((votante, alvo, label))

    ind_align = Counter()
    ind_examples = {"contraditorio": [], "alinhado": []}
    for origem, ator, alvo in indicacoes:
        label = matrix.get((ator, alvo), '')
        status = alignment_from_reaction(label)
        ind_align[status] += 1
        if status in ind_examples and len(ind_examples[status]) < 3:
            ind_examples[status].append((origem, ator, alvo, label))
    ...
    print("### 🧭 Paredão: Coerência Social\n")
    print(f'<p style="font-size: 1.05rem;">Usando o queridômetro do <strong>dia da formação</strong> ({analysis_date}), '
          f'os votos ficaram <strong>{aligned_pct:.0f}% alinhados</strong> e <strong>{contrad_pct:.0f}% contraditórios</strong>. '
          f'Isso indica se o paredão foi mais emocional (alinhado) ou estratégico (contraditório).</p>\n')

    if indicacoes:
        total_ind = len(indicacoes)
        ind_aligned = ind_align["alinhado"]
        ind_contra = ind_align["contraditorio"]
        print(f'<p style="font-size: 1.05rem;">Nas indicações públicas, '
              f'{ind_aligned}/{total_ind} foram coerentes com reações negativas, '
              f'e {ind_contra}/{total_ind} contrariaram o queridômetro.</p>\n')

    if vote_examples["contraditorio"]:
        print("<p style='font-size:1.05rem;'><strong>Contradições de voto (❤️ no queridômetro):</strong></p>")
        print("<ul>")
        for votante, alvo, label in vote_examples["contraditorio"]:
            print(f"<li>{avatar_html(votante, AVATARS, 24)} votou em {avatar_html(alvo, AVATARS, 24)} apesar de dar ❤️.</li>")
        print("</ul>\n")

    if ind_examples["alinhado"]:
        print("<p style='font-size:1.05rem;'><strong>Indicações coerentes:</strong></p>")
        print("<ul>")
        for origem, ator, alvo, label in ind_examples["alinhado"]:
            print(f"<li>{origem}: {avatar_html(ator, AVATARS, 24)} já dava negativa para {avatar_html(alvo, AVATARS, 24)}.</li>")
        print("</ul>\n")

    # ══════════════════════════════════════════════════════════════════════════
    # GRÁFICO: DISTRIBUIÇÃO DOS VOTOS POR TIPO DE RELACIONAMENTO
    # ══════════════════════════════════════════════════════════════════════════
    ...
    rel_labels = {
        'inimigos_declarados': '⚔️ Hostilidade Mútua',
        'hostilidade_forte': '🐍 Hostilidade Forte',
        'hostilidade_leve': '🌱 Hostilidade Leve',
        'ponto_cego': '🎯 Ponto Cego',
        'falso_amigo': '🎭 Falso Amigo',
        'aliados_mutuos': '💔 Traição de Aliado',
        'neutro': '❓ Neutro'
    }
    rel_colors = {
    ...
    rel_descriptions = {
        'inimigos_declarados': 'Ambos se dão reações negativas. Voto coerente.',
        'hostilidade_forte': 'Votante deu reação muito negativa (🐍🎯🤮🤥). Voto coerente.',
        'hostilidade_leve': 'Votante deu reação negativa leve (🌱💼🍪💔). Voto coerente.',
        'ponto_cego': 'Alvo deu ❤️ ao votante, mas votante o detesta. Alvo não viu o perigo.',
        'falso_amigo': 'Votante deu ❤️ ao alvo, mas votou contra. Traição.',
        'aliados_mutuos': 'Ambos trocavam ❤️. Traição grave.',
        'neutro': 'Sem reação clara entre os dois.'
    }
    ...
    fig.update_layout(
        title=f"Distribuição dos Votos por Tipo de Relacionamento<br><sup>{paredao['numero']}º Paredão — Total: {total_votes} votos</sup>",
        height=350,
    ...
    print(f"\n**Resumo:**\n")
    print(f"✅ **{coerentes} votos coerentes** ({pct_coerente:.0f}%)<br>")
    print(f"Votaram em quem davam reação negativa no queridômetro\n")
    print(f"❌ **{incoerentes} votos incoerentes** ({pct_incoerente:.0f}%)<br>")
    print(f"Votaram em quem davam ❤️ publicamente\n")
    print(f"🎯 **{n_pontos_cegos} pontos cegos**<br>")
    print(f"Alvos que confiavam no votante (davam ❤️) mas eram odiados por ele\n")

    # ══════════════════════════════════════════════════════════════════════════
    # AS TRAIÇÕES — FALSOS AMIGOS E ALIADOS QUE TRAÍRAM
    # ══════════════════════════════════════════════════════════════════════════
    ...
    def artigo(nome, definido=True):
        """Retorna artigo definido/indefinido baseado no gênero."""
        if genero(nome) == 'f':
    ...
    if betrayals:
        print("### 🎭 As Traições\n")
        print(f"**{len(betrayals)} voto(s)** de pessoas que davam ❤️ ao alvo — contradição entre reação e voto.\n")
        ...
        if betrayal_targets:
            ...
            traido = "traída" if gen == 'f' else "traído"
            art = "a" if gen == 'f' else "o"
            print(f"**Principal alvo:** {top_target} ({top_count} voto[s]) — {art} mais {traido}\n")

        print('<table class="table table-striped" style="font-size: 1.05rem;">')
        print('<thead><tr><th>Votante</th><th>Alvo</th><th style="text-align:center;">O votante dava</th><th>O alvo retribuía?</th></tr></thead>')
        print('<tbody>')
        for b in betrayals:
            reciproca = f"Sim, {b['emoji_recebido']}" if b['tipo'] == 'aliados_mutuos' else "Não"
            print(f'<tr><td>{avatar_html(b["votante"], AVATARS, 32)}</td><td>{avatar_html(b["alvo"], AVATARS, 32)}</td><td style="text-align:center; font-size:1.2em;">{b["emoji_dado"]}</td><td>{reciproca}</td></tr>')
        print('</tbody></table>\n')

    # ══════════════════════════════════════════════════════════════════════════
    # OS PONTOS CEGOS — QUEM NÃO PERCEBEU O PERIGO
    # ══════════════════════════════════════════════════════════════════════════

    if blind_spots:
        print("### 🎯 Os Pontos Cegos\n")
        print(f"**{len(blind_spots)} voto(s)** de inimigos disfarçados — a vítima dava ❤️ sem saber que era odiada.\n")
        ...
        if blind_spot_victims:
            ...
            print(f"**Maior risco:** {top_victim} — {top_count} pessoas que {ele_ela} achava {amigas_os} votaram contra\n")

        print('<table class="table table-striped" style="font-size: 1.05rem;">')
        print('<thead><tr><th>Vítima</th><th>Votante</th><th style="text-align:center;">A vítima dava</th><th style="text-align:center;">O votante dava</th></tr></thead>')
        print('<tbody>')
        for bs in blind_spots:
            print(f'<tr><td>{avatar_html(bs["alvo"], AVATARS, 32)}</td><td>{avatar_html(bs["votante"], AVATARS, 32)}</td><td style="text-align:center; font-size:1.1em;">{bs["emoji_recebido"]} (confiava)</td><td style="text-align:center; font-size:1.1em;">{bs["emoji_dado"]} (hostil)</td></tr>')
        print('</tbody></table>\n')

    # ══════════════════════════════════════════════════════════════════════════
    # EVOLUÇÃO TEMPORAL — COMO OS RELACIONAMENTOS MUDARAM
    # ══════════════════════════════════════════════════════════════════════════
    ...
    if has_temporal_insights and n_daily >= 3:
        print("### ⏳ Evolução dos Relacionamentos\n")
        print(f"Analisando o **histórico de {n_daily} dias** para entender se os votos seguem padrões antigos ou mudanças recentes.\n")

        # Recent enemies (suspicious timing!)
        if recem_inimigos:
            print("#### 🚨 Mudança de Última Hora\n")
            print(f"**{len(recem_inimigos)} voto(s)** de quem mudou de opinião nos últimos 2 dias — timing suspeito!\n")
            print('<table class="table table-striped" style="font-size: 1.05rem;">')
            print('<thead><tr><th>Votante</th><th>Alvo</th><th>Situação</th></tr></thead>')
            print('<tbody>')
            for v in recem_inimigos:
                print(f'<tr><td>{avatar_html(v["votante"], AVATARS, 32)}</td><td>{avatar_html(v["alvo"], AVATARS, 32)}</td><td>{v["hist_narrative"]}</td></tr>')
            print('</tbody></table>\n')

        # Ex-friends who became enemies
        if ex_amigos:
            print("#### 💔 Ex-Amigos\n")
            print(f"**{len(ex_amigos)} voto(s)** de quem já deu ❤️ no passado mas depois se afastou:\n")
            print('<table class="table table-striped" style="font-size: 1.05rem;">')
            print('<thead><tr><th>Votante</th><th>Alvo</th><th>Situação</th></tr></thead>')
            print('<tbody>')
            for v in ex_amigos:
        ...
        if always_friends_voted:
            print("#### 🗡️ Traição de Amizade de Longa Data\n")
            print(f"**{len(always_friends_voted)} voto(s)** contra quem sempre recebeu ❤️ — traição grave:\n")
            print('<table class="table table-striped" style="font-size: 1.05rem;">')
            ...
            for v in always_friends_voted:
                print(f'<tr><td>{avatar_html(v["votante"], AVATARS, 32)}</td><td>{avatar_html(v["alvo"], AVATARS, 32)}</td><td style="text-align:center; font-weight:bold;">{v["days_as_friends"]} dias de ❤️</td></tr>')
            print('</tbody></table>\n')
        ...
        if sempre_inimigos:
            ...
            if long_enemies:
                print("#### ⚔️ Inimigos de Longa Data\n")
                print(f"**{len(long_enemies)} voto(s)** de rivalidades que já duram ≥5 dias — votos esperados:\n")
                print('<table class="table table-striped" style="font-size: 1.05rem;">')
                ...
                for v in long_enemies[:8]:
                    print(f'<tr><td>{avatar_html(v["votante"], AVATARS, 32)}</td><td>{avatar_html(v["alvo"], AVATARS, 32)}</td><td style="text-align:center; font-weight:bold;">{v["days_as_enemies"]} dias</td></tr>')
                print('</tbody></table>\n')
        ...
        print("\n#### 📊 Resumo Temporal\n")
        print("Classifica cada voto pelo **histórico do relacionamento** entre votante e alvo:\n")
        print("| Padrão | Votos | O que significa |")
        print("|--------|-------|-----------------|")
        if pattern_counts.get('sempre_amigos', 0):
            print(f"| 💚 **Sempre amigos** | {pattern_counts['sempre_amigos']} | Votante sempre deu ❤️ ao alvo — voto inesperado |")
        if pattern_counts.get('sempre_inimigos', 0):
            print(f"| 💀 **Sempre inimigos** | {pattern_counts['sempre_inimigos']} | Votante nunca deu ❤️ ao alvo — voto esperado |")
        if pattern_counts.get('ex_amigos', 0):
            print(f"| 💔 **Ex-amigos** | {pattern_counts['ex_amigos']} | Davam ❤️ no passado, hoje não mais |")
        if pattern_counts.get('recem_inimigos', 0):
            print(f"| ⚡ **Mudança recente** | {pattern_counts['recem_inimigos']} | Mudou de opinião nos últimos 2 dias |")
        if pattern_counts.get('instavel', 0):
            print(f"| 🎭 **Instáveis** | {pattern_counts['instavel']} | Relação oscilou muito ao longo do jogo |")
        print("")

    # ══════════════════════════════════════════════════════════════════════════
    # O MAIS VOTADO — ANÁLISE APROFUNDADA
    # ══════════════════════════════════════════════════════════════════════════

    print(f"### 📊 O Caso {mais_votado}\n")
    ...
    print(f"| Reações negativas recebidas | {mv_neg} |")
    print(f"| Votos de quem dava negativo | {mv_from_enemies} |")
    print(f"| Votos de \"amigos\" (traição) | {mv_from_traitors} |")
    ...
    if mv_from_traitors > n_votos_mais / 2:
        ...
        print(f"> 💔 **{mais_votado} foi {apunhalado_a} pelas costas.** "
              f"Mais da metade dos votos ({mv_from_traitors}/{n_votos_mais}) veio de pessoas "
              f"que lhe davam ❤️ no queridômetro.\n")
    elif mv_from_enemies == n_votos_mais:
        print(f"> ⚔️ **{mais_votado} era um alvo óbvio.** "
              f"Todos os {n_votos_mais} votos vieram de quem já dava reação negativa.\n")
    elif mv_from_blind >= 2:
        print(f"> 🎯 **{mais_votado} não percebia seus inimigos.** "
              f"Dava ❤️ para {mv_from_blind} pessoas que {'a' if gen_mv == 'f' else 'o'} detestavam e votaram contra {'ela' if gen_mv == 'f' else 'ele'}.\n")
    ...
    if lider and indicado:
        ...
        print(f"**{lider}** indicou **{indicado}** ao paredão.\n")
        ...
        print('<thead><tr><th>De</th><th>Para</th><th style="text-align:center;">Reação</th></tr></thead>')
        print('<tbody>')
        print(f'<tr><td>{avatar_html(lider, AVATARS, 32)}</td><td>{avatar_html(indicado, AVATARS, 32)}</td><td style="text-align:center; font-size:1.3em;">{emoji_lider}</td></tr>')
        print(f'<tr><td>{avatar_html(indicado, AVATARS, 32)}</td><td>{avatar_html(lider, AVATARS, 32)}</td><td style="text-align:center; font-size:1.3em;">{emoji_indicado}</td></tr>')
        print('</tbody></table>\n')

        if rel_type == 'aliados_mutuos':
            print(f"> 💔 **Indicação surpreendente!** {lider} e {indicado} trocavam ❤️ mútuos. "
                  f"A indicação foi uma traição de aliança.\n")
        elif rel_type == 'falso_amigo':
            print(f"> 🎭 **Indicação contraditória.** {lider} dava ❤️ para {indicado} no queridômetro. "
                  f"A indicação contradiz a reação dada no queridômetro.\n")
        elif rel_type == 'inimigos_declarados':
            print(f"> ⚔️ **Indicação coerente.** {lider} e {indicado} têm hostilidade mútua no queridômetro. "
                  f"O voto está alinhado com a reação.\n")
        elif rel_type == 'ponto_cego':
            print(f"> 🎯 **{indicado} não viu a indicação chegando.** "
                  f"Dava ❤️ para {lider}, mas {lider} já mostrava hostilidade.\n")
        else:
        ...
        if n_daily >= 2:
            hist_indicado = analyze_relationship_history(lider, indicado, daily_matrices, daily_snapshots, analysis_date)

            # Only show history section if we have data
            if hist_indicado['pattern'] != 'sem_dados':
                print(f"#### 📅 Histórico: {lider} → {indicado}\n")

                if hist_indicado['pattern'] == 'sempre_inimigos':
                    print(f"**{lider}** nunca deu ❤️ para {indicado} — inimizade desde o início ({hist_indicado['days_as_enemies']} dias).\n")
                elif hist_indicado['pattern'] == 'sempre_amigos':
                    print(f"**{lider}** sempre deu ❤️ para {indicado} ({hist_indicado['days_as_friends']} dias). "
                          f"A indicação contradiz o histórico!\n")
                elif hist_indicado['pattern'] == 'ex_amigos':
                    print(f"**{lider}** deu ❤️ por {hist_indicado['days_as_friends']} dias, "
                          f"mas mudou em **{hist_indicado['change_date']}**. "
                          f"Desde então, são {hist_indicado['days_as_enemies']} dias de hostilidade.\n")
                elif hist_indicado['pattern'] == 'recem_inimigos':
                    print(f"⚠️ **Mudança recente!** {lider} dava ❤️ por {hist_indicado['days_as_friends']} dias, "
                          f"mas virou inimigo há poucos dias (em {hist_indicado['change_date']}).\n")
                elif hist_indicado['pattern'] == 'instavel':
                    print(f"Relacionamento instável: {hist_indicado['days_as_friends']} dias de ❤️ "
                          f"vs {hist_indicado['days_as_enemies']} dias de hostilidade.\n")
            else:
                # No reaction data for this leader
                print(f"> ℹ️ **{lider}** não tem histórico de reações no queridômetro — análise indisponível.\n")
        ...
        if worst_for_leader and worst_for_leader != indicado and worst_w < 0:
            ...
            print(f"#### 🔄 Por que não {worst_for_leader}?\n")
            print(f"A pessoa que **{lider}** mais detesta é **{worst_for_leader}** ({emoji_worst}), não {indicado}.\n")

            # Compare history with the alternative
            if n_daily >= 2 and hist_indicado['pattern'] != 'sem_dados':
                hist_worst = analyze_relationship_history(lider, worst_for_leader, daily_matrices, daily_snapshots, analysis_date)

                print('<table class="table" style="max-width: 600px; font-size: 1.05rem;">')
                print(f'<thead><tr><th>Comparação</th><th style="text-align:center;">{avatar_html(indicado, AVATARS, 28)}</th><th style="text-align:center;">{avatar_html(worst_for_leader, AVATARS, 28)}</th></tr></thead>')
                print('<tbody>')
                print(f'<tr><td>Reação do líder</td><td style="text-align:center; font-size:1.2em;">{emoji_lider}</td><td style="text-align:center; font-size:1.2em;">{emoji_worst}</td></tr>')
                print(f'<tr><td>Reação de volta</td><td style="text-align:center; font-size:1.2em;">{emoji_indicado}</td><td style="text-align:center; font-size:1.2em;">{emoji_worst_back}</td></tr>')
                print(f'<tr><td>Dias de ❤️</td><td style="text-align:center; font-weight:bold;">{hist_indicado["days_as_friends"]}</td><td style="text-align:center; font-weight:bold;">{hist_worst["days_as_friends"]}</td></tr>')
                ...
                if hist_worst['days_as_enemies'] > hist_indicado['days_as_enemies']:
                    print(f"> 🧠 **Escolha estratégica.** {worst_for_leader} é inimigo há mais tempo ({hist_worst['days_as_enemies']} dias), "
                          f"mas {lider} escolheu {indicado}.\n")
                elif hist_indicado['pattern'] == 'recem_inimigos':
                    print(f"> ⚡ **Timing suspeito.** {lider} mudou de opinião sobre {indicado} recentemente.\n")

    # ══════════════════════════════════════════════════════════════════════════
    # TABELA COMPLETA DE VOTOS
    # ══════════════════════════════════════════════════════════════════════════

    print("### 📋 Tabela Completa de Votos\n")

    # Pattern emoji mapping
    pattern_emoji = {
        'sempre_amigos': '💚',      # Always friends
        'sempre_inimigos': '💀',    # Always enemies
        'ex_amigos': '💔',          # Ex-friends
        'recem_inimigos': '⚡',     # Recent change
    ...
    print('<table class="table table-striped" style="font-size: 1.05rem;">')
    print('<thead><tr>')
    print('<th>Votante</th><th>Votou em</th><th style="text-align:center;">Deu</th><th style="text-align:center;">Recebeu</th><th>Tipo</th><th>Histórico</th>')
    print('</tr></thead>')
    print('<tbody>')
    for v in vote_analysis:
        hist = f"{pattern_emoji.get(v['hist_pattern'], '?')} {v['days_as_friends']}d❤️"
        print(f'<tr>')
        print(f'<td>{avatar_html(v["votante"], AVATARS, 32)}</td>')
        print(f'<td>{avatar_html(v["alvo"], AVATARS, 32)}</td>')
    ...
    print("\n**Legenda histórico:** 💚 Sempre amigos | 💀 Sempre inimigos | 💔 Ex-amigos | ⚡ Mudança recente | 🎭 Instável\n")

    # ══════════════════════════════════════════════════════════════════════════
    # SCATTER PLOT: NEGATIVIDADE VS VOTOS RECEBIDOS
    # ══════════════════════════════════════════════════════════════════════════

    print("\n### 📈 Reações Negativas vs Votos Recebidos\n")

    print("""<p style="font-size: 1.1rem; line-height: 1.6;"><strong>O que este gráfico mostra?</strong></p>

<p style="font-size: 1.05rem; line-height: 1.6;">
Cada ponto representa um participante. A posição horizontal mostra quantas reações negativas
(🐍🎯🤮🤥💔🌱💼🍪) essa pessoa <strong>recebeu</strong> dos colegas. A posição vertical mostra quantos
<strong>votos</strong> essa pessoa recebeu na votação da casa.
</p>

<p style="font-size: 1.05rem; line-height: 1.6;">
Se os participantes votassem de acordo com o que registraram no queridômetro, veríamos os pontos
seguindo uma linha diagonal: quanto mais reações negativas recebeu, mais votos receberia.
</p>

<p style="font-size: 1.05rem; line-height: 1.6;">
A <strong>correlação (r)</strong> mede essa relação:<br>
• <strong>r próximo de 1</strong>: forte relação (quem recebe mais negatividade recebe mais votos)<br>
• <strong>r próximo de 0</strong>: sem relação (votos são estratégicos, não seguem o queridômetro)<br>
• <strong>r negativo</strong>: relação inversa (quem é mais odiado recebe menos votos — raro)
</p>
...
""")
    ...
    if len(df_scatter) > 2:
        ...
        fig = px.scatter(
            ...
            title=f"Quem é mais detestado recebe mais votos? (r = {corr_val:.2f})",
            labels={'neg_recebidas': 'Reações negativas recebidas', 'votos_recebidos': 'Votos na casa'}
        )
        ...
        # (In the flattened paredão structure: lider, anjo, imunizado are at top level)
        # ══════════════════════════════════════════════════════════════════════════
        nao_votaveis = set()

        # Líder can't be voted
        if ultimo.get('lider'):
        ...
        # People already indicated to paredão can't receive house votes
        # (they're already nominated via Líder, Dinâmica, Big Fone, etc.)
        for p in ultimo.get('participantes', []):
        ...
        # Filter out people who couldn't be voted for "não alinhado" analysis
        ...
        if abs(corr_val) < 0.3:
            print(f'<p style="font-size: 1.05rem;"><strong>Correlação fraca (r = {corr_val:.2f})</strong> — Os votos <strong>não seguem</strong> as reações do queridômetro.</p>')
            print('<p style="font-size: 1.05rem;">Isso significa que os participantes estão votando de forma <strong>estratégica</strong>, não de acordo com suas reações. '
                  'Alguém pode receber muitas reações negativas no queridômetro mas poucos votos (está sendo protegido ou é útil como aliado), '
                  'enquanto alguém com poucas negativas pode receber muitos votos (é visto como ameaça no jogo).</p>\n')
        elif corr_val > 0.5:
            print(f'<p style="font-size: 1.05rem;"><strong>Correlação forte (r = {corr_val:.2f})</strong> — Os votos <strong>seguem</strong> o queridômetro.</p>')
            print('<p style="font-size: 1.05rem;">Os participantes estão votando de acordo com o que registraram no queridômetro. '
                  'Quem recebe mais reações negativas tende a receber mais votos. '
                  'Isso indica que as relações pessoais estão pesando mais que a estratégia de jogo.</p>\n')
        else:
            print(f'<p style="font-size: 1.05rem;"><strong>Correlação moderada (r = {corr_val:.2f})</strong> — Mistura de queridômetro e estratégia.</p>')
            print('<p style="font-size: 1.05rem;">Existe alguma relação entre negatividade no queridômetro e votos recebidos, mas não é perfeita. '
                  'Alguns votos seguem as reações negativas, outros seguem estratégia de jogo.</p>\n')
        ...
        if len(poupados) > 0:
            print("\n#### 🛡️ Poupados Suspeitos\n")
            print('<p style="font-size: 1.05rem;">Participantes com muitas reações negativas no queridômetro mas que receberam poucos ou nenhum voto. '
                  'Podem estar sendo <strong>protegidos estrategicamente</strong> pela casa.</p>\n')

            print('<table style="font-size: 1.05rem; width: 100%;">')
            print('<thead><tr><th>Participante</th><th style="text-align:center;">Negativas</th><th style="text-align:center;">Votos</th><th>Observação</th></tr></thead>')
            print('<tbody>')
            for _, row in poupados.iterrows():
                obs = "Recebeu muita negatividade mas nenhum voto" if row['votos_recebidos'] == 0 else f"Só {int(row['votos_recebidos'])} voto apesar da negatividade"
                print(f'<tr>')
                print(f'<td>{avatar_html(row["nome"], AVATARS, 28)}</td>')
        ...
        if len(alvos) > 0:
            print("\n#### ⚠️ Alvos Surpreendentes\n")
            print('<p style="font-size: 1.05rem;">Participantes com poucas reações negativas no queridômetro mas que receberam muitos votos. '
                  'Estão sendo <strong>votados por estratégia</strong>, não por antipatia declarada.</p>\n')

            print('<table style="font-size: 1.05rem; width: 100%;">')
            print('<thead><tr><th>Participante</th><th style="text-align:center;">Negativas</th><th style="text-align:center;">Votos</th><th>Observação</th></tr></thead>')
            print('<tbody>')
            for _, row in alvos.iterrows():
                obs = f"Recebeu {int(row['votos_recebidos'])} votos mas só {int(row['neg_recebidas'])} negativas — alvo estratégico"
                print(f'<tr>')
                print(f'<td>{avatar_html(row["nome"], AVATARS, 28)}</td>')
                print(f'<td style="text-align:center; font-size:1.2em;">{int(row["neg_recebidas"])}</td>')
                print(f'<td style="text-align:center; font-size:1.2em;">{int(row["votos_recebidos"])}</td>')
        ...
        if most_hated is not None and most_voted_name:
            if most_hated['nome'] == most_voted_name:
                print(f'<p style="font-size: 1.05rem;">✅ <strong>Alinhado:</strong> {avatar_html(most_hated["nome"], AVATARS, 28)} é quem recebeu mais reações negativas '
                      f'({int(most_hated["neg_recebidas"])}) e também mais votos '
                      f'({int(most_hated["votos_recebidos"])}). Os votos seguiram o queridômetro.</p>\n')
            else:
                ...
                if most_voted_data is not None:
                    print(f'<p style="font-size: 1.05rem;">⚠️ <strong>Parcialmente alinhado:</strong> O mais votado foi {avatar_html(most_voted_name, AVATARS, 28)} '
                          f'({int(most_voted_data["votos_recebidos"])} votos, {int(most_voted_data["neg_recebidas"])} negativas), '
                          f'mas quem recebeu mais negatividade no queridômetro é {avatar_html(most_hated["nome"], AVATARS, 28)} '
        ...
        if nao_votaveis:
            ...
            if nao_votaveis_list:
                ...
                if indicados_nomes:
                    indicados_str = ", ".join([f'{avatar_html(n, AVATARS, 24)}' for n in indicados_nomes])
                    print(f'<em>Nota: {indicados_str} já estava(m) indicado(s) ao paredão (por Líder ou Dinâmica), então não podia(m) receber votos da casa.</em>')
                if outros_nomes:
                    outros_str = ", ".join([f'{avatar_html(n, AVATARS, 24)}' for n in outros_nomes])
                    print(f'<br><em>{outros_str} não podia(m) receber votos (líder, imunizado ou anjo autoimune).</em>')
                print('</p>\n')

        # Compare with previous paredão if available
        if len(paredoes) > 1:
            prev_paredao = paredoes[-2]
            if prev_paredao.get('votos_casa'):
                print("\n#### Comparação com o paredão anterior\n")
                print(f'<p style="font-size: 1.05rem;">No <strong>{prev_paredao.get("titulo", "paredão anterior")}</strong>, a dinâmica pode ter sido diferente. ')
                if abs(corr_val) < 0.3:
                    print('Neste paredão, a correlação é <strong>fraca</strong> — os participantes estão votando mais estrategicamente.</p>\n')
                else:
                    print('Neste paredão, a correlação é mais forte — os votos estão mais alinhados com o queridômetro.</p>\n')

        # ══════════════════════════════════════════════════════════════════════════
        # ANÁLISE DA DINÂMICA (if there was one)
        # ══════════════════════════════════════════════════════════════════════════
        ...
        if dinamica and dinamica.get('indicaram') and dinamica.get('indicado'):
            ...
            print(f"\n#### 🎲 A Decisão da Dinâmica: {dinamica.get('nome', 'Dinâmica')}\n")

            print(f'<p style="font-size: 1.05rem;">')
            print(f'{", ".join([avatar_html(n, AVATARS, 28) for n in indicaram])} escolheram indicar {avatar_html(indicado, AVATARS, 28)} ao paredão.')
            print(f'</p>\n')

            print(f'<p style="font-size: 1.05rem;"><strong>O que o queridômetro diz sobre essa escolha?</strong></p>\n')
            ...
            pattern_emoji = {'sempre_amigos': '💚', 'sempre_inimigos': '💀', 'ex_amigos': '💔', 'recem_inimigos': '⚡', 'instavel': '🎭', 'sem_dados': '—'}

            # Build analysis table with historical data
            print('<table style="font-size: 1.05rem; width: 100%;">')
            print('<thead><tr><th>Quem decidiu</th><th style="text-align:center;">Deu</th><th style="text-align:center;">Recebeu</th><th>Histórico</th><th>Interpretação</th></tr></thead>')
            print('<tbody>')

            for decisor in indicaram:
                # What did decisor give to indicado?
                dado = matrix.get((decisor, indicado), '')
                emoji_dado = REACTION_EMOJI.get(dado, '—')

                # What did indicado give to decisor?
                recebido = matrix.get((indicado, decisor), '')
                emoji_recebido = REACTION_EMOJI.get(recebido, '—')

                # Get historical relationship data
                hist = get_hist_summary(decisor, indicado)
                hist_str = f"{pattern_emoji.get(hist['pattern'], '?')} {hist['days_pos']}d❤️ / {hist['days_neg']}d👎"

                # Interpretation based on current + historical
                if dado in (STRONG_NEGATIVE | MILD_NEGATIVE):
                    if hist['pattern'] == 'sempre_inimigos':
                        interp = f"Inimigos desde o início ({hist['days_neg']} dias)"
                    elif hist['pattern'] == 'ex_amigos':
                        interp = f"Ex-amigos — virou inimigo em {hist['change']}"
                    elif hist['pattern'] == 'recem_inimigos':
                        interp = f"⚠️ Mudança recente! Era amigo até {hist['change']}"
                    else:
                        interp = "Não gostava do indicado"
                elif dado == 'Coração':
                    if hist['pattern'] == 'sempre_amigos':
                        interp = f"Sempre foram amigos ({hist['days_pos']} dias) — surpreendente!"
                    else:
                        interp = "Gostava do indicado — escolha inesperada"
                else:
                    interp = "—"

                print(f'<tr>')
                print(f'<td>{avatar_html(decisor, AVATARS, 28)}</td>')
            ...
            print('</tbody></table>\n')
            print('<p style="font-size: 1.05rem; color: #aaa;"><em>Legenda histórico: 💚 Sempre amigos | 💀 Sempre inimigos | 💔 Ex-amigos | ⚡ Mudança recente | 🎭 Instável</em></p>\n')
            ...
            if all_gave_negative:
                print(f'<p style="font-size: 1.05rem;">✅ <strong>Escolha coerente:</strong> Quem decidiu já dava reações negativas para {avatar_html(indicado, AVATARS, 28)} no queridômetro.</p>\n')
            else:
                print(f'<p style="font-size: 1.05rem;">⚠️ <strong>Escolha estratégica:</strong> A indicação não seguiu necessariamente as reações do queridômetro — pode ter sido por estratégia de jogo.</p>\n')

else:
    print("*Aguardando votação da casa para análise estratégica.*")
```
