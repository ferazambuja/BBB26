...
""")
        ...
        if poll:
            ...
            print(f'<div style="font-size:0.8em; color:#9b59b6; font-weight:bold; margin-bottom:0.3rem;">📊 VOTALHADA (CONSOLIDADO)</div>')
            print(f'<div style="font-size:1.3em; font-weight:bold;">{_consol_elim}</div>')
            print(f'<div style="color:#888;">com {_consol_pct:.1f}% dos votos</div>')
            print(f'</div>')
            # Model card
            if _model_pred:
                _model_elim = _model_pred['predicao_eliminado']
                _model_pct = _model_pred['prediction'].get(_model_elim, 0)
                _agree = '✅ Concordam' if _model_elim == _consol_elim else '⚠️ Divergem'
                ...
                print(f'<div style="font-size:0.8em; color:#00bc8c; font-weight:bold; margin-bottom:0.3rem;">🎯 MODELO PONDERADO</div>')
                print(f'<div style="font-size:1.3em; font-weight:bold;">{_model_elim}</div>')
                print(f'<div style="color:#888;">com {_model_pct:.1f}% dos votos</div>')
                print(f'<div style="margin-top:0.5rem; font-size:0.8em; color:#888;">{_agree}</div>')
                print(f'</div>')
            print(f'</div>\n')
            ...
            # Model row
            if _model_pred:
                print(f'<tr style="background:rgba(0,188,140,0.2);"><td style="font-weight:bold;">🎯 Modelo Ponderado</td>')
                for nome in participantes_poll:
                    ...
                    print(f'<td style="text-align:center; {style}">{val:.1f}%</td>')
                print(f'<td style="text-align:center; color:#888; font-size:0.8em;">ponderado</td>')
                print('</tr>')
            ...
            if _model_pred:
                ...
                print(f'<summary style="cursor:pointer; color:#888;">ℹ️ Pesos do modelo: {" · ".join(_weight_parts)}</summary>')
                ...
                print(f'<p style="margin:0 0 0.5rem 0;"><strong>Por que esses pesos?</strong> O Votalhada pondera pelo volume de votos — Sites recebem ~70% do peso porque têm os maiores veículos (UOL Splash, CNN). Mas Sites são a plataforma <strong>menos precisa</strong> (RMSE {_rmse_info.get("sites", "?"):.1f} p.p.) porque sobre-representam fanbases organizadas.</p>')
                print(f'<p style="margin:0 0 0.5rem 0;"><strong>Nosso modelo</strong> usa o inverso do RMSE² (peso = 1/RMSE²). Plataformas mais precisas recebem exponencialmente mais peso:</p>')
                print(f'<table style="font-size:0.9em; margin:0.3rem 0;">')
                print(f'<tr><th>Plataforma</th><th style="text-align:center;">RMSE</th><th style="text-align:center;">Votalhada</th><th style="text-align:center;">Modelo</th></tr>')
                ...
                for p, w in sorted(_wu.items(), key=lambda x: -x[1]):
                    ...
                    print(f'<tr><td>{_plat_labels.get(p,p)}</td><td style="text-align:center;">{rmse_val:.1f} p.p.</td><td style="text-align:center;">{_vol_approx.get(p,0)}%</td><td style="text-align:center; font-weight:bold;">{w*100:.0f}%</td></tr>')
                print(f'</table>')
                print(f'<p style="margin:0.5rem 0 0 0;">Validado com leave-one-out (erro cai de 9,8 → 4,3 p.p.). Pesos recalculados a cada paredão finalizado.</p>')
                print(f'</div></details>')
            print()
...
if ultimo and (ultimo.get('status') == 'finalizado' or any(p.get('resultado') for p in ultimo.get('participantes', []))):
        ...
        if poll and poll.get('resultado_real'):
            ...
            print(f'{status_html}')
            print(f'<span style="margin-left:0.5rem; color:#888;">Erro: <strong>{erro_medio:.1f}</strong> p.p.</span>')
            ...
            if _model_pred_fin and _model_mae_fin is not None:
                ...
                print(f'<span style="margin-left:0.5rem; color:#888;">Erro: <strong>{_model_mae_fin:.1f}</strong> p.p.</span>')
            print(f'</div>\n')
...
if _pp_data:
    ...
    if _not_nom:
        ...
        _html += f'🛡️ <strong>Não indicados:</strong> {_not_html}</div>\n'
    _html += '</div>\n'
    print(_html)
    ...
    if _duelo:
        ...
        _duel_html += f'<div style="background:#1a1a2a; border:1px solid #4363d8; border-radius:8px; padding:10px 14px; text-align:center; min-width:140px;">'
        _duel_html += f'<div style="font-size:0.75rem; color:#888;">📞 Big Fone</div>'
        ...
        _duel_html += f'<div style="display:flex; align-items:center; font-size:1.5rem; color:#888;">→</div>'
        _duel_html += f'<div style="background:#2a2a1a; border:1px solid #ffe119; border-radius:8px; padding:10px 14px; text-align:center; min-width:140px;">'
        _duel_html += f'<div style="font-size:0.75rem; color:#888;">🎯 Escolheu</div>'
        ...
        _duel_html += f'<div style="display:flex; align-items:center; font-size:1.5rem; color:#888;">→</div>'
        _duel_html += f'<div style="background:#2a1a1a; border:1px solid #E6194B; border-radius:8px; padding:10px 14px; text-align:center; min-width:120px;">'
        _duel_html += f'<div style="font-size:0.75rem; color:#888;">🔥 Paredão</div>'
        _duel_html += f'{avatar_img(_emparedado, AVATARS, 40)}<br/>'
        _duel_html += f'<strong style="color:#E6194B;">{_emparedado}</strong></div>'

        _duel_html += f'<div style="background:#1a2a1a; border:1px solid #3CB44B; border-radius:8px; padding:10px 14px; text-align:center; min-width:120px;">'
        _duel_html += f'<div style="font-size:0.75rem; color:#888;">🛡️ Imune</div>'
