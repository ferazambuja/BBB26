...
if leaderboard:
    ...
    for i, p in enumerate(leaderboard):
        ...
        tags = []
        if wins > 0:
            tags.append(f'<span style="background: #383838; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.75rem; margin: 0.15rem; display: inline-block;">🏆 {wins} vitória{"s" if wins > 1 else ""}</span>')
        if top3 > 0:
            tags.append(f'<span style="background: #383838; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.75rem; margin: 0.15rem; display: inline-block;">🥉 {top3} top-3</span>')
        tags.append(f'<span style="background: #383838; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.75rem; margin: 0.15rem; display: inline-block;">📊 avg {avg:.1f} pts</span>')
        tags.append(f'<span style="background: #383838; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.75rem; margin: 0.15rem; display: inline-block;">📋 {participated}/{available} provas</span>')
...
<div style="font-size: 0.85rem; color: #aaa; margin-top: 0.2rem;">📅 {date} &middot; {emoji} {label} &middot; Semana {week} &middot; {n_participantes} participantes</div>
<div style="margin-top: 0.4rem;">
<span style="background: {color}; color: #fff; padding: 0.2rem 0.6rem; border-radius: 5px; font-size: 0.85rem; font-weight: bold;">🏆 Vencedor: {vencedor}</span>
</div>
</div>
</div>
''')
        ...
        for fase in fases:
            ...
            if classificacao:
                ...
                print('<table class="table-full" style="font-size: 0.9rem;">')
        ...
        for fase in fases:
            ...
            if quartas and semis and final:
                ...
                print('<div class="tc text-muted" style="font-size: 0.75rem; font-weight: bold; margin-bottom: 4px;">Quartas</div>')
                for d in quartas:
                    print(matchup_box(d))
                print('</div>')
                ...
                # Column 2: Semis
                print('<div style="display: flex; flex-direction: column; justify-content: space-around; gap: 0.5rem;">')
                print('<div class="tc text-muted" style="font-size: 0.75rem; font-weight: bold; margin-bottom: 4px;">Semis</div>')
                for d in semis:
                    print(matchup_box(d))
                print('</div>')
                ...
                print('<div class="tc text-muted" style="font-size: 0.75rem; font-weight: bold; margin-bottom: 4px;">Final</div>')
                for d in final:
                    print(matchup_box(d))
                    winner = d["vencedor"]
                    print(f'<div class="tc" style="margin-top: 6px; font-size: 1rem;">🏆 <span style="color: #f1c40f; font-weight: bold;">{winner}</span></div>')
                print('</div>')
        ...
        if scored_entries:
            print(f'\n<details style="margin-top: 0.5rem;"><summary style="cursor: pointer; color: #3498db; font-size: 0.9rem;">📊 Pontos atribuídos (multiplicador {label}: {mult_label})</summary>')
...
<p class="text-muted" style="font-size: 0.8rem; margin-top: 0.8rem; margin-bottom: 0;">Fórmula: <code>pontos_base × multiplicador</code></p>
</div>
