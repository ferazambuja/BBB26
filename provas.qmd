...
<div style="font-size: 2rem; font-weight: bold; color: #fff;">{total_provas}</div>
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">📋 Provas</div>
</div>
...
<div style="font-size: 2rem; font-weight: bold; color: #fff;">{unique_winners}</div>
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">🏆 Vencedores únicos</div>
</div>
...
<div style="font-size: 2rem; font-weight: bold; color: #fff;">{best_avg_p["avg_points"]:.1f}</div>
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">📊 Melhor média ({best_avg_p["name"].split()[0]})</div>
</div>
...
''')
        ...
        if excluidos:
            ...
            print(f'\n<div style="background: #333; border-radius: 8px; padding: 0.6rem 1rem; margin-bottom: 1rem; font-size: 0.85rem; color: #aaa;">🚫 Não participaram: {nomes_exc}</div>\n')
        ...
        for fase in fases:
            ...
            if classificacao:
                ...
                if any(e.get("pontuacao") is not None for e in classificacao):
                    print('<th style="text-align: right; padding: 0.4rem; color: #888;">Pontuação</th>')
                ...
                for entry in classificacao:
                    ...
                    if has_scores:
                        ...
                        pontuacao_cell = f'<td style="text-align: right; padding: 0.4rem; color: #3498db; font-weight: bold;">{score if score is not None else "—"}</td>'
        ...
        for fase in fases:
            ...
            if quartas and semis and final:
                ...
                for d in final:
                    print(matchup_box(d))
                    winner = d["vencedor"]
                    print(f'<div style="text-align: center; margin-top: 6px; font-size: 1rem;">🏆 <span style="color: #f1c40f; font-weight: bold;">{winner}</span></div>')
                print('</div>')
        ...
        if scored_entries:
            ...
            print(f'<th style="text-align: right; padding: 0.3rem; color: #888;">× {mult_label}</th>')
            print('</tr></thead><tbody>')
...
for pos, pts in sorted(placement_pts.items(), key=lambda x: int(x[0])):
    ...
    print(f'<tr><td style="color: #ccc; padding: 0.3rem 0;">{medal} {pos}º lugar</td><td style="color: #3498db; text-align: right; font-weight: bold; padding: 0.3rem 0;">{pts}</td></tr>')

print(f'<tr><td style="color: #888; padding: 0.3rem 0;">9º+ lugar</td><td style="color: #888; text-align: right; font-weight: bold; padding: 0.3rem 0;">{default_pts}</td></tr>')
...
for tipo, mult in sorted(multipliers.items(), key=lambda x: -x[1]):
    ...
    label = TIPO_LABEL.get(tipo, tipo.title())
    color = TIPO_COLOR.get(tipo, "#888")
    print(f'<tr><td style="color: #ccc; padding: 0.3rem 0;">{emoji} {label}</td><td style="color: {color}; text-align: right; font-weight: bold; padding: 0.3rem 0;">{mult:.2f}×</td></tr>')

print('''
</table>
<p style="color: #888; font-size: 0.8rem; margin-top: 0.8rem; margin-bottom: 0;">Fórmula: <code>pontos_base × multiplicador</code></p>
</div>
