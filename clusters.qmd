...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">Coesão Média</div>
</div>
...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">Tensões</div>
</div>
...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">Contradições</div>
</div>
...
for comm in communities:
    ...
    for m in members:
        ...
        short = m.split()[0]
        if avatar_url:
            avatar_html += f'<img src="{avatar_url}" title="{m}" style="width:36px;height:36px;border-radius:50%;margin:2px;border:2px solid {color};" />'
        else:
            avatar_html += f'<div style="display:inline-block;width:36px;height:36px;border-radius:50%;background:#444;text-align:center;line-height:36px;margin:2px;border:2px solid {color};font-size:12px;">{short[0]}</div>'
    ...
    card_html += '</div>'
    card_html += f'<div style="margin:0.6rem 0;">{avatar_html}</div>'
...
<tr style="border-bottom: 2px solid #555;">
  ...
  <th style="padding: 8px; text-align: center;">Queridômetro</th>
...
for entry in top3:
    ...
    avatar_html = f'<img src="{avatar}" style="width:48px;height:48px;border-radius:50%;border:2px solid {cluster_colors[cl]};" />' if avatar else ""
    ...
    p_html += f'<div style="display:flex;gap:0.7rem;align-items:center;margin-bottom:0.5rem;">{avatar_html}<div><div style="font-weight:700;">{name}</div><div style="font-size:0.8rem;color:#aaa;">{entry["grupo"]} · {cluster_names[cl]}</div></div></div>'
