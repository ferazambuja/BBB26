...
print(f'''
<div class="info-panel" style="border-left-color: #2ecc71; border-radius: 8px; padding: 0.8rem 1rem; margin: 1rem 0;">
<div style="font-weight: 700; margin-bottom: 0.5rem;">Destaques — Semana {current_week}</div>
...
<p class="text-muted small" style="margin-top: 0.3rem;">Baseado em scores compostos (queridômetro + votos + eventos de poder + Sincerão + VIP). Referência: {ref_date}. Resolução Louvain: {resolution_val}.</p>
''')
```
...
for comm in communities:
    ...
    card_html += f' <div class="fs-base" style="display:inline;color:#aaa;margin-left:0.5rem;">{len(members)} membros · {comp_str}</div>'
    card_html += f' <div class="fs-sm" style="display:inline;background:#333;padding:2px 8px;border-radius:12px;margin-left:auto;">Coesão: {cohesion:+.2f}</div>'
    card_html += '</div>'
    card_html += f'<div style="margin:0.6rem 0;">{members_html}</div>'
    card_html += f'<div class="fs-base" style="color:#ccc;">🤝 Aliança externa: <b>{ext_alliance}</b> &nbsp;·&nbsp; ⚔️ Rivalidade externa: <b>{ext_rivalry}</b></div>'
    card_html += '</div>'
    print(card_html)
```
...
#| output: asis
print(f'''
<div class="info-panel fs-base" style="border-left-color: #3498db; border-radius: 8px; padding: 0.6rem 1rem; margin: 0.5rem 0;">
...
if not voting_blocs:
    ...
    for bloc in sorted(voting_blocs, key=lambda b: (b.get("week", 0), -b.get("count", 0))):
        ...
        for v in voters:
            short = v.split()[0]
            if v in multi_bloc:
                voter_tags.append(f'<b class="fs-md" style="background:#f39c12;color:#000;padding:1px 6px;border-radius:10px;font-weight:600;">{short}</b>')
            else:
                voter_tags.append(f'<b class="fs-md" style="background:#444;padding:1px 6px;border-radius:10px;font-weight:normal;">{short}</b>')
        ...
        bloc_html += f'<div style="font-weight:600;">Semana {week} — Alvo: <b style="color:#e74c3c;">{target}</b> <b style="color:#888;">({count} votos)</b></div>'
    ...
    if multi_bloc:
        ...
        print(f'\n<p class="text-muted small" style="margin-top: 0.5rem;">Votantes em múltiplos blocos (destaque amarelo): {multi_names}</p>')
```
...
if not contradictions_list:
    ...
    print('''
<div class="scroll-x" style="margin: 1rem 0;">
<table class="table-full fs-base">
<thead>
<tr style="border-bottom: 2px solid #555;">
  <th class="tl" style="padding: 8px;">Ator</th>
  <th class="tl" style="padding: 8px;">Alvo</th>
  <th class="tc" style="padding: 8px;">Queridômetro</th>
  <th class="tc" style="padding: 8px;">Voto</th>
  <th class="tc" style="padding: 8px;">Tipo</th>
  <th class="tc" style="padding: 8px;">Semana</th>
</tr>
</thead>
<tbody>
''')
  ...
  <td class="tc" style="padding: 6px 8px;">{kind}</td>
  <td class="tc" style="padding: 6px 8px;">{week}</td>
</tr>''')
...
for entry in top3:
    ...
    p_html += f'<div style="display:flex;gap:0.7rem;align-items:center;margin-bottom:0.5rem;">{avatar_img_html}<div><div style="font-weight:700;">{name}</div><div class="fs-md" style="color:#aaa;">{entry["grupo"]} · {cluster_names[cl]}</div></div></div>'
    p_html += f'<div class="fs-base">'
    p_html += f'<div style="color:#aaa;">Dispersão: <b>{entry["std_received"]:.2f}</b> · Média: <b>{entry["avg_received"]:+.2f}</b></div>'
    p_html += f'<div style="margin-top:0.3rem;">💚 Mais positivos: {love_str}</div>'
    p_html += f'<div>🔴 Mais negativos: {hate_str}</div>'
    p_html += f'<div style="color:#f39c12;margin-top:0.2rem;">⚡ Contradições: {entry["contradictions"]}</div>'
    p_html += '</div></div>'
    print(p_html)
...
if evolution_path.exists():
    ...
    if len(timeline) >= 2:
        ...
        print(f'''
<div class="info-panel" style="border-left-color: #9b59b6; border-radius: 8px; padding: 0.8rem 1rem; margin: 1rem 0;">
<div style="font-weight: 700; margin-bottom: 0.5rem;">Resumo da Evolução ({n_weeks} semanas)</div>
        ...
        if most_mobile:
            ...
            for item in most_mobile:
                ...
                if avatar_url:
                    ...
                    print(f'<span style="background:#2a2a2a;padding:4px 10px;border-radius:6px;margin:2px;"><strong>{name.split()[0]}</strong> ({moves}×)</span>')
            print('</div>\n')

        # ── Transitions table ──
        if all_transitions:
            print("\n### 📋 Registro de Transições\n")
            print('<table class="table table-sm fs-base" style="max-width: 600px;">')
