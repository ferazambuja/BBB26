...
subtitle: "Laboratório experimental: análise de agrupamentos e padrões de relacionamento"
lang: pt-BR
format:
  html:
    code-tools: false
...
Esta página contém análises experimentais de clustering. Os algoritmos e visualizações estão em desenvolvimento e podem mudar.
:::
...
from data_utils import load_snapshot, get_all_snapshots, build_reaction_matrix, require_clean_manual_events
...
# Clustering por Sentimento Mútuo

Agrupamento hierárquico baseado em sentimento mútuo entre participantes. Participantes no mesmo cluster tendem a se dar bem entre si.
...
## Avaliação de k (Silhouette Score)

O silhouette score mede quão bem cada participante se encaixa no seu cluster. Valores mais altos indicam clusters mais bem definidos.
...
fig.update_layout(
    title=f"Silhouette Score por Número de Clusters (Ótimo: k={int(optimal_k)})",
...
<strong>Resultado:</strong> O algoritmo sugere <strong>k={int(optimal_k)}</strong> clusters como a melhor divisão
...
if not rivalry_pairs and not alliance_pairs:
    print("*Clusters relativamente independentes — sem tensões ou afinidades fortes.*")
```
...
Esta página é experimental. Melhorias planejadas:

- [ ] **Auto-naming de clusters** — Nomear clusters por característica dominante (ex: "Núcleo Veterano")
- [ ] **Temporal tracking** — Acompanhar como clusters mudam ao longo do tempo
- [ ] **Vote-based clustering** — Agrupar por padrões de votação (após 3+ paredões)
- [ ] **Dendrograma interativo** — Visualização da árvore hierárquica
...
<ul class="mb-0">
  ...
  <li><a href="trajetoria.html">📈 Trajetória</a> — Evolução ao longo do tempo</li>
