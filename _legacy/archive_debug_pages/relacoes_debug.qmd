...
description: "Tally completo do score de relações A→B no BBB 26 — queridômetro + eventos"
---
...
if not bfc or not bfc.get("attendees"):
    ...
    html += f'<span class="fs-base meta-inline">Alvo real: <strong class="u-s282">{ACTUAL_TARGET}</strong></span>'
    html += '</div>'
    ...
    html += '<strong class="meta-inline">Nota sobre consensus %:</strong> '
...
if not vp_by_paredao:
    ...
    if enh.get("total"):
        ...
        html_vp += f'<div class="tc u-s047"><div class="fs-md meta-text">Modelo Aprimorado</div><div class="text-pos fs-4xl">{enh_pct:.1f}%</div><div class="text-muted table-md">{enh["correct"]}/{enh["total"]}</div></div>'
        html_vp += f'<div class="tc u-s047"><div class="fs-md meta-text">Baseline (sentimento)</div><div class="fs-4xl u-s043">{base_pct:.1f}%</div><div class="text-muted table-md">{base["correct"]}/{base["total"]}</div></div>'
        html_vp += f'<div class="tc u-s047"><div class="fs-md meta-text">Diferença</div><div class="fs-4xl" style="color:{diff_color}; font-weight:bold;">{diff_str}</div><div class="text-muted table-md">aprimorado vs baseline</div></div>'
        html_vp += '</div></div>'
