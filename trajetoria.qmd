...
power_events = manual_events.get("power_events", [])
```
...
Esses registros são **manuais** (via `power_events`) e servem para enriquecer a análise do jogo — **não têm relação com Cartola**.
...
if not power_events:
    ...
    fig = px.scatter(
        ...
        color='Impacto',
        symbol='Tipo',
        color_discrete_map={'Benefício': '#2ecc71', 'Prejuízo': '#e74c3c', 'Neutro': '#95a5a6'},
        hover_data={
            'actor': True,
            'source': True,
            'detail': True,
        ...
        labels={'target': 'Quem recebeu', 'actor': 'Quem deu', 'source': 'Origem', 'detail': 'Detalhe'},
        title='Poderes e Consequências ao longo do tempo'
    )
    fig.update_layout(height=420, xaxis_title='', yaxis_title='')
    fig.show()

    df_show = df_power.sort_values('Data')[['date', 'Tipo', 'actor', 'target', 'detail']]
    df_show.columns = ['Data', 'Tipo', 'Quem deu', 'Quem recebeu', 'Consequência']
...
if not power_events:
    ...
    if top_pos and top_neg:
        print(f"**Mais beneficiado:** {top_pos} (+{int(impact.loc[top_pos, 'positivo'])})")
        print(f"**Mais prejudicado:** {top_neg} (-{int(impact.loc[top_neg, 'negativo'])})")
```
...
if not df_timeline.empty:
    ...
    # Collect endpoint data for avatars
    avatar_positions = []
    latest_date = df_timeline['Data'].max()
    ...
    for name in sorted_participants:
        ...
        # Store position for avatar
        if name in AVATARS:
            avatar_positions.append({
                'name': name,
                'x': latest_date,
                'y': latest_rank,
                'url': AVATARS[name]
    ...
    # Add avatars at endpoints
    # For datetime x-axis, sizex is in milliseconds
    date_range_days = (df_timeline['Data'].max() - df_timeline['Data'].min()).days
    one_day_ms = 24 * 60 * 60 * 1000
    avatar_size_x = one_day_ms * max(1, date_range_days * 0.06)  # ~6% of x range in ms
    avatar_size_y = 0.8  # ~0.8 rank positions

    for av in avatar_positions:
        fig.add_layout_image(
            dict(
                source=av['url'],
                xref="x",
                yref="y",
                x=av['x'] + pd.Timedelta(days=0.3),  # Slight offset to the right
                y=av['y'],
                sizex=avatar_size_x,
                sizey=avatar_size_y,
                xanchor="left",
    ...
    fig.update_layout(
        ...
        # Extend x-axis to make room for avatars
        xaxis=dict(
            range=[df_timeline['Data'].min() - pd.Timedelta(days=0.5),
                   df_timeline['Data'].max() + pd.Timedelta(days=date_range_days * 0.15)]
        ),
    )
