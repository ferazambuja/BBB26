---
title: "BBB 26 — Trajetória"
subtitle: "Evolução do jogo ao longo da temporada: alianças, rivalidades e tendências"
lang: pt-BR
format:
  html:
    code-tools: false
...
if late_entrants:
    ...
    late_caption = (
        ...
        f"têm menos dias de dados comparados aos demais participantes que estavam presentes "
        f"desde o início da temporada, e isso pode afetar comparações temporais."
    )
else:
    late_caption = ""
...
# Auto power events from API (Líder / Anjo / Monstro / Imune) — prefer derived file
power_events_auto = []
...
Esta página mostra a **evolução do jogo** ao longo da temporada do Big Brother Brasil 26. Aqui você encontra análises acumuladas: alianças que persistem, rivalidades que crescem, e tendências de longo prazo.
...
📈 Analisando <strong>{n_daily} dias</strong> de dados ({daily_snapshots[0]['label']} → {daily_snapshots[-1]['label']})
</div>
""")
```
...
print(
    ...
    f'<strong>Nota:</strong> Participantes que entraram após o início ({late_names}) '
    'têm menos dias de dados.'
    '</div>'
)
```
...
Linha do tempo dos principais eventos da temporada: entradas, saídas e mudanças de roles.
...
for i, snap in enumerate(snapshots):
    ...
    if i > 0:
        ...
        if appeared:
            events_data.append({
                'Data': snap['date'],
                'Evento': f"✅ Entraram: {', '.join(sorted(appeared))}",
    ...
    for p in snap['participants']:
        roles = parse_roles(p.get('characteristics', {}).get('roles', []))
        for role in roles:
            if role in ('Líder', 'Paredão') and i > 0:
...
if events_data:
    ...
    # Mostrar apenas eventos importantes (entradas/saídas/roles)
    key_events = df_events[df_events['Evento'].str.contains('Entraram|Saiu|Líder')]
    if not key_events.empty:
        display(key_events.style.hide(axis='index'))
```

# Sincerão (Evolução semanal) {#sincerao}

Evolução semanal dos sinais do Sincerão (agregados), comparando **pódio**, **não ganha** e **planta**.  
São sinais **raros** e não substituem o queridômetro — servem como **modificadores leves**.

```{python}
#| label: sincerao-timeline
#| output: asis

if not sincerao_data or not sincerao_data.get("aggregates"):
    print("*Sem dados de Sincerão ainda.*")
else:
    agg = sincerao_data.get("aggregates", [])
    rows = []
    for entry in agg:
        week = entry.get("week")
        date = entry.get("date")
        for name, count in entry.get("podio_mentions", {}).items():
            rows.append({"week": week, "date": date, "name": name, "metric": "pódio", "value": count})
        for name, count in entry.get("nao_ganha_mentions", {}).items():
            rows.append({"week": week, "date": date, "name": name, "metric": "não ganha", "value": count})
        for name in entry.get("sem_podio", []) or []:
            rows.append({"week": week, "date": date, "name": name, "metric": "sem pódio", "value": 1})
        planta = entry.get("planta")
        if planta:
            rows.append({"week": week, "date": date, "name": planta, "metric": "planta", "value": 1})

    df = pd.DataFrame(rows)
    if df.empty:
        print("*Sem métricas agregadas do Sincerão.*")
    else:
        fig = px.bar(
            df,
            x="name",
            ...
            barmode="group",
            facet_row="week",
            title="Sincerão — métricas agregadas por semana",
            labels={"value": "Menções", "name": "Participante", "metric": "Sinal"},
        )
        fig.update_layout(height=380 + 140 * df["week"].nunique(), margin=dict(l=20, r=20, t=60, b=40))
        fig.show()
```

# Poderes & Consequências {#poderes}

Eventos de **dinâmicas da casa** que concedem poderes ou impõem consequências.  
Esses registros vêm de **dois lugares**: `power_events` manuais e eventos **auto‑detectados da API** (Líder/Anjo). **Não têm relação com Cartola**.
...
if not power_events:
    ...
    df_show = df_power.sort_values('Data')[['date', 'Tipo', 'actor', 'target', 'detail', 'impacto']]
    ...
    print('<thead><tr style="text-align: left; border-bottom: 1px solid #444;">'
          ...
          '<th style="padding: 0.5rem;">Consequência</th>'
    ...
    def render_person_cell(value, border):
        ...
        lower = label.lower()
        icon = next((icon_map[k] for k in icon_map if k in lower), '🎬')
...
if len(daily_snapshots) >= 2:
    ...
    if days_leading > 1:
        facts.append(f"há **{days_leading} dias** consecutivos")

    detail_facts = []
    if biggest_rise and biggest_rise[1] > 2:
        detail_facts.append(f"• Maior subida: **{biggest_rise[0]}** (+{biggest_rise[1]} posições)")
    if biggest_drop and biggest_drop[1] < -2:
        detail_facts.append(f"• Maior queda: **{biggest_drop[0]}** ({biggest_drop[1]} posições)")
    if new_in_top5:
        names = ', '.join(sorted(new_in_top5))
        detail_facts.append(f"• Novos no Top 5: **{names}**")

    details_str = "<br>".join(detail_facts) if detail_facts else ""

    print(f"""
::: {{.callout-note title="Fatos Rápidos" appearance="simple"}}
{' '.join(facts[:2])}
...
### Evolução do Sentimento {#evolucao}
...
if late_entrants:
    ...
    for d, names in sorted(_by_date.items()):
        ...
        _parts.append(f"{names_str} (entraram em {d})")
    _late_str = '; '.join(_parts)
    print(f'<p class="text-muted small">⚠️ Participantes que entraram após o início do confinamento ({_late_str}) têm menos dias de dados.</p>')
```
...
O **bump chart** mostra a **posição** de cada participante no ranking de sentimento. Mais fácil de acompanhar movimentações do que o gráfico de linhas.
...
if not df_timeline.empty:
    ...
    fig.update_layout(
        title="Evolução do Ranking de Sentimento (Bump Chart)",
        xaxis_title="Data",
        yaxis_title="Posição no Ranking",
        yaxis=dict(
...
Gráfico de linhas mostrando o **score absoluto** de sentimento. Top 3 e bottom 3 são destacados por padrão.
...
if not df_timeline.empty:
    ...
    # Paredão date lines (load from paredoes.json)
...
if len(daily_snapshots) >= 3:
    # Reuse ranking computation from Fatos Rápidos
    ...
    if leader_first_rank != 1 and leader_first_rank != "?":
        sections.append(f"**🏆 {current_leader}** subiu da **{leader_first_rank}ª posição** para o topo do ranking. "
                       f"Com **{leader_score:+.1f}** pontos de sentimento, consolidou-se como o participante mais querido.")
    else:
        sections.append(f"**🏆 {current_leader}** lidera com **{leader_score:+.1f}** pontos de sentimento.")

    # Rise story
    if biggest_rise and biggest_rise[1] >= 3:
        name, change, current_rank = biggest_rise
        grupo = MEMBER_OF.get(name, '?')
        sections.append(f"**📈 Maior Ascensão:** **{name}** ({grupo}) subiu **{change} posições** e agora está em {current_rank}º lugar. "
                       f"Uma trajetória de recuperação que vale acompanhar.")

    # Fall story
    if biggest_fall and biggest_fall[1] <= -3:
        name, change, current_rank = biggest_fall
        grupo = MEMBER_OF.get(name, '?')
        sections.append(f"**📉 Maior Queda:** **{name}** ({grupo}) caiu **{abs(change)} posições** para o {current_rank}º lugar. "
                       f"Pode estar em risco se houver paredão.")
    ...
    if most_stable and most_stable[1] <= 2:
        ...
        grupo = MEMBER_OF.get(name, '?')
        if change == 0:
            sections.append(f"**⚓ Mais Estável:** **{name}** ({grupo}) manteve a mesma posição ({rank}º) desde o início. "
                           f"Consistência pode indicar uma base sólida de apoio.")
        else:
            sections.append(f"**⚓ Mais Estável:** **{name}** ({grupo}) variou apenas {change} posição (de {first_rank}º para {rank}º).")
...
if len(daily_snapshots) >= 2:
    ...
    # Build callout
    facts = [f"**{n_stable}** alianças estáveis (❤️ mútuo em >70% dos dias)"]

    if most_connected and max_allies > 0:
        facts.append(f"• Mais conectado: **{most_connected}** ({max_allies} aliados)")

    if n_isolated > 0:
        isolated_names = ', '.join(sorted(isolated)[:3])
        if n_isolated > 3:
            isolated_names += f" e +{n_isolated-3}"
        facts.append(f"• Sem alianças estáveis: **{isolated_names}**")
...
Quem são os aliados? Análise das relações positivas mais duradouras da temporada.
...
# Calcular alianças mútuas ao longo do tempo (1 por dia)
...
for (a, b), count in top_alliances:
    alliance_data.append({
        'Par': f"{a} ↔ {b}",
        'Dias com ❤️ mútuo': count,
        'Total de dias': n_daily,
        'Consistência': f"{count/n_daily*100:.0f}%"
    })

df_alliances = pd.DataFrame(alliance_data)

fig = go.Figure(go.Bar(
    x=[a['Dias com ❤️ mútuo'] for a in alliance_data],
...
if rivalry_data:
    fig = go.Figure(go.Bar(
        ...
        orientation='h',
        marker_color='#d73027',
        text=[r['Persistência'] for r in rivalry_data],
        textposition='outside',
...
Os membros do mesmo grupo (Vip/Xepa) favorecem uns aos outros com mais ❤️?
...
if not df_fav.empty:
    ...
    fig.update_layout(
        ...
        xaxis_title="Data",
        yaxis_title="% de reações que são ❤️",
        height=400,
        yaxis_range=[50, 100]
    )
    ...
    if avg_bias > 2:
        print(f"Viés médio: **{avg_bias:+.1f} pontos percentuais** a favor do próprio grupo.")
    elif avg_bias < -2:
        print(f"Viés médio: **{avg_bias:+.1f} pp** — surpreendentemente, há *mais* corações para o grupo oposto!")
    else:
        print(f"Viés médio: **{avg_bias:+.1f} pp** — praticamente nenhum favoritismo de grupo.")
```
...
if len(daily_snapshots) >= 3:
    # Compute alliance stats (similar to Fatos Rápidos but with more detail)
    ...
    if strongest:
        ...
        if same_group:
            sections.append(f"**❤️ Aliança Mais Forte:** **{a}** e **{b}** (ambos {grupo_a}) trocaram corações mútuos em "
                           f"**{days} de {n_daily} dias** ({pct:.0f}%). Uma aliança sólida dentro do mesmo grupo.")
        else:
            sections.append(f"**❤️ Aliança Mais Forte:** **{a}** ({grupo_a}) e **{b}** ({grupo_b}) mantêm corações mútuos há "
                           f"**{days} dias** ({pct:.0f}%). Uma ponte entre grupos que pode ser estratégica.")
    ...
    if most_connected and len(most_connected[1]) >= 3:
        ...
        sections.append(f"**🌟 Mais Conectado:** **{name}** ({grupo}) tem **{len(allies)} aliados estáveis**: {ally_list}. "
                       f"Uma posição social forte que pode proteger em votações.")
    ...
    if isolated:
        ...
        sections.append(f"**⚠️ Sem Aliados Estáveis:** {isolated_str}. "
                       f"Participantes isolados são mais vulneráveis em paredões — sem aliados para defendê-los na votação.")
...
if len(daily_snapshots) >= 2:
    ...
    for giver, receiver, from_rxn, to_rxn in changes_yesterday:
        ...
        if from_positive and to_strong_neg:
            ...
            drama_score = 1  # ❤️ → mild negative
        elif not from_positive and to_positive:
            drama_score = 1  # negative → ❤️
    ...
    # Build callout
    facts = [f"**{n_changes}** reações mudaram ontem ({change_pct:.0f}% do total)"]
    facts.append(f"Volatilidade: **{volatility_level}**")

    detail_facts = []
    if biggest_flip:
        detail_facts.append(f"• Maior virada: **{biggest_flip[0]}** → {biggest_flip[1]} ({biggest_flip[2]} → {biggest_flip[3]})")
    if top_viracasaca:
        detail_facts.append(f"• Mais instável: **{top_viracasaca[0]}** ({top_viracasaca[1]} mudanças totais)")
...
### O Que Está Mudando? {#tendencias}

Quem muda de opinião? Quais relacionamentos estão se transformando?

#### Quem Muda Mais de Opinião? ("Vira-Casacas")

Participantes que mais mudaram suas reações ao longo do tempo.
...
# Normalizar pelo número de transições diárias em que estiveram ativos
...
for name, changes in giver_change_count.most_common():
    ...
    if trans >= 3:  # mínimo de 3 transições
...
df_flip = pd.DataFrame(flipflop_data).sort_values('Mudanças', ascending=True)
...
if not df_flip.empty:
    fig = go.Figure(go.Bar(
        ...
        text=[f"{m} ({t:.1f}/dia)" for m, t in zip(df_flip['Mudanças'], df_flip['Taxa'])],
        textposition='outside',
        hovertemplate='%{y}: %{x} mudanças totais<extra></extra>'
    ))

    fig.update_layout(
        title="Vira-Casacas: Quem Muda Mais de Opinião?",
        xaxis_title="Total de mudanças de reação",
        yaxis_title="",
...
if len(daily_snapshots) >= 3:
    ...
    for i in range(1, len(daily_snapshots)):
        ...
        for pair in common:
            if prev_m[pair] != curr_m[pair]:
                ...
                from_rxn = prev_m[pair]
                to_rxn = curr_m[pair]
                # Check if dramatic (positive ↔ strong negative)
    ...
    if top_viracasaca:
        ...
        sections.append(f"**🔄 Mais Instável:** **{name}** ({grupo}) mudou de opinião **{changes} vezes** "
                       f"(média de {avg_per_day:.1f} mudanças/dia). Opiniões voláteis podem indicar indecisão ou estratégia.")
    ...
    if _dramatic_flips:
        ...
        if from_rxn in POSITIVE:
            sections.append(f"**💔 Virada Dramática:** **{giver}** mudou de {from_emoji} para {to_emoji} com **{receiver}** "
                           f"em {date}. Uma amizade que virou hostilidade.")
        else:
            sections.append(f"**💚 Reconciliação:** **{giver}** mudou de {from_emoji} para {to_emoji} com **{receiver}** "
    ...
    if len(_changes_per_day) >= 3:
        ...
        if recent_avg > earlier_avg * 1.5:
            sections.append(f"**📈 Tendência:** O jogo está **mais volátil** recentemente. "
                           f"Média de {recent_avg:.0f} mudanças/dia nos últimos 3 dias vs {earlier_avg:.0f} no início.")
        elif recent_avg < earlier_avg * 0.7:
            sections.append(f"**📉 Tendência:** O jogo está **mais estável** recentemente. "
                           f"As opiniões parecem estar se consolidando.")
...
As hostilidades mais duradouras da temporada — quem se detesta há mais tempo e quem mantém mágoa unilateral.

#### Rivalidades Mais Longas

Pares que se detestam mutuamente há mais tempo.
...
if top_persistent_two:
    ...
    fig = go.Figure(go.Bar(
        ...
        orientation='h',
        marker_color='#d73027',
        text=[d['Persistência'] for d in persistent_two_data],
        textposition='outside',
        hovertemplate='%{y}: %{x}/' + str(n_daily) + ' dias<extra></extra>',
    ))

    fig.update_layout(
        title="Top 10 Rivalidades Mais Persistentes (Hostilidade Mútua)",
        xaxis_title=f"Dias de hostilidade mútua (de {n_daily})",
        yaxis_title="",
    ...
    if top_persistent_two:
        ...
        if longest[1] >= n_daily * 0.8:
            print(f"\n> 🔥 **Rivalidade central:** {a} vs {b} se detestam há **{longest[1]} dias** "
                  f"({longest[1]/n_daily*100:.0f}% da temporada). Este é um conflito estrutural do jogo.")
else:
...
Quem mais tempo detesta alguém que lhe dá ❤️.
...
if top_persistent_one:
    ...
    for (hostile, friendly), days in top_persistent_one:
        ...
        persistent_one_data.append({
            'Relação': f"{hostile} 😠→ {friendly} ❤️",
            'Dias': days,
            'Persistência': f"{pct:.0f}%"
        })

    fig = go.Figure(go.Bar(
        y=[d['Relação'] for d in persistent_one_data],
        x=[d['Dias'] for d in persistent_one_data],
        orientation='h',
        marker_color='#f39c12',
        text=[d['Persistência'] for d in persistent_one_data],
        textposition='outside',
        hovertemplate='%{y}: %{x}/' + str(n_daily) + ' dias<extra></extra>',
    ))
    ...
    if top_persistent_one:
        ...
        if longest[1] >= n_daily * 0.5:
            print(f"\n> ⚠️ **Ponto cego persistente:** {friendly} dá ❤️ para {hostile} "
                  f"há **{longest[1]} dias**, mas {hostile} continua hostil. "
...
if not df_balance.empty:
    ...
    # Top 3 e bottom 3 pelo último saldo — visíveis por padrão
...
if len(daily_snapshots) >= 1:
    ...
    if longest_rivalry:
        ...
        detail_facts.append(f"• Rivalidade mais longa: **{pair_names[0]} vs {pair_names[1]}** ({longest_rivalry[1]} dias)")
    if most_polarizing:
        detail_facts.append(f"• Mais polarizante: **{most_polarizing[0]}** ({most_polarizing[1]} inimigos mútuos)")
...
### Grafo de Relações {#grafo}

<p class="text-muted small">📸 <strong>Dado do dia</strong> — mostra apenas a coleta mais recente.</p>

Rede mostrando alianças (❤️ mútuo = verde) e rivalidades (negativa mútua = vermelho) na última coleta.
...
# Adicionar arestas de aliança e rivalidade
checked = set()
...
fig.update_layout(
    ...
    annotations=[
        dict(text="Tamanho do nó = sentimento | Cor = grupo (🔴 Camarote, 🟢 Veterano, 🔵 Pipoca)",
...
Análise de hostilidades **unilaterais** (A detesta B, mas B dá ❤️ para A) e **mútuas** (ambos se detestam).
Esses padrões revelam dinâmicas importantes do jogo: hostilidades unilaterais criam **pontos cegos** —
quem dá corações a inimigos pode ser surpreendido na votação.
...
summary_lines = [
    f"**📊 Resumo das Hostilidades — {latest['label']}**",
    "",
    f"**{n_two}** hostilidades mútuas (ambos se detestam) | **{n_one}** hostilidades unilaterais (A detesta B, mas B dá ❤️)",
    "",
    "**Por que isso importa?**",
    "- **Hostilidade mútua** = inimigos declarados. Votos previsíveis, sem surpresas.",
    "- **Hostilidade unilateral** = *ponto cego*. Quem dá ❤️ a um inimigo pode ser traído na votação.",
]
...
print("Participantes que dão mais reações negativas para pessoas que lhes dão corações.\n")
...
if hostility['hostile_giver']:
    ...
    fig = go.Figure(go.Bar(
        ...
        textposition='outside',
        hovertemplate='%{y}: %{x} ataques a quem lhe deu ❤️<extra></extra>',
    ))

    fig.update_layout(
        title="Quem Mais Ataca Quem Lhe Dá ❤️ (Hostilidade Unilateral)",
        xaxis_title="Nº de pessoas que dão ❤️ mas recebem negativa",
        yaxis_title="",
...
print("Participantes que dão corações para pessoas que os detestam — *pontos cegos* no jogo.\n")
...
if hostility['loving_victim']:
    ...
    for name, count in top_victims:
        ...
        victim_data.append({
            ...
            '❤️ a Inimigos': count, 'Ataca Amigos': given_hostility,
    ...
    fig = go.Figure(go.Bar(
        ...
        x=[d['❤️ a Inimigos'] for d in victim_data],
        orientation='h',
        marker_color=[GROUP_COLORS.get(d['Grupo'], '#666') for d in victim_data],
        text=[f"{d['❤️ a Inimigos']} (vuln: {d['Vulnerabilidade']:.1f})" for d in victim_data],
        textposition='outside',
        hovertemplate='%{y}: dá ❤️ para %{x} pessoas que o detestam<extra></extra>',
    ))

    fig.update_layout(
        title="Quem Mais Dá ❤️ a Quem O Detesta (Pontos Cegos)",
        xaxis_title="Nº de pessoas que recebem ❤️ mas dão negativa",
        yaxis_title="",
    ...
    if victim_data:
        ...
        if most_vulnerable['Vulnerabilidade'] > 2:
            print(f"\n> ⚠️ **Alerta de vulnerabilidade:** {most_vulnerable['Participante']} tem "
                  f"índice de vulnerabilidade {most_vulnerable['Vulnerabilidade']:.1f} — "
                  f"dá muitos ❤️ a inimigos e pode ser surpreendido(a) em votações.")
else:
...
print("Participantes com mais hostilidades mútuas (ambos se detestam).\n")
...
if hostility['two_sided_count']:
    ...
    for name, count in top_enemies:
        grupo = MEMBER_OF.get(name, '?')
        enemy_data.append({'Participante': name, 'Grupo': grupo, 'Inimigos Mútuos': count})

    df_enemies = pd.DataFrame(enemy_data)

    fig = go.Figure(go.Bar(
        y=[d['Participante'] for d in enemy_data],
        x=[d['Inimigos Mútuos'] for d in enemy_data],
        orientation='h',
        marker_color=[GROUP_COLORS.get(d['Grupo'], '#666') for d in enemy_data],
        text=[d['Inimigos Mútuos'] for d in enemy_data],
        textposition='outside',
        hovertemplate='%{y}: %{x} inimigos declarados<extra></extra>',
    ))

    fig.update_layout(
        title="Quem Tem Mais Inimigos Declarados (Hostilidade Mútua)",
        xaxis_title="Nº de inimigos mútuos",
        yaxis_title="",
    ...
    print("*Nenhuma hostilidade mútua detectada.*")
```
...
if hostility['loving_victim']:
    ...
    print("**Pontos Cegos** — Participantes que dão muitos ❤️ a quem os detesta (maior risco de serem traídos em votações):\n")
    for name, received, given in most_vulnerable:
        ratio = received / (given + 1)
        print(f"- **{name}**: dá ❤️ para {received} inimigos, ataca {given} amigos → vulnerabilidade {ratio:.1f}")
    print()

# Find most polarizing participants
if hostility['two_sided_count']:
    most_enemies = hostility['two_sided_count'].most_common(3)
    print("**Participantes Mais Polarizantes** — Quem tem mais inimigos declarados (hostilidade mútua):\n")
    for name, count in most_enemies:
        print(f"- **{name}**: {count} inimigos mútuos")
    print()

# Connection to voting
print("""
**Relevância para Votações:**

- **Hostilidade mútua** → votos previsíveis entre inimigos declarados
- **Hostilidade unilateral** → votos surpresa (quem dá ❤️ pode ser traído)
- Participantes com alta vulnerabilidade têm **pontos cegos** — confiam em quem não deviam
- No 1º Paredão, Paulo Augusto foi votado por 11 pessoas, **6 das quais lhe davam ❤️** — exemplo clássico de ponto cego
""")
```
...
A análise de clusters (agrupamento por afinidade) foi movida para uma [página experimental dedicada](clusters.html).

**O que você encontra lá:**

- Avaliação automática do número ideal de clusters (silhouette score)
- Grupos identificados e sua composição
- Dinâmica entre clusters (tensões e afinidades)
- Mapa de afinidade (heatmap ordenado por cluster)

[🧪 Ver análise de clusters →](clusters.html)
:::
...
Ranking de quem distribui mais reações negativas.
...
if len(daily_snapshots) >= 2:
    ...
    _two_sided = []
    _one_sided = []
    _blind_spots = []  # (victim, attacker) - victim gives ❤️ to attacker who hates them
    checked = set()
    ...
    for (a, b), rxn_a_to_b in latest_matrix.items():
        ...
        if a_dislikes and b_dislikes and pair not in checked:
            ...
            _blind_spots.append((b, a))  # b is the victim (gives ❤️, receives hate)
    ...
    if most_blind and most_blind[1] >= 2:
        ...
        sections.append(f"**⚠️ Maior Ponto Cego:** **{victim}** ({grupo}) dá ❤️ para **{count} pessoas** que o(a) detestam "
                       f"({attacker_list}). Em uma votação, pode receber votos surpresa de quem considera aliado.")
    ...
    if longest_rivalry:
        ...
        pct = days / n_daily * 100
        if pct >= 80:
            sections.append(f"**🔥 Rivalidade Estrutural:** **{a}** vs **{b}** se detestam há **{days} dias** ({pct:.0f}% do jogo). "
                           f"Este conflito parece ser central na dinâmica da casa.")
        elif pct >= 50:
            sections.append(f"**⚔️ Rivalidade Persistente:** **{a}** vs **{b}** mantêm hostilidade mútua há **{days} dias**. "
                           f"Uma tensão que pode explodir em votação.")

    # Voting prediction insight
    if _one_sided and len(_one_sided) >= 3:
        n_blind = len(_one_sided)
        sections.append(f"**🗳️ Previsão:** Com **{n_blind} pontos cegos** ativos, há risco de votos surpresa. "
                       f"Participantes que confiam em quem os detesta são os mais vulneráveis.")
...
<ul class="mb-0">
  <li><a href="index.html">📊 Painel</a> — Estado atual das reações</li>
  <li><a href="mudancas.html">📅 O Que Mudou</a> — Mudanças do dia anterior</li>
  <li><a href="paredao.html">🗳️ Paredão</a> — Paredão atual</li>
  <li><a href="paredoes.html">📚 Arquivo</a> — Histórico de paredões</li>
</ul>
</div>
