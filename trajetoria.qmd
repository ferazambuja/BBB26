...
Como as reações evoluem ao longo do tempo.

#### Mudanças Entre Dias

Quantas reações mudaram entre cada par de dias consecutivos.
...
for i in range(1, len(daily_snapshots)):
    ...
    change_counts.append({
        'Transição': f"{daily_snapshots[i-1]['date']} → {daily_snapshots[i]['date']}",
...
if not df_changes.empty:
    ...
    fig.add_trace(go.Bar(
        x=df_changes['Transição'], y=df_changes['❤️→Neg'],
        name='❤️ → Negativa', marker_color='#d73027'
    ))
    fig.add_trace(go.Bar(
        x=df_changes['Transição'], y=df_changes['Neg→❤️'],
        name='Negativa → ❤️', marker_color='#1a9850'
    ))

    fig.update_layout(
        title="Mudanças de Reação Entre Dias",
        xaxis_title="", yaxis_title="Quantidade de mudanças",
        barmode='group',
        height=400,
...
print("### Hostilidades Mútuas (Hoje)\n")
...
print("A dá reação negativa para B, mas B dá ❤️ para A:\n")
...
Agrupamento hierárquico baseado em sentimento mútuo. Participantes no mesmo cluster tendem a se dar bem entre si.
...
for c in sorted(cluster_data.keys()):
    ...
    print(f"**Cluster {c}** ({len(members)} membros — {composition}):\n")
    print(f"{member_list}\n")
...
if rivalry_pairs:
    ...
    for (c1, c2), sent in sorted(rivalry_pairs, key=lambda x: x[1]):
        print(f"- Cluster {c1} vs Cluster {c2}: sentimento médio {sent:+.2f} ⚔️")
    print("")

if alliance_pairs:
    print("**Afinidades:**\n")
    for (c1, c2), sent in sorted(alliance_pairs, key=lambda x: -x[1]):
        print(f"- Cluster {c1} + Cluster {c2}: sentimento médio {sent:+.2f} ❤️")
    print("")
...
print(f"**Mais rivalidades mútuas:** {', '.join(f'{name} ({count})' for name, count in top_enemies)}\n")
```
...
fig = go.Figure(go.Heatmap(
    ...
    hovertemplate='%{y} → %{x}<br>Sentimento mútuo: %{z:+.2f}<extra></extra>',
    colorbar=dict(title="Sentimento", tickvals=[-1, -0.5, 0, 0.5, 1],
                  ticktext=['Rivalidade', 'Tensão', 'Neutro', 'Simpatia', 'Aliança'])
))
...
Existe correlação entre ter mais Estalecas e ser bem-visto pelos colegas?
...
if len(df_bs) > 2:
    ...
    fig = px.scatter(
        ...
        title=f"Saldo vs Sentimento (correlação: r = {corr:.2f})",
    ...
    fig.add_trace(go.Scatter(x=x_line, y=y_line, mode='lines',
                             ...
                             name='Tendência', showlegend=True))
