...
if contradictions:
    ...
    if ctx.get("week_1"):
        print(f"<div class='text-muted fs-base' style='margin-top:0.3rem;'>⚠️ Semana 1: {ctx['week_1']}</div>")
...
<div class="text-muted fs-base" style="line-height: 1.6;">
<ul>
...
if not bfc or not bfc.get("attendees"):
    ...
    html += f'<span class="fs-base" style="color:#aaa;">Alvo real: <strong style="color:#eee;">{ACTUAL_TARGET}</strong></span>'
    html += '</div>'
    ...
    html += '<p class="text-muted fs-md" style="margin-bottom:0.5rem;">Somente alvos que <em>todos os 3</em> davam reação negativa. Ordenado pelo score combinado (soma dos 3 pair scores).</p>'
    ...
    html += '<th class="tc text-light" style="padding:6px 8px;">Jonas→alvo?</th>'
    html += '</tr>'
    ...
    if actual:
        ...
        html += '<p class="text-muted fs-md" style="margin-bottom:0.5rem;">Decomposição do pair score de cada atendente para Jonas. Queridômetro é o emoji diário; os demais são eventos acumulados.</p>'
        ...
        html += '<th class="tc text-light" style="padding:6px 8px;">Jonas→</th>'
        html += '</tr>'
        ...
        for att in attendees:
            ...
            for ck in all_comp_keys:
                v = comps.get(ck, 0)
                if v == 0:
                    html += f'<td class="tc" style="padding:6px 8px; color:#555;">—</td>'
                else:
    ...
    html += '<div class="text-muted fs-md" style="margin-top:0.8rem; padding:0.6rem 0.8rem; background:#222; border:1px solid #444; border-radius:6px;">'
...
for par in paredoes_file.get("paredoes", []):
    ...
    if no_score_voters > 0:
        html_retro += f'<span class="fs-base" style="background:#555; color:#eee; padding:4px 12px; border-radius:6px;">Excl. eliminados: {exact}/{scorable_voters} ({exact_scorable_pct:.0f}%)</span>'
    html_retro += '</div>'
...
if not vp_by_paredao:
    ...
    if enh.get("total"):
        ...
        html_vp += f'<div class="tc" style="background:#222; padding:12px 20px; border-radius:8px;"><div class="fs-md" style="color:#aaa;">Modelo Aprimorado</div><div class="text-pos fs-4xl">{enh_pct:.1f}%</div><div class="text-muted table-md">{enh["correct"]}/{enh["total"]}</div></div>'
        html_vp += f'<div class="tc" style="background:#222; padding:12px 20px; border-radius:8px;"><div class="fs-md" style="color:#aaa;">Baseline (sentimento)</div><div class="fs-4xl" style="color:#f39c12; font-weight:bold;">{base_pct:.1f}%</div><div class="text-muted table-md">{base["correct"]}/{base["total"]}</div></div>'
        html_vp += f'<div class="tc" style="background:#222; padding:12px 20px; border-radius:8px;"><div class="fs-md" style="color:#aaa;">Diferença</div><div class="fs-4xl" style="color:{diff_color}; font-weight:bold;">{diff_str}</div><div class="text-muted table-md">aprimorado vs baseline</div></div>'
        html_vp += '</div></div>'
    ...
    for num_str in sorted(vp_by_paredao.keys(), key=int, reverse=True):
        ...
        if lider_pred:
            ...
            html_vp += f'<div class="text-light fs-base" style="margin-bottom:0.5rem;">{lider_mark} Indicação do Líder: previu <strong>{lider_pred["predicted"]}</strong> ({lider_pred["score"]:+.2f}), real: <strong>{lider_actual}</strong></div>'
        ...
        if retro:
            ...
            html_vp += f'<span class="fs-base" style="background:#555; color:#eee; padding:4px 12px; border-radius:6px;">Baseline: {baseline_acc:.0f}%</span>'
            html_vp += '</div>'
        ...
        html_vp += '<h5 class="fs-base" style="color:#aaa; margin:0 0 0.3rem;">Previsão</h5>'
        ...
        for i, (target, votes) in enumerate(sorted_conc[:5]):
            ...
            html_vp += f'<div class="fs-base" style="display:flex; align-items:center; gap:6px; padding:2px 0;"><span style="min-width:120px; color:#eee;"><strong>{target}</strong></span><span class="tc" style="min-width:20px; font-weight:bold;">{votes}</span><div style="background:{bar_c}; height:12px; width:{bar_w}px; border-radius:3px;"></div></div>'
        html_vp += '</div>'
        ...
        if real_votes:
            ...
            html_vp += '<div style="flex:1; min-width:250px;">'
            html_vp += '<h5 class="fs-base" style="color:#aaa; margin:0 0 0.3rem;">Real</h5>'
        ...
        # Detailed vote table
        if real_votes:
            html_vp += '<details style="margin:0.5rem 0;"><summary class="fs-base" style="cursor:pointer; color:#f0c040;">🗳️ Detalhe voto a voto</summary>'
        ...
        if retro and retro.get("errors"):
            errors = retro["errors"]
            html_vp += f'<details style="margin:0.5rem 0;"><summary class="fs-base" style="cursor:pointer; color:#e74c3c;">💡 Análise dos {len(errors)} erros</summary>'
            html_vp += '<div class="text-light fs-base" style="line-height:1.8; margin-top:0.5rem;">'
            for err in errors:
        ...
        if has_adjustments:
            html_vp += '<details style="margin:0.5rem 0;"><summary class="fs-base" style="cursor:pointer; color:#aaa;">🔧 Ajustes do modelo aprimorado</summary>'
