...
if contradictions:
    ...
    if ctx.get("week_1"):
        print(f"<div class='text-muted' style='margin-top:0.3rem;font-size:0.85rem;'>⚠️ Semana 1: {ctx['week_1']}</div>")
...
if voting_blocs:
    for bloc in voting_blocs:
        ...
        print(f"<div style='margin:0.4rem 0;'><strong>Semana {bloc.get('week')}</strong> — "
...
if streak_breaks:
    for b in streak_breaks:
        ...
        print(
            f"<div style='margin:0.4rem 0;'>{sev_icon} <strong>{b['giver']}</strong> → "
...
if not bfc or not bfc.get("attendees"):
    ...
    html += '<h4 style="color:#f0c040; margin:1rem 0 0.5rem;">Alvos viáveis: quem os 3 atacavam no queridômetro</h4>'
    html += '<p style="color:#888; font-size:0.8em; margin-bottom:0.5rem;">Somente alvos que <em>todos os 3</em> davam reação negativa. Ordenado pelo score combinado (soma dos 3 pair scores).</p>'
    ...
    html += '<th style="padding:6px 8px; text-align:center; color:#ccc;">Jonas→alvo?</th>'
    html += '</tr>'
    ...
    if actual:
        html += '<h4 style="color:#f0c040; margin:1.2rem 0 0.5rem;">O que compõe cada score → Jonas</h4>'
        html += '<p style="color:#888; font-size:0.8em; margin-bottom:0.5rem;">Decomposição do pair score de cada atendente para Jonas. Queridômetro é o emoji diário; os demais são eventos acumulados.</p>'
        ...
        html += '<th style="padding:6px 8px; text-align:center; color:#ccc;">Jonas→</th>'
        html += '</tr>'
        ...
        for att in attendees:
            ...
            for ck in all_comp_keys:
                v = comps.get(ck, 0)
                if v == 0:
                    html += f'<td style="padding:6px 8px; text-align:center; color:#555;">—</td>'
                else:
    ...
    html += '<h4 style="color:#f0c040; margin:1.2rem 0 0.5rem;">Por que Jonas e não os outros alvos?</h4>'
    ...
    if rank_score == 1:
        ...
        html += f'<span style="color:#ccc;">O scoring chegou perto (#{rank_score}). Considerar se o peso de power_events está calibrado.</span>'
    else:
        html += '<span style="color:#ccc;">O scoring errou. Revisar pesos dos componentes — especialmente a proporção entre queridômetro e eventos.</span>'
    html += '</div>'
...
for par in paredoes_file.get("paredoes", []):
    ...
    html_retro += f'<h4 style="color:#f0c040; margin:0 0 0.8rem;">{numero}º Paredão ({data_form})</h4>'
...
if not vp_by_paredao:
    ...
    html_vp += '<p style="color:#888;">Arquivo <code>vote_prediction.json</code> não encontrado. Execute <code>python scripts/build_derived_data.py</code>.</p>'
    html_vp += '</div>'
    print(html_vp)
    ...
    if enh.get("total"):
        ...
        html_vp += '<h4 style="color:#2ecc71; margin:0 0 0.8rem;">📊 Acurácia acumulada (todos os paredões com votos)</h4>'
        html_vp += '<div style="display:flex; gap:16px; flex-wrap:wrap;">'
        html_vp += f'<div style="background:#222; padding:12px 20px; border-radius:8px; text-align:center;"><div style="color:#aaa; font-size:0.8em;">Modelo Aprimorado</div><div style="color:#2ecc71; font-size:1.8em; font-weight:bold;">{enh_pct:.1f}%</div><div style="color:#888; font-size:0.85em;">{enh["correct"]}/{enh["total"]}</div></div>'
        html_vp += f'<div style="background:#222; padding:12px 20px; border-radius:8px; text-align:center;"><div style="color:#aaa; font-size:0.8em;">Baseline (sentimento)</div><div style="color:#f39c12; font-size:1.8em; font-weight:bold;">{base_pct:.1f}%</div><div style="color:#888; font-size:0.85em;">{base["correct"]}/{base["total"]}</div></div>'
        html_vp += f'<div style="background:#222; padding:12px 20px; border-radius:8px; text-align:center;"><div style="color:#aaa; font-size:0.8em;">Diferença</div><div style="color:{diff_color}; font-size:1.8em; font-weight:bold;">{diff_str}</div><div style="color:#888; font-size:0.85em;">aprimorado vs baseline</div></div>'
        html_vp += '</div></div>'
    ...
    for num_str in sorted(vp_by_paredao.keys(), key=int, reverse=True):
        ...
        html_vp += f'<h4 style="color:#f0c040; margin:0 0 0.8rem;">{status_icon} {num_str}º Paredão — {formation_date} ({status_label})</h4>'

        # Game state
        html_vp += '<div style="display:flex; gap:12px; flex-wrap:wrap; font-size:0.85em; margin-bottom:0.8rem;">'
        ...
        if lider_pred:
            ...
            html_vp += f'<div style="font-size:0.9em; margin-bottom:0.5rem; color:#ccc;">{lider_mark} Indicação do Líder: previu <strong>{lider_pred["predicted"]}</strong> ({lider_pred["score"]:+.2f}), real: <strong>{lider_actual}</strong></div>'
        ...
        if retro:
            ...
            html_vp += f'<span style="background:{hc_color}; color:#fff; padding:4px 12px; border-radius:6px; font-weight:bold; font-size:0.85em;">Alta confiança: {hc["correct"]}/{hc["total"]} ({hc["pct"]:.0f}%)</span>'
            html_vp += f'<span style="background:{top2_color}; color:#fff; padding:4px 12px; border-radius:6px; font-weight:bold; font-size:0.85em;">Top-2: {"✅" if top2 else "❌"}</span>'
        ...
        html_vp += '<h5 style="color:#aaa; margin:0 0 0.3rem; font-size:0.85em;">Previsão</h5>'
        ...
        for i, (target, votes) in enumerate(sorted_conc[:5]):
            ...
            html_vp += f'<div style="display:flex; align-items:center; gap:6px; font-size:0.85em; padding:2px 0;"><span style="min-width:120px; color:#eee;"><strong>{target}</strong></span><span style="min-width:20px; text-align:center; font-weight:bold;">{votes}</span><div style="background:{bar_c}; height:12px; width:{bar_w}px; border-radius:3px;"></div></div>'
        html_vp += '</div>'
        ...
        # Detailed vote table
        if real_votes:
            html_vp += '<details style="margin:0.5rem 0;"><summary style="cursor:pointer; color:#f0c040; font-size:0.9em;">🗳️ Detalhe voto a voto</summary>'
            html_vp += '<div style="overflow-x:auto; margin-top:0.5rem;">'
        ...
        if retro and retro.get("errors"):
            errors = retro["errors"]
            html_vp += f'<details style="margin:0.5rem 0;"><summary style="cursor:pointer; color:#e74c3c; font-size:0.9em;">💡 Análise dos {len(errors)} erros</summary>'
            html_vp += '<div style="color:#ccc; font-size:0.9em; line-height:1.8; margin-top:0.5rem;">'
            for err in errors:
                icon = "🎲" if err["analysis"] == "gap mínimo (coin flip)" else "🤝"
                html_vp += f'<p style="margin:0.3rem 0;">{icon} <strong>{err["voter"]}</strong>: previu <span style="color:#e74c3c;">{err["predicted"]}</span> → votou <strong style="color:#2ecc71;">{err["actual"]}</strong> ({err["confidence"]}) — {err["analysis"]}</p>'
            html_vp += '</div></details>'
        ...
        if has_adjustments:
            html_vp += '<details style="margin:0.5rem 0;"><summary style="cursor:pointer; color:#aaa; font-size:0.9em;">🔧 Ajustes do modelo aprimorado</summary>'
            html_vp += '<div style="font-size:0.85em; margin-top:0.5rem; color:#ccc;">'
            for voter in sorted(predictions.keys()):
                pred = predictions[voter]
                for t in pred.get("top3", []):
                    expl = t.get("explanation")
                    if expl:
                        html_vp += f'<div style="padding:2px 0;">{voter} → {t["target"]}: {expl}</div>'
            html_vp += '</div></details>'
    ...
    html_vp += '<div style="background:#111; border:1px solid #333; border-radius:8px; padding:0.8rem 1rem; margin:1rem 0; font-size:0.85em; color:#aaa;">'
    html_vp += '<strong style="color:#ccc;">Metodologia — Modelo Aprimorado (v2)</strong><br>'
...
lider_list = _rd_latest.get("roles", {}).get("Líder", [])
...
if not lider_name:
    ...
    # Eligible targets: active participants minus the Líder
    ...
    # Check if there's an active paredão with immunization info
    ...
    # ── Load edges for detail rows ───────────────────────────────────────
    _all_edges = relations.get("edges", [])

    # ── Build queridômetro history per pair (Líder ↔ each target) ─────
    _slug_to_label = {
        "coracao": "Coração", "planta": "Planta", "cobra": "Cobra",
        "mala": "Mala", "biscoito": "Biscoito", "coracao-partido": "Coração partido",
        "alvo": "Alvo", "vomito": "Vômito", "mentiroso": "Mentiroso",
    }
    _emoji_map = {
    ...
    for _date, _fp in sorted(_by_date.items()):
        ...
        _mat = build_reaction_matrix(_parts)
    ...
    comp_meta = {
        "queridometro": ("🐍", "#e74c3c", "#2ecc71"),  # emoji, color_neg, color_pos
    ...
    html_pred += f'🎯 <strong style="color:#e74c3c;">Previsão de Indicação d{art} Líder</strong></div>'
    ...
    html_pred += f'<span style="color:#888;">— Líder da Semana {DISPLAY_WEEK_DAILY}</span></span></div>'
    ...
    for i, (tname, tentry) in enumerate(top3):
        ...
        html_pred += f'<div style="color:#888; font-size:0.75em;">#{i+1} mais provável</div>'
    ...
    # Anjo note
    if anjo_name:
        html_pred += f'<div style="color:#3498db; font-size:0.9em;">😇 Anjo: <strong>{anjo_name}</strong>'
        if imunizado_nome:
            html_pred += f' — imunizou <strong>{imunizado_nome}</strong> (bloqueado como alvo)'
        html_pred += '</div>'
    else:
    ...
    for hdr in ["#", "Participante", "Score", "Barra", "Componentes", "Reciprocidade", "Streak", "VIP"]:
        html_pred += f'<th style="padding:0.5rem 0.4rem; text-align:left; color:#aaa; white-space:nowrap;">{hdr}</th>'
    ...
    for rank, (tname, tentry) in enumerate(ranked, 1):
        ...
        html_pred += f'<tr style="border-bottom:1px solid #333; {row_bg}">'

        # Rank
        html_pred += f'<td style="padding:0.4rem; color:#888; text-align:center;">{rank}</td>'
        ...
        # Score
        html_pred += f'<td style="padding:0.4rem; color:{sc_color}; font-weight:bold; font-family:monospace;">{sc:+.2f}</td>'
        ...
        html_pred += " ".join(chips) if chips else '<span style="color:#555;">—</span>'
        html_pred += '</td>'

        # Reciprocity: target → Líder score
        ...
        if sc < 0 and recip_score < 0:
            ...
            recip_label = "🔍 Alvo cego"
            recip_color = "#e67e22"
        elif sc >= 0 and recip_score < 0:
            recip_label = "⚠️ Risco oculto"
            recip_color = "#e67e22"
        else:
            recip_label = "💚 Aliados"
            recip_color = "#2ecc71"
        ...
        html_pred += f'<td style="padding:0.4rem; white-space:nowrap;">'
        html_pred += f'<span style="color:{recip_color}; font-size:0.85em;">{recip_label}</span> '
        html_pred += f'<span style="color:#888; font-size:0.78em;">({recip_score:+.2f})</span></td>'

        # Streak
        streak_icon = "🔴" if has_break else ("🟢" if streak_len >= 5 else "⚪")
        break_tag = ' <span style="color:#e74c3c; font-size:0.75em;">BREAK</span>' if has_break else ""
        html_pred += f'<td style="padding:0.4rem; text-align:center; white-space:nowrap;">'
        ...
        if pair_edges or hist_fwd:
            ...
            html_pred += f'padding:0.2rem 0;">📋 {len(pair_edges)} evento(s) · '
            html_pred += f'{len(hist_fwd)} dia(s) de queridômetro</summary>'
            ...
            if pair_edges:
                ...
                for edge in pair_edges:
                    ...
                    e_emoji, e_label = _edge_display.get(e_type, ("❓", e_type))
                    e_wcolor = "#e74c3c" if e_weight < 0 else "#2ecc71"
                    is_backlash = edge.get("backlash", False)

                    # Direction arrow
                    if e_actor == lider_name:
                        arrow = f'{lider_name.split()[0]} → {tname.split()[0]}'
                    else:
                        arrow = f'{tname.split()[0]} → {lider_name.split()[0]}'
                    ...
                    html_pred += f'<td style="padding:2px 4px; color:#888; white-space:nowrap;">{e_date}</td>'
                    ...
                    html_pred += f'<td style="padding:2px 4px; color:{e_wcolor}; text-align:right; font-family:monospace;">{e_weight:+.2f}</td>'
                    html_pred += f'<td style="padding:2px 4px; color:#888;">{detail_str}</td>'
                    html_pred += '</tr>'
            ...
            if hist_fwd:
                ...
                html_pred += f'<strong>Queridômetro</strong> (últimos {len(show_dates)} dias)</div>'
                html_pred += '<table style="width:100%; border-collapse:collapse; font-size:0.78em;">'
                html_pred += '<thead><tr style="border-bottom:1px solid #333;">'
                html_pred += f'<th style="padding:2px 4px; color:#666; text-align:left;">Data</th>'
                html_pred += f'<th style="padding:2px 4px; color:#666; text-align:center;">{lider_name.split()[0]}→</th>'
                html_pred += f'<th style="padding:2px 4px; color:#666; text-align:center;">→{lider_name.split()[0]}</th>'
                html_pred += '</tr></thead><tbody>'
                for dt in show_dates:
                    ...
                    html_pred += f'<tr style="border-bottom:1px solid #222;">'
                    html_pred += f'<td style="padding:1px 4px; color:#888; font-size:0.9em;">{dt[5:]}</td>'
                    html_pred += f'<td style="padding:1px 4px; text-align:center; color:{fwd_c};">{fwd_e}</td>'
                    html_pred += f'<td style="padding:1px 4px; text-align:center; color:{rev_c};">{rev_e}</td>'
                    html_pred += '</tr>'
                html_pred += '</tbody></table></div>'
