...
<p class="text-muted">Tabela diária — queridômetro base usa rolling de 3 dias. Eventos acumulam sem decay (peso integral).</p>
...
def compute_badges_for_snapshot(fp, date_str, vote_week, paredao_data):
    ...
    # roles current
    roles_current = {r: [] for r in ["Líder", "Anjo", "Monstro", "Imune", "Paredão"]}
    for p in active:
        name = p["name"]
    ...
    # Indicações públicas (alvos já no paredão)
    ineligible = set()
    ...
    for p in active:
        ...
        for ev in target_events_all:
            ...
            if ev_type in ["lider", "anjo", "monstro", "imunidade"]:
                role_label = next((k for k, v in {"Líder": "lider", "Anjo": "anjo", "Monstro": "monstro", "Imune": "imunidade"}.items()
                                   if v == ev_type), None)
                if role_label and name in roles_current.get(role_label, []):
                    current_events.append(ev)
            elif ev_week == vote_week:
                current_events.append(ev)

        neg_events = [ev for ev in current_events if ev.get("impacto") == "negativo"]
        neg_public = [ev for ev in neg_events if ev.get("visibility", "public") != "secret"]

        votes_count = sum(votes_received_by_week.get(vote_week, {}).get(name, {}).values())
        indic_count = indicacoes.get(name, 0)
        external_score = (
            1.0 * votes_count
            + sum(NEG_EVENT_WEIGHTS.get(ev.get("type"), 1.0) for ev in neg_public)
        )
        if "Paredão" in roles:
            external_score += 2
        ...
        rows.append({
            ...
            "Indicações": indic_count,
            "Elegível voto": "sim" if name not in ineligible else "não",
            "Vulnerabilidade": vuln,
            "Risco externo": external_level,
            "External score": round(external_score, 2),
            "Falsos amigos": ff,
        })
...
for par in paredoes_data.get("paredoes", []) if paredoes_data else []:
    ...
    df.insert(0, "Paredão", f"{par.get('numero')}º ({data_form})")
    rows_all.append(df)
