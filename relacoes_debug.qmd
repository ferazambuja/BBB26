...
if not vp_by_paredao:
    ...
    html_vp += '<strong style="color:#ccc;">Metodologia — Modelo Aprimorado (v1)</strong><br>'
    html_vp += '<ul style="margin:0.3rem 0 0;">'
    html_vp += '<li><strong>Pass 1 (base):</strong> para cada votante, o alvo previsto é o participante elegível com o menor score <code>pairs_daily</code> combinado (queridômetro + eventos acumulados).</li>'
    ...
    html_vp += '<li><strong>Fonte dos scores:</strong> usa <code>pairs_daily</code> para participantes ativos e <code>pairs_all</code> para eliminados/desistentes.</li>'
    html_vp += '<li><strong>Nota:</strong> para paredões antigos, a acurácia é limitada pois os scores atuais refletem a evolução do jogo (não os sentimentos na época).</li>'
    html_vp += '</ul></div>'
