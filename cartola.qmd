...
# Load paredões data (formation, resultados, bate-volta)
...
# data/paredoes.json covers: salvo / não eliminado / não emparedado.
# ══════════════════════════════════════════════════════════════════════════════
...
for snap in daily_snapshots:
    ...
    # Regra: quando o Anjo é autoimune, acumula com "Imunizado"
    ...
    # Regra: Líder não acumula com outros itens → não pontua como imunizado
    ...
    # Regra: Líder não acumula com VIP
...
# ─────────────────────────────────────────────────────────────────────────────
# 2b. PAREDÃO-DERIVED EVENTS (salvo / não eliminado / não emparedado)
...
for p in paredoes_data.get('paredoes', []):
    ...
    # Salvo do paredão: venceu o Bate e Volta (escapou)
    vencedor_bv = None
    ...
    if isinstance(formacao, dict):
        ...
        if isinstance(bv, dict):
            ...
            if vencedor_bv:
                # Regra: Imunizado por dinâmica não acumula com "Salvo do Paredão"
    ...
    # Não eliminado no paredão: indicado que permaneceu
    ...
    # Elegíveis para votação da casa (disponíveis para receber votos)
    snap = get_snapshot_on_or_before(paredao_date)
    if snap:
        ativos = {pp['name'].strip() for pp in snap['participants']}

        # Excluir quem não é elegível pela formação oficial
        ...
        # Imunidades adicionais vindas de dinâmicas (power_events)
        extra_imunes = set()
        ...
        # Não emparedado: elegíveis que NÃO foram para o paredão final
        ...
        # Não recebeu votos da casa: elegíveis com zero votos
...
# ─────────────────────────────────────────────────────────────────────────────
# 2c. RULE NORMALIZATION (no accumulation with Líder)
# Keep only Líder points for the week
# ─────────────────────────────────────────────────────────────────────────────
