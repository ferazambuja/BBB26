...
for event, points in CARTOLA_POINTS.items():
    if points > 0:
        ...
        auto = ' <span class="text-muted" style="font-size: 0.7rem;">(API)</span>' if event in ['lider', 'anjo', 'imunizado', 'vip'] else ''
...
for event, points in CARTOLA_POINTS.items():
    if points < 0:
        ...
        label = POINTS_LABELS.get(event, event)
        # Mark auto-detected events
        auto = ' <span class="text-muted" style="font-size: 0.7rem;">(API)</span>' if event in ['monstro', 'emparedado'] else ''
