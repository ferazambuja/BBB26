...
fig = go.Figure(go.Bar(
    ...
    marker_color='#1a9850',
    text=[a['Consistência'] for a in alliance_data], textposition='outside',
...
if streak_breaks:
    print(f"**{len(streak_breaks)} rupturas** de aliança detectadas na temporada.\n")
...
if n_daily >= 2:
    ...
    if has_changes:
        ...
        if new_two:
            ...
            hover_texts.append('<br>'.join(f"{sorted(p)[0]} ↔ {sorted(p)[1]}" for p in new_two))
        if resolved_two:
            categories.append('✅ Resolvidas\nMútuas')
            values.append(len(resolved_two))
            colors.append('#1a9850')
            hover_texts.append('<br>'.join(f"{sorted(p)[0]} ↔ {sorted(p)[1]}" for p in resolved_two))
        if new_one:
            categories.append('😠 Novas\nUnilaterais')
            values.append(len(new_one))
            colors.append('#fc8d59')
            hover_texts.append('<br>'.join(f"{h} → {f}" for h, f in sorted(new_one))[:500])
        if resolved_one:
            categories.append('💚 Resolvidas\nUnilaterais')
            values.append(len(resolved_one))
            colors.append('#91cf60')
            hover_texts.append('<br>'.join(f"{h} → {f}" for h, f in sorted(resolved_one))[:500])
        ...
        fig.update_layout(
            title=f"Mudanças em Hostilidades: {prev_snap['date']} → {curr_snap['date']}",
        ...
        if new_two:
            ...
            for pair in sorted(new_two, key=lambda x: tuple(sorted(x))):
                ...
                print(f"- **{a}** {ea}↔{eb} **{b}**")
        if new_one:
            print("\n### 😠 Novas Hostilidades Unilaterais\n")
...
<strong>Saldo:</strong> Mútuas <span style="color: {m_color};">{net_mutual:+d}</span> | Unilaterais <span style="color: {o_color};">{net_one:+d}</span>
</div>""")
    else:
