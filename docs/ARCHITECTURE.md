...
| `index_data.json` | Precomputed tables for `index.qmd`. Sincerao: top-level (`current_week`, `available_weeks`, `radar`, `pairs`), per-profile (`summary` with received/given totals + contradiction count, `current` received/given, `season` received_by_week/given_by_week) |
