---
title: "BBB 26 — Evolução"
subtitle: "Como o jogo está evoluindo: rankings, sentimento, impacto e pulso diário"
lang: pt-BR
format:
  html:
    code-tools: false
...
from data_utils import (
    ...
    load_index_data, load_manual_events, load_auto_events, load_game_timeline,
    avatar_img,
...
# ── Load snapshots for diff heatmap + balance charts ──
...
📅 Comparando: <strong>{daily_snapshots[-2]['label']}</strong> → <strong>{daily_snapshots[-1]['label']}</strong>
| 📈 Analisando <strong>{n_daily} dias</strong> de dados
</div>
""")
```
...
Linha do tempo completa dos eventos da temporada — eventos mais recentes primeiro.
...
if streak_breaks:
    ...
    if _active_breaks:
        print("\n### 💔 Rupturas de Aliança Recentes\n")
        for sb in _active_breaks[:5]:
            emoji = REACTION_EMOJI.get(sb.get("new_emoji", ""), "❓")
            severity_icon = "🔴" if sb.get("severity") == "strong" else "🟡"
            print(f"- {severity_icon} **{sb['giver']}** → **{sb['receiver']}**: "
                  f"{sb['previous_streak']} dias de ❤️ rompidos → {emoji} ({sb['date']})")
```

# Pulso Diário {#pulso}

📅 **Comparação dia-a-dia** — O que mudou de ontem para hoje.
...
if _latest_dc and _latest_dc.get("pair_changes"):
    ...
    if receiver_data:
        fig = make_horizontal_bar(
            ...
            title="Impacto Líquido: Quem Ganhou/Perdeu Pontos de Sentimento",
...
if _giver_vol:
    ...
    if giver_data:
        ...
        fig = make_stacked_bar(
            _vol_names,
            [
                dict(values=[d['melhora'] for d in giver_data], name='Melhorou opinião', color='#1a9850'),
                dict(values=[d['piora'] for d in giver_data], name='Piorou opinião', color='#d73027'),
                dict(values=[d['lateral'] for d in giver_data], name='Mudança lateral', color='#888'),
            ],
            title="Volatilidade: Quem Mais Mudou de Opinião?",
        ...
        vol_str = ', '.join(f"{d['name']} ({d['total']} mudanças)" for d in top_volatile)
        print(f"**Mais voláteis:** {vol_str}")
```
...
# Convert "A→B" keys to (A, B) tuples
...
if transition_counts:
    ...
    reaction_colors = {
        'Coração': '#1a9850', 'Planta': '#91cf60', 'Mala': '#d9ef8b',
        'Biscoito': '#fee08b', 'Cobra': '#fc8d59', 'Alvo': '#f46d43',
        'Vômito': '#d73027', 'Mentiroso': '#a50026', 'Coração partido': '#67001f'
    }
    ...
    if values:
        ...
        fig.update_layout(title=f"Fluxo de Reações: {prev_date} → {curr_date}", font_size=10, height=500)
        fig.show()
...
<style>
.diff-table {{ border-collapse: separate; border-spacing: 0; font-size: var(--fs-md); width: max-content; }}
.diff-table th, .diff-table td {{ padding: 3px 5px; text-align: center; border: 1px solid #555; min-width: 40px; white-space: nowrap; }}
.diff-table thead th {{ position: sticky; top: 0; background: #222; z-index: 2; font-weight: bold; font-size: var(--fs-sm); }}
.diff-table tbody th {{ position: sticky; left: 0; background: #222; z-index: 1; font-weight: bold; text-align: right; font-size: var(--fs-sm); }}
.diff-table thead th:first-child {{ z-index: 3; left: 0; }}
.diff-table td {{ font-size: var(--fs-base); }}
.diff-table td:hover {{ outline: 2px solid #fff; }}
</style>
<table class="diff-table"><thead><tr><th class="u-s001">↓ / →</th>''']
    ...
    for i, giver in enumerate(active_names_sorted):
        ...
        for j, receiver in enumerate(active_names_sorted):
            if giver == receiver:
                html.append('<td class="u-s116">—</td>')
            else:
                key = (giver, receiver)
                if key in change_lookup:
                    c = change_lookup[key]
                    style = get_cell_style(c['Delta'])
                    star = "⭐" if abs(c['Delta']) >= 1.5 else ""
                    tooltip = f"{giver} → {receiver}: {c['Antes']}→{c['Depois']} (Δ{c['Delta']:+.1f})"
                    html.append(f'<td style="{style}" title="{tooltip}">{c["Antes"]}→{c["Depois"]}{star}</td>')
                else:
                    html.append('<td class="u-s114">·</td>')
        html.append('</tr>')
...
# Evolução do Sentimento {#sentimento}

📈 **Dado acumulado** — Como o sentimento de cada participante evoluiu ao longo da temporada.
...
# ── Helper: visibility selector buttons (wraps data_utils.make_visibility_buttons with MEMBER_OF) ──
...
if not df_timeline.empty:
    ...
    fig.update_layout(
        ...
        xaxis_title="Data", yaxis_title="Posição no Ranking",
...
if not df_timeline.empty:
    ...
    for p in paredoes_data.get('paredoes', []):
        ...
        fig.add_annotation(x=paredao_date, y=y_range[1],
                           text=f"🗳️ {p['numero']}º Paredão", showarrow=False,
...
if strategic_timeline:
    ...
    if not df_strat.empty and 'date' in df_strat.columns and 'name' in df_strat.columns and 'score' in df_strat.columns:
        ...
        fig.update_layout(
            title="Score Estratégico (Queridômetro + Eventos)",
        ...
        print("*Dados estratégicos indisponíveis.*")
else:
    print("*Dados estratégicos indisponíveis.*")
```
...
if len(daily_snapshots) >= 3:
    ...
    if leader_first_rank != 1 and leader_first_rank != "?":
        sections.append(f"**🏆 {current_leader}** subiu da **{leader_first_rank}ª posição** para o topo. "
                        f"Com **{leader_score:+.1f}** pontos, consolidou-se como o mais querido.")
    else:
        sections.append(f"**🏆 {current_leader}** lidera com **{leader_score:+.1f}** pontos.")

    if biggest_rise and biggest_rise[1] >= 3:
        name, change, rank = biggest_rise
        sections.append(f"**📈 Maior Ascensão:** **{name}** ({MEMBER_OF.get(name, '?')}) subiu **{change} posições** → {rank}º lugar.")
    if biggest_fall and biggest_fall[1] <= -3:
        name, change, rank = biggest_fall
        sections.append(f"**📉 Maior Queda:** **{name}** ({MEMBER_OF.get(name, '?')}) caiu **{abs(change)} posições** → {rank}º lugar.")
    if most_stable and most_stable[1] <= 2:
        name, change, rank, first_rank = most_stable
        if change == 0:
            sections.append(f"**⚓ Mais Estável:** **{name}** manteve a mesma posição ({rank}º) desde o início.")
        else:
            sections.append(f"**⚓ Mais Estável:** **{name}** variou apenas {change} posição (de {first_rank}º para {rank}º).")
...
📈 **Dado acumulado** — Impacto de eventos de poder, votos e Sincerão sobre cada participante.
...
if received_impact:
    ...
    if impact_data:
        ...
        fig = make_stacked_bar(
            ...
            title="Impacto Acumulado por Participante (Eventos + Votos + Sincerão)",
            xaxis_title="Impacto", barmode='relative',
        )
        fig.show()
else:
    print("*Dados de impacto indisponíveis.*")
```
...
if impact_history:
    ...
    fig = make_line_evolution(
        _impact_series,
        title="Trajetória de Impacto Negativo Acumulado",
    ...
    print("*Dados de trajetória de impacto indisponíveis.*")
```
...
if vulnerability_history:
    ...
    fig = make_line_evolution(
        ...
        yaxis_title="Nº de falsos amigos (dá ❤️ a quem detesta)",
        highlight=highlight, colors=participant_colors, member_of=MEMBER_OF,
        hover_format="%{y} falsos amigos", height=500,
    )
    fig.show()
    print("Quem está cada vez mais exposto a votos surpresa?")
else:
    print("*Dados de vulnerabilidade indisponíveis.*")
```

:::

# Volatilidade Histórica {#volatilidade}

📅 **Comparação dia-a-dia** — Como a volatilidade do jogo evoluiu ao longo do tempo.
...
if daily_changes:
    ...
    fig.add_trace(go.Scatter(
        ...
        name='Total de mudanças', line=dict(color='#3498db', width=2),
    ))
    fig.add_trace(go.Bar(
        x=dates, y=hearts_gained, name='❤️ ganhos', marker_color='#2ecc71', opacity=0.6,
    ))
    fig.add_trace(go.Bar(
        x=dates, y=[-h for h in hearts_lost], name='❤️ perdidos', marker_color='#e74c3c', opacity=0.6,
    ))

    # Annotate paredão dates
    for p in paredoes_data.get('paredoes', []):
        ...
        fig.add_annotation(x=pdate, y=max(total_changes) if total_changes else 50,
                           text=f"🗳️ {p['numero']}º", showarrow=False,
                           font=dict(size=9, color='#FF6B6B'), yshift=15)

    fig.update_layout(
        title="Volatilidade Diária: Mudanças de Reação ao Longo do Tempo",
    ...
    print("*Dados históricos de mudanças indisponíveis.*")
```
...
.saldo-table .saldo-val { text-align: right; font-weight: 700; font-variant-numeric: tabular-nums; font-size: var(--fs-lg); }
</style>
<table class="saldo-table">
<thead><tr>
<th class="u-s475">#</th>
<th>Participante</th>
<th class="u-s478">Estalecas</th>
</tr></thead>
<tbody>
""")
...
<td class="saldo-name u-s339">{avatar} <span>{name}</span></td>
...
📅 **Comparação por líder** — Quem cada Líder escolheu para o VIP e quantas vezes cada participante foi selecionado.

::: {.panel-tabset}

## 📊 Composição por Líder

```{python}
#| label: vip-heatmap
#| fig-cap: "VIP/Xepa por período de liderança"
...
if not leader_periods:
    ...
    col_labels = [
        f"👑 {' + '.join(n.split()[0] for n in lp.get('leaders', [lp['leader']]))}<br><sub>{lp['start'][5:]}</sub>"
        if lp.get('leader')
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        showscale=False,
        hovertemplate="%{y} — %{text} (Líder: %{x})<extra></extra>",
    ))

    fig.update_layout(
        title="Composição VIP/Xepa por Líder",
...
if not leader_periods:
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color='#f1c40f',
        text=[f"{v}×" if v > 0 else "" for v in vip_vals],
        textposition='outside',
        hovertemplate="%{y}: VIP %{x}×<extra></extra>",
    ))
    fig.add_trace(go.Bar(
        y=sorted_names, x=xepa_vals, orientation='h', name='Xepa',
        marker_color='#7f8c8d',
        text=[f"{abs(v)}×" if v < 0 else "" for v in xepa_vals],
        textposition='outside',
        hovertemplate="%{y}: Xepa %{customdata}×<extra></extra>",
        customdata=[xepa_count.get(n, 0) for n in sorted_names],
    ))

    n_periods = len(leader_periods)
    fig.update_layout(
        title=f"VIP vs Xepa ({n_periods} períodos de liderança)",
        barmode='overlay',
        height=max(400, len(sorted_names) * 28),
...
if not leader_periods:
    ...
    for i, lp in enumerate(leader_periods):
        ...
        print(f"""
<div class="u-s132">
<h4 class="u-s436">{leader_av}👑 {leader} <span class="fs-base u-s009">({start} → {end})</span></h4>
...
📈 **Dado acumulado** — Eventos de dinâmicas da casa: poderes, castigos e consequências.
...
if not power_events:
    ...
    tipo_labels = {
        ...
        'imunidade': 'Imunidade', 'indicacao': 'Indicação',
    ...
    def resolve_context(row):
        ...
        if 'prova do líder' in actor: return 'Prova do Líder'
        ...
        if 'dinâmica da casa' in actor: return 'Dinâmica da casa'
        return '—'
    ...
    _DYNAMIC_ICON_MAP = {
        'big fone': '📞', 'caixas-surpresa': '🎁', 'prova do líder': '👑',
        'prova do anjo': '😇', 'prova bate e volta': '🛟', 'ganha-ganha': '🎁',
        'barrado no baile': '🚫',
    }
    ...
    def render_person_cell(value, border):
        ...
        if items:
            ...
            return f'<div class="u-s047">{"".join(avatar_htmls)}<span>{" + ".join(items)}</span></div>'
        label = value or '—'
        if label in AVATARS and AVATARS[label]:
            return (f'<div class="u-s302">'
        ...
        icon = next((v for k, v in _DYNAMIC_ICON_MAP.items() if k in label.lower()), '🎬')
        return f'<div class="u-s047"><span>{icon}</span>{label}</div>'

    def render_context_cell(value):
        if not value or value == '—': return '—'
        icon = next((v for k, v in _DYNAMIC_ICON_MAP.items() if k in value.lower()), '🎬')
        return f'<div class="u-s310"><span>{icon}</span>{value}</div>'

    print('<div class="u-s454">')
    print('<table class="fs-base u-s077">')
    print('<thead><tr class="u-s474">'
          '<th class="u-s016">Data</th><th class="u-s016">Dinâmica</th>'
          '<th class="u-s016">Tipo</th><th class="u-s016">Quem deu</th>'
          '<th class="u-s016">Quem recebeu</th><th class="u-s016">Consequência</th>'
          '<th class="u-s016">Impacto</th></tr></thead><tbody>')

    for _, row in df_show.iterrows():
        impacto = row.get('impacto', 'neutro')
        if impacto == 'positivo':
            border, bg, label = '#2ecc71', 'rgba(46, 204, 113, 0.12)', 'Benefício'
        elif impacto == 'negativo':
            border, bg, label = '#e74c3c', 'rgba(231, 76, 60, 0.12)', 'Prejuízo'
        else:
        ...
        badge = (f'<span class="fs-sm" style="display:inline-block; padding: 0.15rem 0.5rem; border-radius: 999px; '
                 f'background: {bg}; color: {border}; border: 1px solid {border};">{label}</span>')
        print(f'<tr style="border-left: 4px solid {border}; background: {bg};">'
              f'<td class="u-s014">{row["date"]}</td>'
              f'<td class="u-s014">{context_html}</td>'
              f'<td class="u-s014">{row["Tipo"]}</td>'
              f'<td class="u-s014">{actor_html}</td>'
              f'<td class="u-s014">{target_html}</td>'
              f'<td class="u-s014">{row["detail"]}</td>'
              f'<td class="u-s014">{badge}</td></tr>')
...
if not ps_by_part:
    ...
    fig = make_stacked_bar(
        list(impact.index),
        [
            dict(values=[-v for v in impact['negativo']], name='Prejuízo', color='#e74c3c'),
            dict(values=list(impact['positivo']), name='Benefício', color='#2ecc71'),
        ],
...
<a href="relacoes.html"><strong>🤝 Relações</strong></a> — Mapa social |
