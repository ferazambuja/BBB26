...
from data_utils import (
    load_snapshot, get_all_snapshots, parse_roles,
    ...
    POWER_EVENT_EMOJI, POWER_EVENT_LABELS, render_cronologia_html,
)
...
if streak_breaks:
    ...
    headline_parts.append(
        f"💔 **{sb['giver']}** rompeu **{sb['previous_streak']} dias** de aliança com **{sb['receiver']}** → {emoji}"
    )
...
if daily_changes:
    ...
    top_l = dc["top_loser"]
    headline_parts.append(
        f"📊 **{n_ch} mudanças** ({pct:.0f}% das relações) — "
        f"🟢 {dc['n_melhora']} melhorias | 🔴 {dc['n_piora']} pioras"
    )
    stats = []
    if top_r["name"]:
    ...
    if dc["hearts_lost"]:
        stats.append(f"❤️ perdidos: {dc['hearts_lost']}")
    if stats:
...
if _giver_vol:
    ...
    if giver_data:
        ...
        fig.add_trace(go.Bar(
            ...
            name='Melhorou opinião', orientation='h', marker_color='#1a9850',
        ))
        fig.add_trace(go.Bar(
            y=[d['name'] for d in giver_data], x=[d['piora'] for d in giver_data],
            name='Piorou opinião', orientation='h', marker_color='#d73027',
        ))
        fig.add_trace(go.Bar(
            y=[d['name'] for d in giver_data], x=[d['lateral'] for d in giver_data],
            name='Mudança lateral', orientation='h', marker_color='#888',
        ))
        fig.update_layout(
            title=dict(text="Volatilidade: Quem Mais Mudou de Opinião?", y=0.98, x=0.5, xanchor='center'),
            xaxis_title="Número de opiniões alteradas", barmode='stack',
...
# ── Helper: visibility selector buttons with individual dropdown ──
def make_visibility_buttons(trace_names, highlight_set):
    """Build updatemenus with group buttons + individual participant dropdown."""
    ...
    def vis(active_set):
        return [True if n in active_set else 'legendonly' for n in trace_names]
    ...
    # Individual participant dropdown (sorted alphabetically by first name)
    ...
    return [
        ...
        # Row 2: Individual participant dropdown
        dict(
...
if impact_history:
    ...
    fig.update_layout(
        ...
        updatemenus=make_visibility_buttons(names, highlight),
    )
    fig.show()
else:
...
if vulnerability_history:
    ...
    fig.update_layout(
        ...
        xaxis_title="Data", yaxis_title="Nº de falsos amigos (dá ❤️ a quem detesta)",
        height=500, hovermode='x unified',
        legend=dict(font=dict(size=10)),
        updatemenus=make_visibility_buttons(names, highlight),
    )
...
for i, p in enumerate(active_saldo, 1):
    ...
    avatar = (
        f'<img src="{avatar_url}" alt="{name}" title="{name}" '
        f'style="width:36px;height:36px;border-radius:50%;object-fit:cover;'
        f'border:2px solid {cor};flex-shrink:0;">'
    ) if avatar_url else ""
...
if not leader_periods:
    ...
    for i, lp in enumerate(leader_periods):
        ...
        avatar_html = f'<img src="{avatar}" width="32" style="border-radius:50%;vertical-align:middle;margin-right:6px">' if avatar else ""

        print(f"""
<div style="background:#1a1a2e; border:1px solid #f1c40f33; border-radius:8px; padding:12px; margin-bottom:12px">
<h4 style="margin:0 0 8px">{avatar_html}👑 {leader} <span style="color:#888;font-size:0.85em">({start} → {end})</span></h4>
...
if not power_events:
    ...
    def render_person_cell(value, border):
        ...
        icon_map = {'big fone': '📞', 'caixas-surpresa': '🎁', 'prova do líder': '👑',
                    'prova do anjo': '😇', 'prova bate e volta': '🛟', 'ganha-ganha': '🎁', 'barrado no baile': '🚫'}
        icon = next((icon_map[k] for k in icon_map if k in label.lower()), '🎬')
    ...
    def render_context_cell(value):
        ...
        icon_map = {'big fone': '📞', 'caixas-surpresa': '🎁', 'prova do líder': '👑',
                    'prova do anjo': '😇', 'prova bate e volta': '🛟', 'ganha-ganha': '🎁', 'barrado no baile': '🚫'}
        icon = next((icon_map[k] for k in icon_map if k in value.lower()), '🎬')
...
power_summary = auto_payload.get("power_summary", {}) if 'auto_payload' in dir() and isinstance(auto_payload, dict) else {}
...
if not ps_by_part:
    ...
    fig.add_trace(go.Bar(y=impact.index, x=[-v for v in impact['negativo']],
                         orientation='h', name='Prejuízo', marker_color='#e74c3c'))
    fig.add_trace(go.Bar(y=impact.index, x=impact['positivo'],
                         orientation='h', name='Benefício', marker_color='#2ecc71'))
