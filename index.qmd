...
<thead>
<tr>
<th class="th-soft">→ deu / ↓ recebeu</th>
''')
...
<span>{ctx_active} participantes ativos</span>
    ...
    for card in hl_cards:
        ...
        if ctype == "ranking":
            ...
            # ── Top 6 (only score > 0, medal borders) ──
            medals = ["🥇", "🥈", "🥉", "4º", "5º", "6º"]
            medal_borders = ["#f1c40f", "#c0c0c0", "#cd7f32", "#27ae60", "#27ae60", "#27ae60"]
            podium_all = card.get("podium_all") or podium
            # Only show participants with positive scores
            podium_positive = [p for p in podium_all if p["score"] > 0]
            inline_podium = podium_positive[:6]
            overflow_podium = podium_positive[6:]
            podium_html = '<div>'
            ...
            podium_html += '</div>'  # close column wrapper

            # ── Bottom 6 (only score < 0, red borders, worst first) ──
            bottom_html = ""
            bottom_all = card.get("bottom_all") or bottom3
            if bottom_all:
                # Only show participants with negative scores
                ...
                if inline_bottom:
                    ...
                    bottom_html += '</div>'  # close column wrapper

            # ── Movers (up to 3 risers + 3 fallers inline, overflow toggle) ──
            movers_html = ""
            ...
            if inline_deltas:
                ...
                for md in mdown_list:
                    movers_html += (
                        ...
                        f'</div>')
                if not mup_list and not mdown_list:
                ...
                movers_html += '</div>'  # close column wrapper
...
<span class="fs-base meta-text">reações mudaram <span style="color:{direction_color};font-weight:600;">({pct}%)</span></span>
</div>
...
<span style="color:{direction_color};font-weight:700;">saldo {net_txt}</span>
        ...
        # ── 💥 Dramatic + ⚠️ Hostilities — pair-based cards ──
            ...
            card_title = card["title"] if scope == "today" else (
                "Hostilidades Recentes" if is_hostile else "Mudanças Dramáticas (Recente)"
            )
            if is_hostile:
                subtitle = (
            ...
            def _event_row(it: dict) -> str:
                ...
                if is_hostile:
                    ...
                    main_txt = f'{g.split()[0]} {new_e} {r.split()[0]}'
                    sub_txt = f'Hostilidade unilateral · {old_e}→{new_e} · {when_txt}'
                    g_color, r_color, row_border = "#f39c12", "#e74c3c", "#f39c12"
                else:
                    old_e = it.get("old_emoji", "?")
                    new_e = it.get("new_emoji", "?")
                    sev = it.get("severity", 0)
                    sev_txt = "alta" if sev >= 1.5 else ("média" if sev >= 1.0 else "leve")
                    main_txt = f'{g.split()[0]} {old_e}→{new_e} {r.split()[0]}'
                    sub_txt = f'Mudança {sev_txt} · {when_txt}'
                    g_color, r_color = "#666", "#27ae60" if new_e == "❤️" else "#e74c3c"
                    row_border = "#e74c3c" if sev >= 1.5 else "#8e44ad"

                return (
                    f'<div class="u-s342" style="border-left:3px solid {row_border};">'
                    f'{av(g, 42, g_color)}'
                    f'<div style="display:flex;flex-direction:column;gap:2px;min-width:0;flex:1;">'
                    f'<span class="fs-sm" style="font-weight:600;">{main_txt}</span>'
                    f'<span class="fs-2xs" style="opacity:0.68;">{sub_txt}</span>'
                    f'</div>'
                    f'{av(r, 42, r_color)}'
                    f'</div>'
                )
...
{rows_html}
        ...
        # ── 💔 Streak Breaks ──
            ...
            def _break_row(it: dict) -> str:
                ...
                date_badge = f'<span class="fs-2xs" style="opacity:0.7;margin-left:auto">Rompimento: {when_txt}</span>' if when_txt else ""
                ...
                return (
                    ...
                    f'{av(g, 42, av_group_border(g))}'
                    f'<div class="u-s057">'
                    f'<span class="fs-xs meta-chip">{streak}d ❤️</span>'
                    f'<span class="fs-base">→ {new_e}</span>'
                    f'</div>'
                    f'{av(r, 42, av_group_border(r))}'
                    f'{alert_html}'
                    f'{date_badge}'
                    f'</div>'
                )
...
{rows_html}
        ...
        # ── 🛡️ Mais Blindados ──
            ...
            def _blindado_row(it: dict) -> str:
                ...
                n_bv = it.get("bv_escapes", 0)
                exposure = it.get("exposure", paredao_count)
                prot = it.get("protected", 0)
                votes = it.get("votes_total", it.get("votes", 0))

                border = "#3498db" if prot >= 3 else ("#27ae60" if exposure == 0 else "#f39c12")
                par_color = "#27ae60" if exposure == 0 else ("#f39c12" if exposure <= 1 else "#e74c3c")
                if n_bv > 0:
                    par_label = f"{paredao_count} {'paredão' if paredao_count == 1 else 'paredões'} + {n_bv} Bate-Volta"
                else:
                    par_label = f"{paredao_count} {'paredão' if paredao_count == 1 else 'paredões'}"
                bar_pct = min(100, prot / n_par * 100) if n_par > 0 else 0
                prot_badges = []
                if n_bv > 0:
                    bv_text = it.get("bv_text", f"Bate-Volta {n_bv}x")
                    prot_badges.append(f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;background:#1b3f5c;color:#9dd3ff;'>🔄 {bv_text}</span>")
                nom_text = it.get("nom_text", "")
                if nom_text:
                    prot_badges.append(f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;background:#2d1b3f;color:#d9b3ff;'>{nom_text}</span>")
                prot_text = it.get("prot_text", "")
                if prot_text:
                    prot_badges.append(f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;background:#123b2a;color:#8fe3b8;'>{prot_text}</span>")
                prot_badges_html = " ".join(prot_badges)
                extra_badges = f"<div style='margin-top:3px;'>{prot_badges_html}</div>" if prot_badges_html else ""
                ...
                return (
                    ...
                    f'</div>'
                    f'<div class="fs-2xs" style="color:#888;">{votes} voto{"s" if votes != 1 else ""} · protegido {prot}x</div>'
                    f'{extra_badges}'
                    f'</div></div>'
                )
            ...
            print(f'''<div class="info-panel" style="border-left-color:{color};">
{card_header(card["icon"], card["title"], card.get("link"), f"{n_par} paredões", source_tag="📈 Acumulado", subtitle="Menor exposição a paredões e Bate-Volta. Proteções (Líder, Imune, Anjo) reduzem exposição.")}
{rows_html}
</div>''')
...
def render_status_chip(emoji, label, level, color, detail_html=""):
    ...
    if detail_html:
        return (
            ...
            f'</summary>'
            f'<div class="status-breakdown">{detail_html}</div>'
            f'</details>'
        )
    return (
...
for p in profiles:
    ...
    for vd in gs.get("house_votes_detail", []):
        ...
        votes_detail_rows += (
            ...
            f"<span class='meta-inline'>Votaram: {voters_str}</span></div>"
        )
    ...
    for ph in gs.get("paredao_history", []):
        ...
        paredao_detail_rows += (
            ...
            f"<span class='meta-inline'>Como: {ph.get('como', '?')}</span> · "
