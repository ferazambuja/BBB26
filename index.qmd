...
- **Votos da casa recebidos** aparecem como **“voto secreto (para participantes)”** — só são conhecidos do **público** após a formação.
...
def render_actor_avatars(actors, border_color="#666"):
    if not actors:
        return ''
    parts = []
    for a in actors:
        if a in AVATARS and AVATARS[a]:
            parts.append(
                f'<img src="{AVATARS[a]}" style="width: 26px; height: 26px; border-radius: 50%; '
                f'border: 2px solid {border_color}; object-fit: cover; margin-right: -4px;">'
            )
        else:
            icon = None
...
def make_event_chips(events, border_color):
    if not events:
    ...
    for (etype, actors), meta in grouped.items():
        ...
        actor_html = render_actor_avatars(actors, border_color)
        actor_label = " + ".join(actors) if actors else ""
        title = f"{label} — {actor_label}" if actor_label else label
        actor_text = f"<span style='color:#aaa; margin-left:0.25rem;'>{actor_label}</span>" if actor_label else ""
        chips.append(
            f"<span style='display:inline-flex; align-items:center; gap:0.3rem; background:#2f2f2f; "
            f"border:1px solid {border_color}; color:#ddd; border-radius:999px; padding:0.25rem 0.55rem; "
            f"font-size:0.78rem; margin:0.15rem;' title='{title}'>"
            f"{count_prefix}{emoji} {label} {actor_html}{actor_text}</span>"
        )
    return ' '.join(chips)
...
for p in sorted_active:
    ...
    pos_events_hist = [ev for ev in target_events_all if ev.get('impacto') == 'positivo']
    neg_events_hist = [ev for ev in target_events_all if ev.get('impacto') == 'negativo']

    pos_html = make_event_chips(pos_events, "#28a745")
    neg_html = make_event_chips(neg_events, "#dc3545")
    pos_hist_html = make_event_chips(pos_events_hist, "#2ecc71")
    neg_hist_html = make_event_chips(neg_events_hist, "#e74c3c")
    ...
    animosity_score = (
        ...
        1.5 * sum(event_weight(ev) for ev in animosity_events)
    )
...
<div style="display: grid; grid-template-columns: 1fr; gap: 0.4rem; font-size: 0.9em;">
  <div><strong style="color: #28a745;">+ Benefícios (semana):</strong> {pos_html}</div>
  <div><strong style="color: #dc3545;">− Prejuízos (semana):</strong> {neg_html}</div>
  <div><strong style="color: #6c757d;">🗳️ Votos recebidos</strong> <span style="color:#888;">(secreto para participantes)</span>: {votes_html}</div>
  <div><strong style="color: #999;">Histórico da temporada:</strong> <span style="color:#28a745;">{pos_hist_html}</span> <span style="color:#dc3545;">{neg_hist_html}</span></div>
</div>
</div>
''' if (pos_events_hist or neg_events_hist or vote_map) else ''}
