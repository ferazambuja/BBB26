            ...
            if mup_list or mdown_list:
                ...
                for md in mdown_list:
                    movers_html += (
                        ...
                        f'<span class="text-neg" style="font-size:0.8rem;">📉 {md["name"].split()[0]} {md["delta"]:+.1f}</span>'
                        f'</div>')
                movers_html += '</div></div>'
        ...
        # ── 🎯 Impact ──
            ...
            for it in items[:4]:
                ...
                rows_html += (
                    ...
                    f'<span class="text-neg" style="font-size:0.75rem;">{neg:.1f}</span>'
                    f'</div>'
            ...
            extra = total - 4
            if extra > 0:
                rows_html += f'<div class="tc" style="font-size:0.75rem;color:#888;"><a href="{card.get("link", "#")}" style="color:#aaa;text-decoration:none;">+{extra} mais ↗</a></div>'
            rows_html += '</div>'
            print(f'''<div class="info-panel" style="border-left-color:{color};">
        ...
        # ── 🧭 Strategic divergence ──
            ...
            for i, p in enumerate(podium[:3]):
                strat_html += (
                    ...
                    f'<span style="font-size:0.8rem;color:#ccc;">{_medals3[i]} {p["name"].split()[0]}</span>'
    ...
    if saldo_profiles:
        ...
        for i, p in enumerate(saldo_profiles[:top_n]):
            ...
            saldo_rows += (
                ...
                f'<div style="flex:1;min-width:0;">'
                f'<div style="display:flex;justify-content:space-between;align-items:baseline;">'
                f'<span style="font-size:0.8rem;font-weight:600;color:#fff;">{label} {name.split()[0]}</span>'
                f'<span style="font-size:0.85rem;color:{bal_color};font-weight:700;">{bal:,}</span>'
                f'</div>'
                f'<div style="background:rgba(255,255,255,0.08);border-radius:3px;height:5px;margin-top:2px;overflow:hidden;">'
...
if paredao_names:
    ...
    status_color = "#6c757d"
    status_icon = "⏳"
    detail_html = '<div style="color: #888; font-size: 0.9rem;">Nenhum paredão ativo no momento</div>'
