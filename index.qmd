        ...
        # ── 🎯 Impact ──
            ...
            for it in items[:4]:
                ...
                rows_html += (
                    ...
                    f'<div class="u-s066">'
                    f'<div class="u-s059">'
                    f'<span class="fs-md u-s068">{name.split()[0]}</span>'
...
{rows_html}
        ...
        # ── 🧭 Strategic divergence ──
            ...
            _medals3 = ["🥇", "🥈", "🥉"]
            strat_html = '<div class="u-s360">'
            for i, p in enumerate(podium[:3]):
                strat_html += (
                    f'<div class="u-s343">'
                    f'{av(p["name"], 42, "#9b59b6")}'
                    f'<span class="fs-md u-s017">{_medals3[i]} {p["name"].split()[0]}</span>'
                    f'<span class="fs-md u-s261">{p["score"]:+.2f}</span>'
                    f'</div>'
                )
            strat_html += '</div>'
            print(f'''<div class="info-panel" style="border-left-color:{color};">
...
def render_mobile_strategic_summary(today_rows, strategic_rows):
    ...
    risk_html = _render_list(
        ...
        lambda r: (
            f"estratégico <span class='u-s005'>{_fmt_signed(r.get('score', 0), 2)}</span> "
    ...
    power_html = _render_list(
        ...
        lambda r: (
            f"estratégico <span class='u-s054'>{_fmt_signed(r.get('score', 0), 2)}</span> "
            f"(queridômetro {r.get('queridometro_score', 0):+.1f})"
        )
    )
    ...
    return (
        ...
        '<p class="ranking-mobile-note u-s011">Quem muda mais de posição entre popularidade e jogo.</p>\n'
        f'{gap_html}\n'
        '</div>\n'
        '<div class="ranking-mobile-card">\n'
        ...
        '<a href="evolucao.html#estrategico" class="u-s059">abrir evolução estratégica ↗</a>\n'
        '</div>\n'
        '</div>'
    )
...
if strategic_rows:
    ...
    if divergences:
        ...
        print("<div class='table-md u-s072'>")
        for name, qr, sr, delta, cs, qs in divergences:
            if delta > 0:
                ...
                color = "#e74c3c"
            print(f"<div class='sep-bottom u-s019'>"
