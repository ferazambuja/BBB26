        ...
        # ── 🛡️ Mais Blindados ──
            ...
            def _blindado_row(it: dict) -> str:
                ...
                if has_bv and bv_text:
                    prot_badges.append(
                        f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;background:#1b3f5c;color:#9dd3ff;'>{escape(bv_text)}</span>"
                    )
                for tag in protection_tags:
                    label = tag.get("label")
                    tone_style = "background:#2d1b3f;color:#d9b3ff;" if label == "Líder" else "background:#123b2a;color:#8fe3b8;"
                    prot_badges.append(
                        f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;{tone_style}'>{escape(tag.get('text', ''))}</span>"
                    )
