...
<span>📅 <b style="color:#ccc;">Semana {ctx_week}</b></span>
...
{strat_html}
    ...
    if saldo_profiles:
        ...
        saldo_rows += '</div>'
        print(f'''<div style="background:#2a2a2a;border-radius:12px;border-left:4px solid #f1c40f;padding:1rem;box-shadow:0 2px 8px rgba(0,0,0,0.3);">
{card_header("💰", "Saldo de Estalecas", "#saldo", source_tag="📸 Dado do dia", subtitle="Ranking dos participantes com mais estalecas. Moeda do jogo usada em compras e dinâmicas de poder.")}
{saldo_rows}
</div>''')
...
# Saldo de Estalecas {#saldo}

<p class="text-muted small">📸 <strong>Dado do dia</strong> — saldo atual de estalecas de cada participante.</p>
...
active_profiles.sort(key=lambda x: x.get("balance", 0), reverse=True)
...
for i, p in enumerate(active_profiles, 1):
    name = p["name"]
    balance = p.get("balance", 0)
    grupo = p.get("group", "")
    cor = GROUP_COLORS.get(grupo, "#999")

    # Color by balance value
    if balance > 1000:
        val_color = "#1a9850"
    elif balance > 0:
        val_color = "#66bd63"
    ...
    avatar = av(name, size=36, border_color=cor)
