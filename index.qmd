...
def render_mobile_queridometro_summary(today_rows, change_week):
    ...
    week_up_html = _render_list(
        ...
        lambda r: f"<span style='color:#27ae60;'>{_fmt_signed(r.get('delta', 0), 1)} pts</span>"
    )
    week_down_html = _render_list(
        week_down,
        "Sem quedas relevantes na semana.",
        lambda r: f"<span style='color:#e74c3c;'>{_fmt_signed(r.get('delta', 0), 1)} pts</span>"
    )
    ...
    return (
        ...
        '</div>\n'
        '<p class="ranking-mobile-note" style="margin-top:0.5rem;">Resumo diferente do card do topo: foco em risco e concentração, não em pódio.</p>\n'
        '</div>\n'
        '<div class="ranking-mobile-card">\n'
        '<h4>🗓️ Viradas da semana</h4>\n'
        '<p class="ranking-mobile-note" style="margin:0;">Maior alta</p>\n'
        f'{week_up_html}\n'
        '<p class="ranking-mobile-note" style="margin:0.45rem 0 0;">Maior queda</p>\n'
        f'{week_down_html}\n'
        '</div>\n'
        '<div class="ranking-mobile-card">\n'
        ...
        '<a href="evolucao.html#sentimento" style="color:#7cb7ff;">ver evolução do queridômetro ↗</a>\n'
        '</div>\n'
        '</div>'
    )


def render_mobile_strategic_summary(today_rows, strategic_rows):
    if not strategic_rows:
        return "<div class='ranking-mobile-summary'><p class='ranking-mobile-note'>Dados estratégicos indisponíveis.</p></div>"

    quer_sorted = sorted(today_rows, key=lambda x: x.get("score", 0), reverse=True)
    strat_sorted = sorted(strategic_rows, key=lambda x: x.get("score", 0), reverse=True)
    ...
    for row in strategic_rows:
        name = row["name"]
        qr = quer_rank.get(name)
        sr = strat_rank.get(name)
    ...
    gap_top = sorted(gaps, key=lambda r: abs(r["gap"]), reverse=True)[:4]
    hidden_risk = sorted(
        [r for r in strategic_rows if r.get("queridometro_score", 0) > 0 and r.get("score", 0) < 0],
    ...
    gap_html = _render_list(
        gap_top,
        "Sem divergências fortes no momento.",
        lambda r: (
            f"Q#{r['qr']} → E#{r['sr']} "
            f"(<span style='color:{'#27ae60' if r['gap'] > 0 else '#e74c3c'};'>"
            f"{'sobe' if r['gap'] > 0 else 'cai'} {abs(r['gap'])}</span>)"
        )
    )
    risk_html = _render_list(
        hidden_risk,
        "Sem risco oculto forte agora.",
        lambda r: (
            f"estratégico <span style='color:#e74c3c;'>{_fmt_signed(r.get('score', 0), 2)}</span> "
            f"(queridômetro {r.get('queridometro_score', 0):+.1f})"
        )
    )
    power_html = _render_list(
        hidden_power,
        "Sem influência subestimada detectada.",
        lambda r: (
            f"estratégico <span style='color:#27ae60;'>{_fmt_signed(r.get('score', 0), 2)}</span> "
            f"(queridômetro {r.get('queridometro_score', 0):+.1f})"
        )
    )
    ...
    return (
        '<div class="ranking-mobile-summary">\n'
        '<div class="ranking-mobile-card">\n'
        '<h4>🧭 Desalinhamento de leitura</h4>\n'
        '<p class="ranking-mobile-note" style="margin:0;">Quem muda mais de posição entre popularidade e jogo.</p>\n'
        f'{gap_html}\n'
        '</div>\n'
        '<div class="ranking-mobile-card">\n'
        '<h4>⚠️ Risco de voto surpresa</h4>\n'
        f'{risk_html}\n'
        '</div>\n'
        '<div class="ranking-mobile-card">\n'
        '<h4>🕵️ Influência subestimada</h4>\n'
        f'{power_html}\n'
        '</div>\n'
        '<div class="ranking-mobile-cta ranking-mobile-note">\n'
        'Gráfico estratégico completo:\n'
        '<a href="evolucao.html#estrategico" style="color:#7cb7ff;">abrir evolução estratégica ↗</a>\n'
        '</div>\n'
        '</div>'
    )
...
def render_mobile_evolution_summary(data_rows, score_key, section_title, cta_href, score_decimals=1):
    ...
    return (
        ...
        f'<p class="ranking-mobile-note" style="margin:0;">Janela: {date_window}</p>\n'
        '<p class="ranking-mobile-note" style="margin:0.35rem 0 0;">Subidas</p>\n'
        f'{up_html}\n'
        '<p class="ranking-mobile-note" style="margin:0.45rem 0 0;">Quedas</p>\n'
        f'{down_html}\n'
        '</div>\n'
        '<div class="ranking-mobile-cta ranking-mobile-note">\n'
        'Série completa:\n'
        f'<a href="{cta_href}" style="color:#7cb7ff;">abrir gráficos detalhados ↗</a>\n'
        '</div>\n'
        '</div>'
    )
```
...
print(render_mobile_strategic_summary(today_rows, strategic_rows))
...
if strategic_rows:
    ...
    if divergences:
        ...
        print("<details class='ranking-desktop-only'><summary><strong>Maiores divergências entre rankings</strong> (≥3 posições)</summary>")
...
_raw_par = json.loads(Path('data/paredoes.json').read_text(encoding='utf-8')).get('paredoes', [])
