...
for p in profiles:
    ...
    sinc = p.get("sincerao", {})
    SINC_EMOJI = {"podio": "🏆", "bomba": "💣", "nao_ganha": "🚫",
                  "paredao_perfeito": "🏛️", "regua": "📏", "regua_fora": "❌",
                  "quem_sai": "🚪", "prova_eliminou": "⚡"}
    all_interactions = sinc.get("all_interactions", [])
    sinc_summary = sinc.get("summary", {})
    sinc_total = sinc_summary.get("total_positive", 0) + sinc_summary.get("total_negative", 0)
    sincerao_html = ""
    if all_interactions:
        sinc_rows = []
        for wi, week_data in enumerate(all_interactions):
            wk = week_data.get("week", "?")
            ints = week_data.get("interactions", [])
            chips_html = ""
            for ix in ints:
                etype = ix.get("type", "")
                emoji = SINC_EMOJI.get(etype, "•")
                actor_first = ix.get("actor", "").split()[0] if ix.get("actor") else ""
                tema = ix.get("tema", etype)
                ...
                chips_html += (
                    ...
                    f"{emoji} {actor_first} <span style='opacity:0.7'>{tema}</span></span> "
                )
            week_label = f"<span class='fs-2xs' style='opacity:0.6;min-width:24px'>S{wk}</span>"
            row = f"<div style='display:flex;flex-wrap:wrap;gap:2px 6px;align-items:baseline'>{week_label}{chips_html}</div>"
            sinc_rows.append((wi, row))
        # Show most recent 2 weeks inline, rest in <details>
        visible = [r for i, r in sinc_rows if i < 2]
        hidden = [r for i, r in sinc_rows if i >= 2]
        sincerao_html = "".join(visible)
        if hidden:
            sincerao_html += (
                f"<details style='margin-top:2px'><summary class='fs-2xs' style='cursor:pointer;opacity:0.6'>"
                f"+ {len(hidden)} semana{'s' if len(hidden) > 1 else ''} anterior{'es' if len(hidden) > 1 else ''}</summary>"
