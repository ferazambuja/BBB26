...
def render_actor_avatars(actors, border_color="#666"):
    ...
    for a in actors:
        if a in AVATARS and AVATARS[a]:
            parts.append(
                f'<img src="{AVATARS[a]}" style="width: 22px; height: 22px; border-radius: 50%; '
                f'border: 2px solid {border_color}; object-fit: cover; margin-right: -6px;">'
            )
        else:
            icon = None
...
def make_event_chips(events, border_color, positive=True):
    if not events:
    ...
    for (etype, actors), meta in grouped.items():
        ...
        chips.append(
            ...
            f"border:1px solid {border_color}; color:#ddd; border-radius:999px; padding:0.2rem 0.45rem; "
...
for p in sorted_active:
    ...
    target_events = [ev for ev in power_events if ev.get('target') == name and ev.get('week') == current_week]
    pos_events = [ev for ev in target_events if ev.get('impacto') == 'positivo']
    neg_events = [ev for ev in target_events if ev.get('impacto') == 'negativo']

    pos_html = make_event_chips(pos_events, "#28a745", positive=True)
    neg_html = make_event_chips(neg_events, "#dc3545", positive=False)
    ...
    animosity_score = (
        ...
        1.5 * sum(1 for ev in power_events if ev.get('impacto') == 'negativo'
                  and name in normalize_actors(ev) and ev.get('week') == current_week) +
        0.75 * sum(1 for ev in power_events if ev.get('type') == 'indicacao'
                  and name in normalize_actors(ev) and ev.get('week') == current_week)
    )
...
<div style="display: grid; grid-template-columns: 1fr; gap: 0.4rem; font-size: 0.9em;">
  <div><strong style="color: #28a745;">+ Benefícios:</strong> {pos_html}</div>
  <div><strong style="color: #dc3545;">− Prejuízos:</strong> {neg_html}</div>
...
''' if (pos_events or neg_events or vote_map) else ''}
