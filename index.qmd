...
def card_header(icon, title, link=None, badge=None, source_tag=None, subtitle=None):
    ...
    return (f'<div class="u-s073 dashboard-card-header">'
            f'<div class="dashboard-card-header-main">'
            f'<span class="dashboard-card-icon fs-2xl">{icon}</span>'
            f'<span class="dashboard-card-title fs-base u-s392">{title_html}</span>'
            f'</div>'
            f'{meta_html}'
            f'{subtitle_html}</div>')
...
def pair_story_card(
    ...
    *,
    ...
    left_border = left_border or av_group_border(giver)
    right_border = right_border or av_group_border(receiver)
    return (
        ...
        if ctype == "ranking":
            ...
            def _toggle(n):
                return (
                    ...
                    f'border-radius:10px;">⋯ {n}</span></summary>'
                )
            ...
            podium_html += '</div>'
            if overflow_podium:
                podium_html += _toggle(len(overflow_podium))
                podium_html += '<div class="u-s065">'
            ...
            if bottom_all:
                ...
                if inline_bottom:
                    ...
                    for p in inline_bottom:
                        name = p["name"]
                        score = p["score"]
                        score_color = "#e74c3c" if score < -1 else "#f39c12"
                        bottom_html += (
                            f'<div class="u-s340">'
                            f'{av(name, 42, "#e74c3c")}'
                    ...
                    bottom_html += '</div>'
                    if overflow_bottom:
                        bottom_html += _toggle(len(overflow_bottom))
                        bottom_html += '<div class="u-s065">'
            ...
            if inline_deltas:
                ...
                movers_html += '</div>'
                if overflow_deltas:
                    movers_html += _toggle(len(overflow_deltas))
                    movers_html += '<div class="u-s065">'
        ...
        # ── 💥 Dramatic + ⚠️ Hostilities — pair-based cards ──
            ...
            def _event_row(it: dict) -> str:
                ...
                if is_hostile:
                    ...
                    transition_html = (
                        f'<span class="pair-story-origin">{old_e}</span>'
                        f'<span class="pair-story-arrow">→</span>'
                        f'<span class="pair-story-destination">{new_e}</span>'
                    )
                    ...
                    sev_txt = "alta" if sev >= 1.5 else ("média" if sev >= 1.0 else "leve")
                    transition_html = (
                        f'<span class="pair-story-origin">{old_e}</span>'
                        f'<span class="pair-story-arrow">→</span>'
                        f'<span class="pair-story-destination">{new_e}</span>'
                    )
                    meta_txt = f"Mudança {sev_txt} · {when_txt}"
                    row_border = "#e74c3c" if sev >= 1.5 else "#8e44ad"
...
{rows_html}
        ...
        # ── 💔 Streak Breaks ──
            ...
            def _break_row(it: dict) -> str:
                ...
                meta_txt = f"Rompimento{' grave' if sev == 'strong' else ''}"
                if when_txt:
                    meta_txt += f" · {when_txt}"
                transition_html = (
        ...
        # ── 🛡️ Mais Blindados ──
            ...
            def _blindado_row(it: dict) -> str:
                ...
                if has_bv and bv_text:
                    prot_badges.append(
                        f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;background:#1b3f5c;color:#9dd3ff;white-space:nowrap;display:inline-flex;align-items:center;flex:0 0 auto;'>{escape(bv_text)}</span>"
                    )
                for tag in protection_tags:
                    label = tag.get("label")
                    tone_style = "background:#2d1b3f;color:#d9b3ff;" if label == "Líder" else "background:#123b2a;color:#8fe3b8;"
                    prot_badges.append(
                        f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;{tone_style}white-space:nowrap;display:inline-flex;align-items:center;flex:0 0 auto;'>{escape(tag.get('text', ''))}</span>"
                    )
                prot_badges_html = "".join(prot_badges)
                extra_badges = (
                    f"<div style='margin-top:3px;display:flex;flex-direction:column;align-items:flex-start;gap:4px;'>{prot_badges_html}</div>"
                    if prot_badges_html else ""
                )
...
{rows_html}
        ...
        # ── 🎯 Mais Visados ──
            ...
            display_limit = int(card.get("display_limit") or len(items) or 5)
            n_par = card.get("n_paredoes", 1)
            recent_window = card.get("recent_window", 3)
            inline_items = items_all[:display_limit]
            overflow_items = items_all[display_limit:]
            max_votes = max((it.get("votes_total", 0) for it in items_all), default=1) or 1

            def _visado_row(it: dict) -> str:
                name = it["name"]
                name_slug = name.lower().replace(" ", "-")
                paredao_count = it.get("paredao", 0)
                votes_total = it.get("votes_total", 0)
                votes_recent = it.get("votes_recent", 0)
                intensity = float(it.get("intensity_prevote", 0.0))
                bv_count = it.get("bv_escapes", 0)
                fake_count = it.get("fake_paredao_count", 0)
                fake_nums = it.get("fake_paredao_nums", [])
                by_lider = it.get("by_lider", 0)
                by_casa = it.get("by_casa", 0)
                by_dynamic = it.get("by_dynamic", 0)
                ...
                par_label = f"{paredao_count} paredão" if paredao_count == 1 else f"{paredao_count} paredões"
                bar_pct = min(100, votes_total / max_votes * 100) if max_votes > 0 else 0
                ...
                if fake_count:
                    fake_txt = ", ".join(f"{n}º" for n in fake_nums)
                    badges.append(
                        f"<span class='fs-2xs' style='padding:1px 6px;border-radius:999px;background:#123b2a;color:#8fe3b8;white-space:nowrap;display:inline-flex;align-items:center;flex:0 0 auto;'>Paredão falso {fake_count}x ({fake_txt})</span>"
                    )
                if by_lider:
                    badges.append(
                ...
                return (
                    ...
                    f'</div>'
                    f'<div class="fs-2xs" style="color:#888;">{votes_total} voto{"s" if votes_total != 1 else ""} total · {votes_recent} recente{"s" if votes_recent != 1 else ""} ({recent_window} ciclos) · intensidade {intensity_pct}%</div>'
                    f'{extra_badges}'
                    f'</div></div>'
                )

            rows_html = '<div class="u-s058">'
            rows_html += "".join(_visado_row(it) for it in inline_items)
            if overflow_items:
                rows_html += (
                    f'<details class="sinc-more">'
                    f'<summary>+{len(overflow_items)} restantes</summary>'
                    f'<div class="u-s058">{"".join(_visado_row(it) for it in overflow_items)}</div>'
                    f'</details>'
                )
            rows_html += '</div>'

            print(f'''<div class="info-panel" style="border-left-color:{color};">
{card_header(card["icon"], card["title"], card.get("link"), f"{n_par} paredões", source_tag="📈 Acumulado", subtitle="Participantes mais pressionados por paredão, votos recentes e intensidade de votos quando elegíveis.")}
{rows_html}
</div>''')
