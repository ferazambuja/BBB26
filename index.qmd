...
from data_utils import require_clean_manual_events
...
if contrad:
    top_list = [f"{c.get('ator')} → {c.get('alvo')} ({'não ganha' if c.get('tipo') == 'nao_ganha' else (c.get('tema') or 'bomba')})"
                for c in contrad][:5]
    print(f'''
...
''')
    ...
    for i, p in enumerate(top_vulnerable):
        ...
        risk_label = p.get('risk_label', 'Atenção')
...
if change_yesterday:
    print(f"<p class='text-muted small'>Mudanças de <strong>{yesterday_label}</strong> para <strong>{latest.get('label','')}</strong></p>")
...
if change_week:
    print(f"<p class='text-muted small'>Mudanças de <strong>{week_ago_label}</strong> para <strong>{latest.get('label','')}</strong></p>")
    print(make_sentiment_change_table(change_week, "7 dias"))
else:
    print(f"<p class='text-muted'>Dados insuficientes (apenas {overview.get('n_daily', 0)} dias disponíveis).</p>")
```
...
align_rows = index_data.get("sincerao", {}).get("alignment", [])
sinc_week = index_data.get("sincerao", {}).get("week")
if not align_rows:
    if sinc_week:
        print(f"<p class='text-muted'>Sem dados detalhados do Sincerão para a semana {sinc_week}.</p>")
    else:
        print("<p class='text-muted'>Sem dados detalhados do Sincerão.</p>")
else:
    df_sinc = pd.DataFrame(align_rows)
    fig = px.scatter(
        df_sinc,
        x="sincerao_score",
        y="sentiment_score",
    ...
    contrad = []
    for edge in index_data.get("contradictions", []):
        contrad.append({"ator": edge.get("ator"), "alvo": edge.get("alvo"), "tipo": edge.get("tipo")})
...
''' if (events.get('pos_week') or events.get('neg_week') or votes or sincerao_html) else ''}
...
<div style="color: #6c757d; font-size: 0.8em; margin-top: 0.3rem;">→ Eles dão negativa para você. Podem votar contra sem que você espere.</div>
</div>
''')
...
<div style="font-weight: bold; color: #6f42c1; margin-bottom: 0.3rem;">🗡️ Inimigos Não Declarados ({len(blind_targets)})</div>
<div style="color: #ccc; font-size: 0.9em;">{bt_str}</div>
<div style="color: #6c757d; font-size: 0.8em; margin-top: 0.3rem;">→ Eles dão ❤️ para você, mas você é hostil. Eles não sabem que são seus alvos.</div>
</div>
''')
