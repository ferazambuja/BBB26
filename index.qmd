...
if timeline_events:
    ...
    for week_num, week_events in sorted_weeks:
        ...
        for ev in reversed(week_events):
            ...
            label = cat_labels.get(cat, cat.replace("_", " ").capitalize())
            emoji = ev.get("emoji", "")
            title = ev.get("title", "")
    ...
    print("<p class='text-muted'>Nenhum evento na cronologia. Execute <code>python scripts/build_derived_data.py</code>.</p>")
```
            ...
            if bottom3:
                ...
                for p in bottom3[:6]:
                    ...
                    bottom_html += (
                        ...
                        f'{av(name, 42, "#e74c3c")}'
                        f'<span style="font-size:0.8rem;color:{score_color};font-weight:700;">⬇ {name.split()[0]} {score:+.1f}</span>'
                        f'</div>'
                    )
                bottom_html += '</div></div>'
...
```{python}
#| label: big-fone-consensus
#| output: asis

bfc = index_data.get("big_fone_consensus")
if bfc and bfc.get("attendees"):
    attendees = bfc["attendees"]
    targets = bfc["targets"]
    potential_3rd = bfc["potential_3rd"]
    n_consensus = bfc.get("n_consensus_targets", 0)

    # --- Header with bracelet holder avatars ---
    html = '<div style="background:#1a1a2e; border:1px solid #444; border-radius:12px; padding:1.5rem; margin:1.5rem 0;">'
    html += '<h2 style="margin-top:0; color:#f0c040;">📞 Big Fone — Consenso para o Paredão</h2>'
    html += '<p style="color:#aaa; font-size:0.9em;">Os portadores da pulseira dourada devem, em consenso, indicar alguém ao paredão.</p>'

    # Attendee avatars
    html += '<div style="display:flex; align-items:center; gap:12px; margin:1rem 0; flex-wrap:wrap;">'
    html += '<span style="color:#999; font-size:0.85em;">Pulseiras:</span>'
    for att in attendees:
        gc = GROUP_COLORS.get(MEMBER_OF.get(att, ""), "#555")
        html += f'<div style="text-align:center;">{av(att, 52, gc)}<div style="font-size:0.75em; color:#ccc; margin-top:2px;">{att.split()[0]}</div></div>'
    # Pending 3rd slot
    html += '<div style="text-align:center;"><div style="width:52px;height:52px;border-radius:50%;border:2px dashed #f0c040;display:flex;align-items:center;justify-content:center;font-size:1.5em;">?</div><div style="font-size:0.75em; color:#f0c040; margin-top:2px;">3º atendente</div></div>'
    html += '</div>'

    # --- Consensus targets ---
    consensus_targets = [t for t in targets if t["tier"] == "consensus"]
    split_targets = [t for t in targets if t["tier"] == "split"]

    if consensus_targets:
        html += f'<h3 style="color:#e74c3c; margin-top:1.5rem;">⭐ Alvos de Consenso ({len(consensus_targets)})</h3>'
        html += '<p style="color:#aaa; font-size:0.85em;">Ambos têm score negativo — indicação natural.</p>'
        html += '<div style="display:flex; flex-wrap:wrap; gap:10px;">'
        for t in consensus_targets:
            gc = GROUP_COLORS.get(t.get("member_of", ""), "#555")
            card = f'<div style="background:#2a1a1a; border:1px solid #c0392b; border-radius:10px; padding:10px; min-width:180px; flex:1; max-width:250px;">'
            card += f'<div style="display:flex; align-items:center; gap:8px;">{av(t["name"], 40, gc)}<div><strong style="color:#eee;">{t["name"]}</strong><div style="font-size:0.8em; color:#e74c3c;">Score: {t["combined_score"]:+.2f}</div></div></div>'
            card += '<div style="display:flex; gap:6px; margin-top:8px; flex-wrap:wrap;">'
            for att in attendees:
                sc = t["scores"].get(att, 0)
                em = t["emojis"].get(att, "?")
                sc_color = "#e74c3c" if sc < 0 else "#2ecc71"
                att_short = att.split()[0]
                card += f'<span style="background:#333; padding:2px 6px; border-radius:4px; font-size:0.8em;" title="{att_short}">{em} <span style="color:{sc_color}">{sc:+.2f}</span></span>'
            card += '</div></div>'
            html += card
        html += '</div>'

    if split_targets:
        html += f'<h3 style="color:#f39c12; margin-top:1.5rem;">🔀 Opinião Dividida ({len(split_targets)})</h3>'
        html += '<p style="color:#aaa; font-size:0.85em;">Um dos portadores tem score positivo — mais difícil consenso.</p>'
        html += '<div style="display:flex; flex-wrap:wrap; gap:10px;">'
        for t in split_targets[:6]:
            gc = GROUP_COLORS.get(t.get("member_of", ""), "#555")
            card = f'<div style="background:#2a2a1a; border:1px solid #f39c12; border-radius:10px; padding:10px; min-width:180px; flex:1; max-width:250px;">'
            card += f'<div style="display:flex; align-items:center; gap:8px;">{av(t["name"], 40, gc)}<div><strong style="color:#eee;">{t["name"]}</strong><div style="font-size:0.8em; color:#f39c12;">Score: {t["combined_score"]:+.2f}</div></div></div>'
            card += '<div style="display:flex; gap:6px; margin-top:8px; flex-wrap:wrap;">'
            for att in attendees:
                sc = t["scores"].get(att, 0)
                em = t["emojis"].get(att, "?")
    ...
    # --- Potential 3rd attendee analysis (all participants) ---
    if potential_3rd:
        consensus_names = {t["name"] for t in consensus_targets}
        consensus_target_names = [t["name"] for t in consensus_targets]

        # Sort: facilitators first (most agrees), then partial, then disruptors
        p3_sorted = sorted(potential_3rd, key=lambda x: (-x["n_agrees"], x["n_disagrees"]))

        # Separate consensus targets from others
        p3_targets = [p for p in p3_sorted if p.get("is_consensus_target")]
        p3_others = [p for p in p3_sorted if not p.get("is_consensus_target")]

        html += '<h3 style="color:#9b59b6; margin-top:1.5rem;">🔮 Se o 3º atendente fosse...</h3>'
        html += '<p style="color:#aaa; font-size:0.85em;">Como cada possível 3º atendente se alinharia com os alvos de consenso. Score negativo = concordaria em indicar.</p>'

        # Target name short headers for the grid
        ct_short = [n.split()[0] for n in consensus_target_names]

        def render_3rd_table(entries, section_title=None, section_desc=None):
            """Render a table of potential 3rd attendees with per-target scores."""
            tbl = ''
            if section_title:
                tbl += f'<h4 style="margin-top:1rem; color:#f0c040;">{section_title}</h4>'
            if section_desc:
                tbl += f'<p style="color:#aaa; font-size:0.8em; margin:0.25rem 0 0.5rem;">{section_desc}</p>'
            tbl += '<div style="overflow-x:auto;">'
            tbl += '<table style="width:100%; border-collapse:collapse; font-size:0.85em;">'
            # Header row
            tbl += '<tr style="border-bottom:1px solid #444;">'
            tbl += '<th style="text-align:left; padding:6px 8px; color:#ccc;">Participante</th>'
            for cn in ct_short:
                tbl += f'<th style="text-align:center; padding:6px 4px; color:#e74c3c; font-size:0.85em;">{cn}</th>'
            tbl += '<th style="text-align:center; padding:6px 8px; color:#ccc;">Alinhamento</th>'
            tbl += '</tr>'
            for p in entries:
                gc = GROUP_COLORS.get(p.get("member_of", ""), "#555")
                is_ct = p.get("is_consensus_target", False)
                row_bg = "background:#1a1a2e;" if not is_ct else "background:#2a1a2a;"
                border_style = f"border-left:3px solid #9b59b6;" if is_ct else ""
                tbl += f'<tr style="{row_bg} {border_style} border-bottom:1px solid #333;">'
                # Name cell
                name_label = p["name"].split()[0]
                escape_badge = ' <span style="background:#9b59b6; color:#fff; font-size:0.7em; padding:1px 5px; border-radius:3px;">ESCAPA</span>' if is_ct else ''
                tbl += f'<td style="padding:6px 8px; white-space:nowrap;">'
                tbl += f'<div style="display:flex; align-items:center; gap:6px;">{av(p["name"], 28, gc)}'
                tbl += f'<span style="color:#eee;">{name_label}</span>{escape_badge}</div></td>'
                # Per-target score cells
                t_scores = p.get("target_scores", {})
                for ct_name in consensus_target_names:
                    if ct_name == p["name"]:
                        tbl += '<td style="text-align:center; padding:6px 4px; color:#666;">—</td>'
                    else:
                        sc = t_scores.get(ct_name, 0)
                        if sc < -1:
                            sc_color = "#2ecc71"
                            icon = "✓✓"
                        elif sc < 0:
                            ...
                            sc_color = "#c0392b"
                            icon = "✗✗"
                        tbl += f'<td style="text-align:center; padding:6px 4px;">'
                        tbl += f'<span style="color:{sc_color};" title="{sc:+.2f}">{icon}</span></td>'
                # Alignment summary
                n_a = p["n_agrees"]
                n_d = p["n_disagrees"]
                total = n_a + n_d
                if total == 0:
                    ...
                    align_color = "#2ecc71"
                    align_text = f"{n_a}/{total} ✓"
                elif n_a == 0:
                    align_color = "#e74c3c"
                    align_text = f"{n_d}/{total} ✗"
                else:
                    align_color = "#f39c12"
                    align_text = f"{n_a}✓ {n_d}✗"
                tbl += f'<td style="text-align:center; padding:6px 8px; color:{align_color}; font-weight:bold;">{align_text}</td>'
                tbl += '</tr>'
            tbl += '</table></div>'
            return tbl

        # Consensus targets as 3rd attendees (escape scenario)
        if p3_targets:
            html += render_3rd_table(
                p3_targets,
                "🎯 Se um alvo atendesse o Big Fone",
                "Escapariam da indicação e entrariam no consenso. Score de cada um em relação aos outros alvos."
            )

        # All other participants
        html += render_3rd_table(
            p3_others,
            "📋 Todos os possíveis 3º atendentes",
            "✓✓ = muito alinhado (score &lt; -1) · ✓ = alinhado (score &lt; 0) · ✗ = diverge (score ≥ 0) · ✗✗ = muito divergente (score ≥ 1)"
        )

        # --- Key findings callout ---
        html += '<div style="background:#1a2a1a; border:1px solid #2ecc71; border-radius:10px; padding:1rem 1.2rem; margin-top:1rem;">'
        html += '<h4 style="color:#2ecc71; margin-top:0;">💡 Destaques da Análise</h4>'
        html += '<ul style="color:#ccc; font-size:0.85em; margin:0; padding-left:1.2em; line-height:1.6;">'
        html += '<li><strong style="color:#e74c3c;">Sarah como 3ª atendente</strong>: seria a maior disruptora entre os alvos — discorda de TODOS os 4 outros alvos (scores positivos: +0.97 a +1.55). Dá ❤️ a todos que o grupo quer indicar. Escaparia da indicação E travaria o consenso.</li>'
        html += '<li><strong style="color:#f39c12;">Brígido</strong> é o único alvo de consenso que se alinharia parcialmente — concordaria em indicar Sol Vega (−2.42), mas protegeria Jonas, Gabriela e Sarah.</li>'
        html += '<li><strong style="color:#e74c3c;">Alberto Cowboy</strong> é o maior disruptor entre os não-alvos: discorda de todos os 5 alvos de consenso (dá ❤️ a todos).</li>'
        html += '<li><strong style="color:#2ecc71;">Ana Paula, Juliano e Milena</strong> são facilitadores perfeitos: scores negativos contra todos os 5 alvos.</li>'
        html += '</ul></div>'
...
def make_event_chips(events, border_color, color_lookup=None):
    ...
    for ev in events:
        ...
        chips.append(
            f"<span style='display:inline-flex; align-items:center; gap:0.35rem; background:#2f2f2f; "
            f"border:1px solid {chip_border}; color:#ddd; border-radius:12px; padding:0.25rem 0.6rem; "
            f"font-size:0.8rem; margin:0.15rem; min-height: 36px; line-height: 1;' title='{title}'>"
...
for p in profiles:
    ...
    if votes:
        ...
        for v in votes:
            ...
            count_badge = f'<span style="background:#555;color:#fff;border-radius:50%;font-size:0.65rem;padding:1px 5px;margin-left:0.3rem;">{count}×</span>' if count > 1 else ""
            border = relation_colors.get(voter, "#666")
            reveal_badge = ' <span style="color:#17a2b8;font-size:0.75rem;">👁️</span>' if is_revealed else ""
    ...
    if allies:
        relations_html += (
            ...
            f'<div style="color:#6c757d;font-size:0.75rem;margin-top:0.25rem;">Score positivo mútuo. Votos seguros.</div>'
            f'</div>'
        )
    if enemies:
        ...
        relations_html += (
            ...
            f'<div style="color:#6c757d;font-size:0.75rem;">Score negativo mútuo. Votos contra esperados.</div>'
            f'</div>'
        )
    if false_friends:
        ff_emoji_text = ", ".join(
        ...
        relations_html += (
            ...
            f'<div style="color:#6c757d;font-size:0.75rem;">Seu score é positivo, o deles negativo. Votam contra 2-3×.</div>'
            f'</div>'
        )
    if blind_targets:
        bt_emoji_text = ", ".join(
        ...
        relations_html += (
            ...
            f'<div style="color:#6c757d;font-size:0.75rem;">Seu score é negativo, o deles positivo. Não sabem.</div>'
            f'</div>'
        )
    ...
    if has_dynamics:
        ...
        if events.get('pos_week'):
            dynamics_inner += (
                ...
                f"<strong style='color:#28a745;font-size:0.85rem;'>+ Benefícios</strong> {pos_html}</div>"
            )
        if events.get('neg_week'):
            dynamics_inner += (
                f"<div style='display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;margin-bottom:0.4rem;"
                f"background:rgba(220,53,69,0.06);padding:0.4rem 0.6rem;border-radius:8px;'>"
                f"<strong style='color:#dc3545;font-size:0.85rem;'>− Prejuízos</strong> {neg_html}</div>"
            )
        if votes:
            dynamics_inner += (
                f"<div style='margin-bottom:0.4rem;'>"
                f"<strong style='color:#ffc107;font-size:0.85rem;display:block;margin-bottom:0.25rem;'>🗳️ Votos recebidos</strong>"
                f"<div style='display:flex;flex-direction:column;gap:0.1rem;'>{votes_html}</div></div>"
            )
        if sincerao_html:
            dynamics_inner += (
                f"<div style='display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;margin-bottom:0.4rem;'>"
                f"<strong style='color:#17a2b8;font-size:0.85rem;'>🔥 Sincerão</strong> {sincerao_html}</div>"
            )
        if events.get('pos_hist') or events.get('neg_hist'):
            dynamics_inner += (
                f"<div style='display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;'>"
                f"<strong style='color:#999;font-size:0.85rem;'>📜 Histórico</strong> {hist_html}</div>"
            )
        dynamics_html = (
            f'<div style="padding:0.6rem 1rem;border-bottom:1px solid #444;background:#2a2a2a;">'
            f'<div style="font-weight:700;font-size:0.85rem;color:#ccc;margin-bottom:0.4rem;">⚡ Dinâmicas do Jogo</div>'
    ...
    if curiosities:
        ...
        curiosities_html = (
            ...
            f'<div style="font-weight:700;font-size:0.85rem;color:#f1c40f;margin-bottom:0.3rem;">💡 Curiosidades</div>'
            f'{items_html}</div>'
        )
...
<div style="font-weight:600;color:#888;font-size:0.75rem;margin-bottom:0.3rem;">RECEBEU <span class="rxn-hint" style="font-weight:400;color:#666;font-size:0.65rem;">▶ clique p/ detalhes</span></div>
{rxn_received_chips}
</summary>
...
<details class="rxn-detail">
<summary style="cursor:pointer;list-style:none;">
<div style="font-weight:600;color:#888;font-size:0.75rem;margin-bottom:0.3rem;">DEU <span class="rxn-hint" style="font-weight:400;color:#666;font-size:0.65rem;">▶ clique p/ detalhes</span></div>
{rxn_given_chips}
</summary>
...
{relations_html if relations_html else '<div style="color:#777;font-size:0.85rem;">Sem relações classificadas.</div>'}
</div>
...
<h3 style="color:#999;font-size:1.1rem;margin-bottom:0.8rem;">🖤 Participantes que saíram</h3>
{cards_html}
</div>''')
```
