...
subtitle: "Análise das reações entre os participantes do Big Brother Brasil 2026"
lang: pt-BR
format:
  html:
    code-tools: false
...
# Reações mudam 1x/dia (no Raio-X da manhã); coletas extras no mesmo dia
# têm reações idênticas mas podem diferir em saldo/papéis.
_by_date = {}
...
def find_sincerao_reason(agg, name):
    if not agg:
        return []
...
if len(daily_snapshots) >= 2:
    ...
    for pair, old_rxn, new_rxn in changes:
        ...
        if old_pos and new_strong_neg:
            ...
            dramatic_changes.append((giver, receiver, '❤️', emoji_new, 'heart_to_snake'))
        elif old_strong_neg and new_pos:
            emoji_old = REACTION_EMOJI.get(old_rxn, '?')
            dramatic_changes.append((giver, receiver, emoji_old, '❤️', 'snake_to_heart'))
...
# 5. Paredão status (from latest snapshot)
paredao_check = []
...
Primeiro dia de dados — sem comparações disponíveis ainda.
:::
''')

# Compact callout: Sincerão contradictions (negativo + ❤️)
contrad = []
current_week = get_week_number(latest['date'])
for edge in sincerao_data.get("edges", []):
    ...
    if latest_matrix.get((actor, target), "") == "Coração":
        tema = edge.get("tema")
        label = "não ganha" if edge.get("type") == "nao_ganha" else (tema or "bomba")
        contrad.append(f"{actor} → {target} ({label})")

seen = set()
contrad = [c for c in contrad if not (c in seen or seen.add(c))]

if contrad:
    top_list = contrad[:5]
    print(f'''
::: {{.callout-note title="Contradições do Sincerão (negativo + ❤️)" appearance="simple"}}
''')
...
# ── Calculate hostility metrics for "game temperature" ──
n_two_sided = 0      # Mutual enemies (both hate each other)
n_one_sided = 0      # False friends (A hates B, but B gives ❤️)
blind_spot_victims = set()   # People who give ❤️ to enemies
...
for (a, b), rxn_a_to_b in latest_matrix.items():
    ...
    if a_dislikes_b and b_dislikes_a:
        ...
        blind_spot_victims.add(b)  # B gives ❤️ to an enemy
...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">❤️ Corações</div>
</div>
...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">⚔️ Inimigos</div>
<div style="font-size: 0.65rem; color: rgba(255,255,255,0.6); margin-top: 0.2rem;">pares mútuos</div>
</div>
</a>
...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">🎭 Falsos Amigos</div>
<div style="font-size: 0.65rem; color: rgba(255,255,255,0.6); margin-top: 0.2rem;">A detesta B, B dá ❤️</div>
</div>
</a>
...
<div style="font-size: 0.75rem; color: rgba(255,255,255,0.85); text-transform: uppercase;">👁️ Pontos Cegos</div>
<div style="font-size: 0.65rem; color: rgba(255,255,255,0.6); margin-top: 0.2rem;">dão ❤️ a inimigos</div>
</div>
</a>

</div>

<p class="text-muted small" style="margin-top: 0.3rem; text-align: center;">📅 Dados de <strong>{date_display}</strong> — {n_daily} dias de histórico</p>
''')
```
...
# Paredão Status Card - shows current paredão state
paredao_names = []
for p in latest['participants']:
    roles = parse_roles(p.get('characteristics', {}).get('roles', []))
    if 'Paredão' in roles:
...
# ══════════════════════════════════════════════════════════════════════════════
# WATCHLIST DE RISCO — Top 5 most vulnerable participants
# ══════════════════════════════════════════════════════════════════════════════

from collections import Counter

def get_vulnerability_data(matrix, active_names):
    """Calculate who gives ❤️ to enemies and who attacks friends."""
    loving_victim = Counter()  # Who gives ❤️ to people who hate them
    hostile_giver = Counter()  # Who attacks people who love them
    blind_spots = {}           # {victim: [list of attackers who receive ❤️]}
...
# Calculate vulnerability scores: ❤️ given to enemies / (attacks on friends + 1)
...
Participantes com **pontos cegos** — dão ❤️ para quem os detesta. Podem ser surpreendidos em votações.
...
Dá ❤️ para <strong style="color: #e74c3c;">{p['hearts_to_enemies']}</strong> {('pessoa que o detesta' if p['hearts_to_enemies'] == 1 else 'pessoas que o detestam')}
</div>
<div style="color: #888; font-size: 0.75rem; margin-top: 0.3rem;">
👁️ Pontos cegos: <span style="color: #ccc;">{attackers_display}</span>
</div>
</div>
</div>
''')
    ...
    print('</div>')
    print(f'\n<p style="text-align: center; margin-top: 0.5rem;"><a href="trajetoria.html#hostilidades-dia" style="color: #6ea8fe; font-size: 0.85rem;">Ver análise completa →</a></p>')
else:
...
Cada participante recebe reações dos demais. O **score de sentimento** pondera: ❤️ = +1, reações leves (🌱💼🍪) = -0.5, reações fortes (🐍🎯🤮🤥💔) = -1.
...
def make_sentiment_change_table(today_scores, past_scores, period_label):
    ...
    if not past_scores:
        return f"<p class='text-muted'>Dados insuficientes para comparação de {period_label}.</p>"
...
<th style="padding: 0.6rem; text-align: center; color: #ccc;">Δ</th>
</tr>
</thead>
<tbody>
'''
...
<td style="padding: 0.5rem;"><span style="color: {group_color}; margin-right: 0.3rem;">●</span>{row['name']}</td>
...
if n_daily >= 2:
    ...
    print(f"<p class='text-muted small'>Mudanças de <strong>{yesterday_label}</strong> para <strong>{latest['label']}</strong></p>")
    print(make_sentiment_change_table(today_scores, yesterday_scores, "ontem"))
else:
    print("<p class='text-muted'>Dados insuficientes (apenas 1 dia disponível).</p>")
```
...
if n_daily >= 7:
    ...
    print(f"<p class='text-muted small'>Mudanças de <strong>{week_ago_label}</strong> para <strong>{latest['label']}</strong></p>")
    print(make_sentiment_change_table(today_scores, week_ago_scores, "7 dias"))
else:
    print(f"<p class='text-muted'>Dados insuficientes (apenas {n_daily} dias disponíveis).</p>")
```
...
if not df_timeline.empty and len(df_timeline['Data'].unique()) > 1:
    ...
    print("<p class='text-muted small' style='margin-top: 0.5rem;'>Top 3 e bottom 3 destacados por padrão. Clique na legenda para mostrar/esconder outros participantes.</p>")
else:
...
# Definir active_names (usado por outras seções abaixo)
...
Detalhamento estratégico de cada participante ativo, incluindo análise de hostilidades e vulnerabilidades no jogo.
...
ℹ️ Como calculamos Vulnerabilidade, Risco externo e Animosidade (clique para expandir)
</summary>
...
O badge avalia o risco de receber **votos inesperados** — de pessoas que o participante considera aliadas mas que, na verdade, são hostis ("falsos amigos"). Quem tem muitos inimigos declarados mas zero falsos amigos está em posição mais segura: sabe exatamente de onde vêm as ameaças.
...
**🧭 Risco externo (semana atual)**

Mede exposição real a votos/eventos negativos **na semana atual** (não carrega para a semana seguinte).  
Fórmula:
...
Faixas: 🟢 **MUITO BAIXO** (<1), 🟡 **BAIXO** (>=1), 🟠 **MÉDIO** (>=3), 🔴 **ALTO** (>=6).

**🔥 Animosidade (tendência)**
...
Faixas: 🟢 **MUITO BAIXA** (<1), 🟡 **BAIXA** (>=1), 🟠 **MÉDIA** (>=3), 🔴 **ALTA** (>=6).

**Categorias de Relação:**

- **✅ Aliados** — ❤️ mútuo. Votos seguros, não votarão contra.
- **⚔️ Inimigos Declarados** — Negatividade mútua. Votos contra esperados de ambos os lados.
- **⚠️ Falsos Amigos** — Você dá ❤️, mas eles dão negativa. **PERIGO:** podem votar contra você!
- **🗡️ Inimigos Não Declarados** — Você dá negativa, eles dão ❤️. Eles não sabem que são seus alvos.

**Novos sinais de jogo (dinâmicas públicas):**
- **Poderes recebidos** (benefícios e prejuízos) mostram acontecimentos **públicos** na casa que podem mudar alianças.
- **Votos da casa recebidos** mostram o que o **público** viu na formação; quando há **dedo-duro**, o voto aparece marcado.
- O painel agora separa **Risco social** (percepção) e **Risco externo** (votos/eventos negativos).
- **Risco externo é experimental** e será recalibrado semanalmente após indicações/contragolpes/votações.

</div>
</details>

## Sincerão vs Queridômetro
...
if not sinc_agg:
    print("<p class='text-muted'>Sem dados detalhados do Sincerão nesta semana.</p>")
else:
    scores = sinc_agg.get("scores", {})
    if not scores:
        print("<p class='text-muted'>Sincerão registrado, mas sem pontuações detalhadas.</p>")
    else:
        rows = []
        for p in latest['participants']:
            name = p['name']
        ...
        if not rows:
            print("<p class='text-muted'>Sem participantes com pontuação no Sincerão.</p>")
        else:
            df_sinc = pd.DataFrame(rows)
            max_sinc = max(df_sinc["sincerao_score"].abs().max(), 1e-6)
            max_sent = max(df_sinc["sentiment_score"].abs().max(), 1e-6)
            df_sinc["sinc_norm"] = df_sinc["sincerao_score"] / max_sinc
            df_sinc["sent_norm"] = df_sinc["sentiment_score"] / max_sent
            df_sinc["alignment"] = (1 - (df_sinc["sinc_norm"] - df_sinc["sent_norm"]).abs()).round(2)
            ...
            fig = px.scatter(
                ...
                title="Impacto do Sincerão (semana) vs Sentimento do Queridômetro",
                labels={
                    "sincerao_score": "Sincerão (peso agregado)",
                    "sentiment_score": "Sentimento (queridômetro)",
                    "grupo": "Grupo",
                },
            )
            ...
            df_align = df_sinc[["name", "sincerao_score", "sentiment_score", "alignment"]].sort_values("alignment")
            print("<h4 style='margin-top:1rem;'>Alinhamento Sincerão × Queridômetro</h4>")
            print(df_align.to_html(index=False, classes="table table-dark table-sm", border=0))

            # Contradições: Sincerão negativo vs ❤️ no queridômetro
            contrad = []
            for edge in sincerao_data.get("edges", []):
                if edge.get("week") != current_week:
                    continue
                ...
                if not actor or not target:
                    continue
                label = latest_matrix.get((actor, target), "")
                if label == "Coração":
                    contrad.append({"ator": actor, "alvo": target, "tipo": edge.get("type")})

            if contrad:
                df_c = pd.DataFrame(contrad).drop_duplicates()
                print("<h4 style='margin-top:1rem;'>Contradições do Sincerão (negativo + ❤️)</h4>")
                print(df_c.to_html(index=False, classes="table table-dark table-sm", border=0))
```
...
for p in sorted_active:
    ...
    for rxn in p.get('characteristics', {}).get('receivedReactions', []):
        emoji = REACTION_EMOJI.get(rxn['label'], rxn['label'])
        rxn_summary.append(f"{emoji}×{rxn['amount']}")

    # Reações que este participante deu
    given = {}
    ...
    given_summary = " | ".join(f"{e}×{c}" for e, c in sorted(given.items(), key=lambda x: -x[1]))

    # ══════════════════════════════════════════════════════════════════════════
    # ANÁLISE ESTRATÉGICA DE RELAÇÕES
    # ══════════════════════════════════════════════════════════════════════════

    allies = []           # ❤️ mútuo — aliados seguros
    enemies = []          # Neg mútua — inimigos declarados (votos previsíveis)
    false_friends = []    # Dou ❤️, recebo neg — PERIGO: podem me votar
    blind_targets = []    # Dou neg, recebo ❤️ — posso votar neles sem retaliação esperada
    ...
    sincerao_reasons = find_sincerao_reason(sinc_agg, name) if 'sinc_agg' in globals() else []
    bombs = {}
    for edge in sinc_edges_week:
    ...
    chips = []
    for reason in sincerao_reasons:
        chips.append(
            f"<span style='display:inline-flex; align-items:center; gap:0.3rem; background:#2f2f2f; "
...
<div style="display: grid; grid-template-columns: 1fr; gap: 0.4rem; font-size: 0.9em;">
  {f"<div style='display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;'><strong style='color: #28a745;'>+ Benefícios (semana):</strong> {pos_html}</div>" if pos_events else ""}
  {f"<div style='display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;'><strong style='color: #dc3545;'>− Prejuízos (semana):</strong> {neg_html}</div>" if neg_events else ""}
  {f"<div style='display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;'><strong style='color: #ffc107;'>🗳️ Votos recebidos:</strong> {votes_html}</div>" if vote_map else ""}
  ...
  {f"<div style='display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;'><strong style='color: #999;'>Histórico da temporada:</strong> {hist_html}</div>" if (pos_events_hist or neg_events_hist) else ""}
</div>
</div>
''' if (pos_events_hist or neg_events_hist or pos_events or neg_events or vote_map or sincerao_html) else ''}
...
<div style="color: #6c757d; font-size: 0.8em; margin-top: 0.3rem;">→ Votos seguros. Não votarão contra.</div>
</div>
''')

    # Enemies
    if enemies:
        enemy_items = [f"{n} <span style='color:#888;'>({me}↔{te})</span>" for n, me, te in sorted(enemies, key=lambda x: x[0])]
...
<div style="font-weight: bold; color: #dc3545; margin-bottom: 0.3rem;">⚔️ Inimigos Declarados ({n_enemies})</div>
<div style="color: #ccc; font-size: 0.9em;">{enemies_str}</div>
<div style="color: #6c757d; font-size: 0.8em; margin-top: 0.3rem;">→ Hostilidade mútua. Votos contra esperados de ambos os lados.</div>
</div>
''')
...
<div style="color: #dc3545; font-size: 0.8em; margin-top: 0.3rem;">→ {first_name} dá ❤️ mas recebe negatividade. Podem votar contra sem que {first_name} espere!</div>
</div>
''')
...
<div style="font-weight: bold; color: #6f42c1; margin-bottom: 0.3rem;">🗡️ Inimigos Não Declarados ({n_blind_targets})</div>
<div style="color: #ccc; font-size: 0.9em;">{bt_str}</div>
<div style="color: #6c757d; font-size: 0.8em; margin-top: 0.3rem;">→ Eles dão ❤️ para {first_name}, mas {first_name} é hostil. Eles não sabem que são alvos.</div>
</div>
''')
    ...
    if n_false_friends == 0 and n_enemies == 0:
        insights.append("🛡️ Posição confortável: sem inimigos e sem falsos amigos.")
    if n_false_friends >= 3:
        insights.append(f"🚨 <strong style='color:#dc3545;'>Posição vulnerável</strong>: {n_false_friends} pessoas que considera amigas são hostis.")
    elif n_false_friends >= 1:
        insights.append(f"⚡ Atenção: {n_false_friends} falso(s) amigo(s) pode(m) surpreender em votação.")
    if n_blind_targets >= 3:
        insights.append(f"🗡️ {n_blind_targets} inimigos não declarados: confiam em {first_name} sem saber que são alvos.")
    if n_enemies >= 3:
        insights.append(f"🔥 Polarizante: {n_enemies} inimigos declarados. Alvo provável em paredões.")
    if n_allies >= 10 and total_relations > 0:
        pct = n_allies * 100 // total_relations
        insights.append(f"💪 Base sólida: {n_allies} aliados ({pct}% das relações).")

    if insights:
        print(f'''
<div style="padding: 0.6rem; background: #383838; border-radius: 6px; margin-top: 0.5rem;">
<div style="font-weight: bold; color: #fff; margin-bottom: 0.4rem; font-size: 0.9em;">📊 Análise de Jogo</div>
...
<strong>💡 Sobre este painel:</strong> Este painel acompanha as reações do queridômetro do BBB 26. As reações <strong>mudam diariamente</strong> — os participantes podem trocar suas escolhas a cada Raio-X. Seções marcadas com 📸 mostram apenas o dia mais recente; seções marcadas com 📈 mostram a evolução ao longo do tempo. Acompanhamento desde 13 de janeiro de 2026.
</div>
