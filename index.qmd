...
- **Votos da casa recebidos** aparecem como **“voto secreto (revelado)”** — só são conhecidos após a formação.
...
for p in sorted_active:
    ...
    target_events = [ev for ev in power_events if ev.get('target') == name]
    ...
    vote_map = votes_received.get(name, {})
    if vote_map:
        vote_chips = []
        for voter, count in sorted(vote_map.items(), key=lambda x: (-x[1], x[0])):
            count_prefix = f"{count}x " if count > 1 else ""
            avatar_html = render_actor_avatars([voter], "#6c757d")
            vote_chips.append(
                f"<span style='display:inline-flex; align-items:center; gap:0.3rem; background:#2f2f2f; "
                f"border:1px solid #6c757d; color:#ddd; border-radius:999px; padding:0.2rem 0.45rem; "
                f"font-size:0.75rem; margin:0.15rem;'>"
                f"{count_prefix}{avatar_html}{voter}</span>"
            )
    ...
    votes_count = votes_total.get(name, 0)
    external_score = votes_count + (2 * len(neg_public))
    if 'Paredão' in roles:
        external_score += 2
...
<div style="display: grid; grid-template-columns: 1fr; gap: 0.4rem; font-size: 0.9em;">
  ...
  <div><strong style="color: #6c757d;">🗳️ Votos recebidos</strong> <span style="color:#888;">(secreto revelado)</span>: {votes_html}</div>
</div>
</div>
