...
def render_actor_avatars(actors, border_color="#666", color_lookup=None):
    if not actors:
        return ''
    parts = []
    for a in actors:
        ...
        if a in AVATARS and AVATARS[a]:
            parts.append(
                f'<img src="{AVATARS[a]}" style="width: 30px; height: 30px; border-radius: 50%; '
...
def make_event_chips(events, border_color, color_lookup=None):
    ...
    for (etype, actors), meta in grouped.items():
        ...
        actor_html = render_actor_avatars(actors, chip_border, color_lookup=color_lookup)
        ...
        chips.append(
            ...
            f"border:1px solid {chip_border}; color:#ddd; border-radius:999px; padding:0.25rem 0.55rem; "
...
for p in sorted_active:
    ...
    if vote_map:
        ...
        for voter, count in sorted(vote_map.items(), key=lambda x: (-x[1], x[0])):
            ...
            avatar_html = render_actor_avatars([voter], border, color_lookup=relation_colors)
            ...
            vote_chips.append(
                ...
                f"font-size:0.75rem; margin:0.15rem; {reveal_glow}' title='{title}'>"
...
<div style="display: grid; grid-template-columns: 1fr; gap: 0.4rem; font-size: 0.9em;">
  {f"<div><strong style='color: #28a745;'>+ Benefícios (semana):</strong> {pos_html}</div>" if pos_events else ""}
  {f"<div><strong style='color: #dc3545;'>− Prejuízos (semana):</strong> {neg_html}</div>" if neg_events else ""}
  {f"<div><strong style='color: #ffc107;'>🗳️ Votos recebidos:</strong> {votes_html}</div>" if vote_map else ""}
  {f"<div><strong style='color: #999;'>Histórico da temporada:</strong> {hist_html}</div>" if (pos_events_hist or neg_events_hist) else ""}
</div>
</div>
