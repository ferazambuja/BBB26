...
Heatmap mostrando apenas as células que mudaram. Verde = melhora na relação, vermelho = piora.
...
if n_daily >= 2 and changes_list:
    ...
    n_participants = len(active_names_sorted)
    name_to_idx = {n: i for i, n in enumerate(active_names_sorted)}

    diff_matrix = np.full((n_participants, n_participants), np.nan)
    text_matrix = [['—' for _ in range(n_participants)] for _ in range(n_participants)]

    for c in changes_list:
        i = name_to_idx[c['Emissor']]
        j = name_to_idx[c['Receptor']]
        diff_matrix[i, j] = c['Delta']
        text_matrix[i][j] = f"{c['Antes']}→{c['Depois']}"
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        y=short_names,
        colorscale=[
            [0, '#d73027'],      # -2: strong negative change
            [0.25, '#fc8d59'],   # -1
            [0.5, '#ffffbf'],    # 0 (won't show, but for scale)
            [0.75, '#91cf60'],   # +1
            [1.0, '#1a9850'],    # +2: strong positive change
        ],
        zmin=-2, zmax=2,
        text=text_matrix,
        texttemplate='%{text}',
        textfont=dict(size=14),
        hovertemplate='%{y} → %{x}<br>Δ: %{z:+.1f}<br>%{text}<extra></extra>',
        colorbar=dict(
            title="Δ Sentimento",
            tickvals=[-2, -1, 0, 1, 2],
            ticktext=['-2 (❤️→💔)', '-1', '0', '+1', '+2 (💔→❤️)']
        ),
        showscale=True,
    ))

    # Add star markers for dramatic changes (|Δ| >= 1.5)
    dramatic_annotations = []
    for c in changes_list:
        if abs(c['Delta']) >= 1.5:
            i = name_to_idx[c['Emissor']]
            j = name_to_idx[c['Receptor']]
            dramatic_annotations.append(dict(
                x=j, y=i,
                text='⭐',
                showarrow=False,
                font=dict(size=8),
    ...
    fig.update_layout(
        title=f"Mapa de Mudanças: {prev_date} → {curr_date}<br><sup>⭐ = mudança dramática (|Δ| ≥ 1.5)</sup>",
        xaxis_title="Receptor ←",
        yaxis_title="Emissor →",
        height=850,
        xaxis=dict(tickangle=45, side='bottom'),
        yaxis=dict(autorange='reversed'),
        annotations=dramatic_annotations
    )
