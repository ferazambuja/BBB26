...
# ══════════════════════════════════════════════════════════════
# Platform + Votalhada constants (single source of truth)
# SVG icons: Simple Icons (CC0 public domain)
...
PLATFORM_LABELS: dict[str, str] = {
    "sites": "Sites", "youtube": "YouTube", "twitter": "Twitter", "instagram": "Instagram",
}

PLATFORM_SHORT_LABELS: dict[str, str] = {
    "sites": "Sites", "youtube": "YT", "twitter": "TW", "instagram": "IG",
}
...
PLATFORM_ICON_LABELS: dict[str, str] = {
...
def platform_label(platform: str, *, variant: str = "text") -> str:
    """Return a platform display label in the requested variant.

    Variants: 'text' (plain), 'html' (SVG icon + text), 'short' (YT/TW/IG).
    Unknown keys fall through unchanged.
    """
    if variant == "html":
        return PLATFORM_ICON_LABELS.get(platform, platform)
    if variant == "short":
        return PLATFORM_SHORT_LABELS.get(platform, platform)
    return PLATFORM_LABELS.get(platform, platform)


def get_votalhada_source_url(poll: dict | None = None) -> str:
    """Return the best Votalhada URL for a poll entry.

    Scans poll['fontes'] (flat URL strings) for a votalhada/blogspot link.
    Handles fontes being None, missing, or containing only non-Votalhada URLs.
    """
    if not poll:
        return VOTALHADA_HOME
    fontes = poll.get("fontes") or []
    for url in fontes:
        if isinstance(url, str) and ("votalhada" in url or "blogspot" in url):
            return url
    return VOTALHADA_HOME


def render_votalhada_logo(
    *, href: str | None = None, size: str = "sm", extra_classes: str = "",
) -> str:
    """Return HTML for the Votalhada logo image, optionally wrapped in a link.

    The logo image already contains the word 'VOTALHADA' — never place
    adjacent text saying 'Votalhada'.
    """
    cls = "votalhada-logo--lg" if size == "lg" else "votalhada-logo"
    if extra_classes:
        cls = f"{cls} {extra_classes}"
    img = f'<img src="assets/votalhada-logo.png" alt="Votalhada" class="{cls}" />'
    if href:
        return f'<a href="{href}" target="_blank" rel="noopener">{img}</a>'
    return img
...
def build_precision_methodology_text(polls_data: dict) -> str:
    ...
    weight_parts = [f"{platform_label(p)} (RMSE {rmses.get(p, 0):.1f}) recebe {w:.0%}" for p, w in sorted_plats]
...
def group_cronologia_events(timeline_events: list[dict]) -> list[dict]:
    ...
    for week_num, dates_dict in sorted(weeks.items(), key=lambda item: item[0], reverse=True):
        ...
        for date_str, date_events in sorted(dates_dict.items(), key=lambda item: item[0], reverse=True):
            # Within a day: latest events on top (consistent with newest-first
            # display for weeks and dates). The timeline data is sorted
            # chronologically by CATEGORY_ORDER, so reversing puts the last
            # event of the day at the top.
            dates.append({
                "date": date_str,
