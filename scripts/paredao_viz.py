...
def render_nominee_cards_em_andamento(
    ...
    for p in participantes:
        ...
        if avatar_url:
            lines.append(f'<img src="{avatar_url}" alt="{nome}" class="nominee-avatar" style="border: 3px solid {cor_grupo};">')
        lines.append(f'<h4 class="tc" style="margin: 0; color: #fff; font-size: 1.1em;">{nome}</h4>')
        lines.append(f'<span class="group-badge" style="background: {cor_grupo};">{grupo}</span>')
        if como_indicado and not is_auto:
            lines.append(f'<div style="color: #aaa; font-size: 0.8em; margin-top: 0.5rem;">via {como_indicado}</div>')
        elif is_auto:
...
def render_nominee_cards_finalized(
    ...
    for row in df_rows:
        ...
        if avatar_url:
            lines.append(f'<img src="{avatar_url}" alt="{nome}" style="width: 100px; height: 100px; border-radius: 50%; object-fit: cover; border: 3px solid {border_color}; margin-bottom: 0.75rem; filter: {img_filter};">')
        lines.append(f'<h4 class="tc" style="margin: 0 0 0.25rem 0; color: #fff; font-size: 1em;">{nome}</h4>')
        lines.append(f'<span style="display: block; color: {cor_grupo}; font-size: 0.8em; margin-bottom: 0.5rem;">{grupo}</span>')
...
def render_voting_blocs(
    ...
    if len(ranking) == 2:
        lines.append(f'A casa se dividiu em dois blocos claros: **{blocs[0]["n"]}** votaram em **{blocs[0]["alvo"]}**, **{blocs[1]["n"]}** votaram em **{blocs[1]["alvo"]}**.\n')
    else:
    ...
    for bloc in blocs:
        ...
        lines.append(f'<div style="color:#fff; font-weight:bold; font-size:1.05em; margin-top:0.3rem;">{alvo}</div>')
        ...
        for vot in votantes:
            ...
            if av_url:
                lines.append(f'<img src="{av_url}" style="width:36px; height:36px; border-radius:50%; border:2px solid {vot_cor};" alt="{vot}" title="{vot}">')
            else:
                lines.append(f'<div style="width:36px; height:36px; border-radius:50%; background:{vot_cor}; display:flex; align-items:center; justify-content:center; font-size:0.7em; color:#fff;">{vot[:2]}</div>')
            lines.append(f'<div style="font-size:0.6em; color:#aaa; max-width:50px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{vot.split()[0]}</div>')
            lines.append(f'</div>')
        lines.append(f'</div>')
        lines.append(f'</div>')
    lines.append('</div>')
...
def render_votos_casa_table(
    ...
    ranking = contagem.most_common()
    lines: list[str] = []
    lines.append(f"**Mais votado:** **{ranking[0][0]}** ({ranking[0][1]} votos)\n")
...
def render_duelo_result_card(
    ...
    lines.append(f'<div class="tc">')
    if _av_s:
        lines.append(f'<img src="{_av_s}" style="width:72px; height:72px; border-radius:50%; border:3px solid #9b59b6;" alt="{sorteado}">')
    lines.append(f'<div style="color:#fff; font-weight:bold; margin-top:0.4rem;">{sorteado}</div>')
    ...
    lines.append(f'<div class="tc">')
    if _av_o:
        lines.append(f'<img src="{_av_o}" style="width:72px; height:72px; border-radius:50%; border:3px solid #e67e22;" alt="{oponente}">')
    lines.append(f'<div style="color:#fff; font-weight:bold; margin-top:0.4rem;">{oponente}</div>')
    ...
    lines.append(f'<div class="tc">')
    if _av_t:
        lines.append(f'<img src="{_av_t}" style="width:72px; height:72px; border-radius:50%; border:3px solid #e74c3c;" alt="{consenso_target}">')
    lines.append(f'<div style="color:#fff; font-weight:bold; margin-top:0.4rem;">{consenso_target}</div>')
    ...
    lines.append('</div>')
    if resultado_detail:
        lines.append(f'<p class="tc" style="color:#aaa; font-size:0.85em; margin-top:0.5rem;">{resultado_detail}</p>\n')
    return '\n'.join(lines)
...
def render_overlap_box(
    ...
    lines.append(f'<h5 style="color:{color}; margin-top:0;">{emoji} {title} ({len(items)})</h5>')
    ...
    for it in items:
        ...
        lines.append(f'<span style="background:rgba(0,0,0,0.3); padding:0.3rem 0.6rem; border-radius:8px; font-size:0.85em;">{_img_it}<strong>{it["name"]}</strong>{_mark} <span style="color:#888;">({it["bottleneck"]:+.2f})</span></span>')
...
def render_sincerinho_bar_chart(
    ...
    for name in sorted_names:
        ...
        lines.append(f'<div style="width:180px; display:flex; align-items:center; gap:6px;">{av} <span style="font-size:0.85rem;">{name}</span></div>')
    ...
    if not_nominated:
        not_html = " ".join([f'{avatar_img(n, avatars, 32)} {n}' for n in not_nominated])
...
def render_lider_prediction(
    ...
    html += f'<span style="font-size:1.1em; color:#eee;"><strong>{lider_name}</strong> '
    ...
    for _i, (_tname, _tentry) in enumerate(_top3):
        ...
        html += f'<strong style="color:#eee;">{_tname}</strong>{_vip_tag}</div>'
    ...
    # Anjo note
    if anjo_name:
        html += f'<div style="color:#3498db; font-size:0.9em;">😇 Anjo: <strong>{anjo_name}</strong>'
        if imunizado_nome:
            html += f' — imunizou <strong>{imunizado_nome}</strong> (bloqueado como alvo)'
        html += '</div>'
    else:
    ...
    for _rank, (_tname, _tentry) in enumerate(ranked, 1):
        ...
        html += f'<strong style="color:#eee;">{_tname}</strong>{_immune_tag}</td>'
        ...
        if _pair_edges or _hist_fwd:
            ...
            if _pair_edges:
                ...
                for _edge in _pair_edges:
                    ...
                    _arrow = f'{_e_actor.split()[0]} → {(_tname if _e_actor == lider_name else lider_name).split()[0]}'
                    _detail_parts = []
            ...
            if _hist_fwd:
                ...
                html += f'<th class="text-dim tc" style="padding:2px 4px;">{lider_name.split()[0]}→</th>'
                html += f'<th class="text-dim tc" style="padding:2px 4px;">→{lider_name.split()[0]}</th>'
                html += '</tr></thead><tbody>'
