...
if len(finalized_polls) >= 1:
    ...
    if backtest:
        ...
        print(f'<div style="min-width:150px; border-left:3px solid #00bc8c; padding-left:1rem;"><strong>🎯 Modelo Ponderado</strong><br><span style="font-size:2em; color:{_bt_color}; font-weight:bold;">{_bt_agg["model_mae"]:.1f}</span><br><span style="color:#888;">p.p. (LOO) — {_bt_imp:+.0f}%</span></div>')
        ...
        if model_errs:
            ...
            for r in _bt_results:
                if r['model_mae'] is not None:
                    ...
                    print(f'<td style="text-align:center;"><span style="color:{err_color};">{r["model_mae"]:.1f}</span></td>')
                else:
                    print('<td style="text-align:center;">—</td>')
            avg_model = np.mean(model_errs)
            print(f'<td style="text-align:center; font-weight:bold;">{avg_model:.1f}</td>')
            ...
            print(f'<td style="text-align:center;">{medal_m}</td>')
            print('</tr>')
    ...
    if backtest and len(backtest['per_paredao']) >= 3:
        ...
        print('<th style="text-align:center;">Consolidado (erro)</th>')
        print('<th style="text-align:center;">Modelo LOO (erro)</th>')
        print('<th style="text-align:center;">Melhoria</th>')
        ...
        for r in backtest['per_paredao']:
            ...
            if m_err is not None:
                ...
                print(f'<td style="text-align:center; color:{c_color};">{c_err:.1f} p.p.</td>')
                print(f'<td style="text-align:center; color:{m_color};">{m_err:.1f} p.p.</td>')
                print(f'<td style="text-align:center; color:{diff_color}; font-weight:bold;">{diff_str} p.p.</td>')
                print('</tr>')

        # Summary row
        print(f'<tr style="background:rgba(0,188,140,0.15);"><td colspan="2"><strong>Média</strong></td>')
        print(f'<td style="text-align:center; font-weight:bold;">{_bt_agg["consolidado_mae"]:.1f} p.p.</td>')
        print(f'<td style="text-align:center; font-weight:bold;">{_bt_agg["model_mae"]:.1f} p.p.</td>')
        _imp = _bt_agg.get('improvement_pct', 0)
        print(f'<td style="text-align:center; font-weight:bold; color:#00bc8c;">{_imp:+.0f}%</td>')
        print('</tr>')
        print('</tbody></table>')
        ...
        print(f'<br><span style="color:#888;">Acerto: Consolidado {_bt_agg["consolidado_correct"]}/{_bt_agg["n_paredoes"]} · Modelo {_bt_agg["model_correct"]}/{_bt_agg["n_paredoes"]}</span>')
        print(f'</div>\n')
...
for name in never_voted:
    ...
    print(f'<div style="text-align:center; width:90px;">')
    if url:
    ...
    if exemptions:
        ...
        print(f'<div style="font-size:0.65em; color:#888; margin-top:1px;">elegível {n_eligible}/{len(paredoes_finalizados)}</div>')
    else:
...
if target_votes:
    ...
    print('<thead><tr>')
    print('<th>Alvo</th><th style="text-align:center;">Total</th>')
    for p in paredoes_finalizados:
        print(f'<th style="text-align:center;">{p["numero"]}º</th>')
    ...
    for target, by_paredao in ranked:
        ...
        print(f'<td style="text-align:center; font-weight:bold; font-size:1.1em;">{total}</td>')

        for p in paredoes_finalizados:
            n = len(by_paredao.get(p['numero'], []))
            if n > 0:
                color = '#e74c3c' if n >= 5 else '#f39c12' if n >= 3 else '#aaa'
                print(f'<td style="text-align:center; font-weight:bold; color:{color};">{n}</td>')
            else:
                print('<td style="text-align:center; color:#555;">—</td>')
            ...
            if n_votes > 0:
                ...
                for v in votes_for:
                    ...
                    if has_vp:
                        ...
                        if not pred:
                            vp_cell = '<td style="text-align:center; color:#666;">—</td>'
                        elif pred.get('predicted') == nome:
                            conf = pred.get('confidence', '')
                            vp_cell = f'<td style="text-align:center; color:#3CB44B; font-size:0.8em;">✅ {conf}</td>'
                        else:
                            short = pred.get('predicted', '?').split()[0]
                            vp_cell = f'<td style="text-align:center; color:#E6194B; font-size:0.8em;">❌→{short}</td>'
