...
from data_utils import load_snapshot, get_all_snapshots, build_reaction_matrix
...
Cada ponto é um participante. No eixo horizontal: quantas reações negativas (🐍🎯🤮🤥💔🌱💼🍪)
essa pessoa **recebeu** dos colegas no queridômetro. No eixo vertical: quantos **votos**
essa pessoa recebeu na votação da casa.

A **correlação (r)** mede se existe relação entre ser odiado e ser votado:
- **r = 1**: perfeita relação (todos votam em quem odeiam)
- **r = 0**: nenhuma relação (votos são estratégicos)
- **r negativo**: relação inversa (protegem quem odeiam)
...
<strong>O que este gráfico mostra?</strong> Cada ponto representa um participante.
A posição horizontal mostra quantas reações negativas essa pessoa <strong>recebeu</strong>.
A posição vertical mostra quantos <strong>votos</strong> recebeu na votação da casa.
Se os participantes votassem de acordo com o que registraram no queridômetro, veríamos uma diagonal:
...
""")
            ...
            if abs(corr_val) < 0.3:
                print(f'<p style="font-size: 1.05rem;"><strong>Correlação fraca (r = {corr_val:.2f})</strong> — Votos <strong>não seguem</strong> o queridômetro.</p>')
                print('<p style="font-size: 1.05rem;">Os participantes votaram de forma <strong>estratégica</strong>, não de acordo com suas reações. '
                      'Ser odiado no queridômetro não significou ser votado. '
                      'Alianças, acordos e estratégia de jogo pesaram mais que as reações registradas.</p>\n')
            elif corr_val > 0.5:
                print(f'<p style="font-size: 1.05rem;"><strong>Correlação forte (r = {corr_val:.2f})</strong> — Votos <strong>seguem</strong> o queridômetro.</p>')
                print('<p style="font-size: 1.05rem;">Os participantes votaram de acordo com o que registraram no queridômetro. '
                      'As relações pessoais dominaram a votação.</p>\n')
            else:
                print(f'<p style="font-size: 1.05rem;"><strong>Correlação moderada (r = {corr_val:.2f})</strong> — Mistura de queridômetro e estratégia.</p>')
                print('<p style="font-size: 1.05rem;">Houve alguma relação entre ser odiado e ser votado, mas não foi determinante. '
                      'Alguns votos seguiram as reações, outros seguiram estratégia.</p>\n')

            # Poupados Suspeitos table
            poupados = high_neg_low_votes.sort_values('neg_recebidas', ascending=False).head(5)
            if len(poupados) > 0:
                print("\n### 🛡️ Poupados Suspeitos\n")
                print('<p style="font-size: 1.05rem;">Participantes com muitas reações negativas mas poucos votos — podem ter sido protegidos estrategicamente.</p>\n')
            ...
            if len(alvos) > 0:
                ...
                print('<p style="font-size: 1.05rem;">Participantes com poucas reações negativas mas muitos votos — foram votados por estratégia, não por antipatia.</p>\n')
                print('<table style="font-size: 1.05rem; width: 100%;">')
                print('<thead><tr><th>Participante</th><th style="text-align:center;">Negativas</th><th style="text-align:center;">Votos</th></tr></thead>')
                print('<tbody>')
            ...
            if most_hated is not None and most_voted_name:
                ...
                if most_hated['nome'] == most_voted_name:
                    print(f'<p style="font-size: 1.05rem;">✅ <strong>Alinhado:</strong> <strong>{most_hated["nome"]}</strong> foi quem recebeu mais reações negativas '
                    ...
                    print(f'<p style="font-size: 1.05rem;">⚠️ <strong>Desalinhado:</strong> O mais votado foi <strong>{most_voted_name}</strong> '
            ...
            if nao_votaveis:
                ...
                if nao_votaveis_list:
                    ...
                    print(f'<p style="font-size: 0.95rem; color: #888;"><em>Nota: {nao_votaveis_str} não podia(m) receber votos neste paredão (líder, imunizado ou anjo autoimune).</em></p>\n')
    ...
    if paredao.get('votos_casa'):
        ...
        fig.update_layout(
            title=f"Coerência: Reação vs Voto — {par_num}º Paredão<br><sup>Total: {total_votes} votos</sup>",
            height=250,
            xaxis_title="Número de votantes",
            yaxis_title="",
            margin=dict(l=200),
            barmode='stack'
        )
