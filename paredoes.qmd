...
for i, paredao in enumerate(reversed(paredoes_finalizados)):  # Most recent first
    ...
    if votos and matrix_p:
        ...
        for votante, alvo in votos.items():
            ...
            # Prefer precomputed relationship history from JSON
            _pa_rh = PAREDAO_ANALYSIS.get("by_paredao", {}).get(str(par_num), {}).get("relationship_history", {})
            _rh_key = f"{votante}→{alvo}"
            if _rh_key in _pa_rh:
                hist = _pa_rh[_rh_key]
            else:
