...
# ── Vote target table (active participants only) ─────────────────────────────
active_targets = {t: votes for t, votes in target_votes.items() if t in active_names}

if active_targets:
    # Sort by total votes descending
    ranked = sorted(active_targets.items(), key=lambda x: sum(len(v) for v in x[1].values()), reverse=True)

    print('<h4 style="margin:1.5rem 0 0.8rem 0;">🗳️ Alvos ativos — Quem votou em quem</h4>')
    print('<p style="color:#aaa; font-size:0.9em; margin-bottom:0.8rem;">Apenas participantes ativos. Votos de participantes que já saíram são contabilizados.</p>')
    ...
    for target, by_paredao in ranked:
        ...
        print(f'<tr><td style="white-space:nowrap;">{avatar_html(target, AVATARS, 36)}</td>')
        ...
        voters_html = ' '.join([avatar_img(v, AVATARS, 30) for v in voters_sorted])
