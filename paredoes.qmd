...
def classify_relationship(votante, alvo, matrix):
    """Classifica o relacionamento entre votante e alvo no queridômetro."""
...
for i, paredao in enumerate(reversed(paredoes_finalizados)):  # Most recent first
    ...
    # ══════════════════════════════════════════════════════════════
    # ANÁLISE ESTRATÉGICA DE VOTOS — Categorias de Relacionamento
    # (transferred from paredao.qmd)
    # ══════════════════════════════════════════════════════════════

    votos = paredao.get('votos_casa', {})
    # Load precomputed vote analysis from paredao_analysis.json
    _pa_entry = PAREDAO_ANALYSIS.get("by_paredao", {}).get(str(par_num), {})
    _precomputed_va = _pa_entry.get("vote_analysis", [])
    _precomputed_agg = _pa_entry.get("vote_aggregates", {})

    if votos and _precomputed_va:
        # Use fully precomputed data
        vote_analysis = _precomputed_va
        relationship_counts = Counter(_precomputed_agg.get("relationship_counts", {}))
        betrayals = [e for e in vote_analysis if e["tipo"] in ("falso_amigo", "aliados_mutuos")]
        blind_spots = [e for e in vote_analysis if e["tipo"] == "ponto_cego"]
        total_votes = _precomputed_agg.get("total_votes", len(votos))
        n_traicoes = _precomputed_agg.get("n_traicoes", len(betrayals))
        n_pontos_cegos = _precomputed_agg.get("n_pontos_cegos", len(blind_spots))
        vote_counts = Counter(_precomputed_agg.get("vote_counts", {}))
        mais_votado = _precomputed_agg.get("mais_votado", "")
        n_votos_mais = _precomputed_agg.get("n_votos_mais", 0)
        n_esperados = _precomputed_agg.get("n_esperados", 0)
        ...
        rel_labels = {
            'inimigos_declarados': '⚔️ Hostilidade Mútua', 'hostilidade_forte': '🐍 Hostilidade Forte',
            'hostilidade_leve': '🌱 Hostilidade Leve', 'ponto_cego': '🎯 Ponto Cego',
            'falso_amigo': '🎭 Falso Amigo', 'aliados_mutuos': '💔 Traição de Aliado', 'neutro': '❓ Neutro'
        }
        rel_colors = {
        ...
        fig_rel.update_layout(
            title=f"Distribuição dos Votos por Tipo de Relacionamento<br><sup>Total: {total_votes} votos</sup>",
        ...
        print(f"\n✅ **{coerentes} votos coerentes** — votaram em quem davam negativa")
        print(f"<br>❌ **{incoerentes} votos incoerentes** — votaram em quem davam ❤️")
        print(f"<br>🎯 **{n_pontos_cegos} pontos cegos** — alvos confiavam no votante\n")

        # ── Betrayals table ──
        if betrayals:
            print("#### 🎭 As Traições\n")
            print(f"**{n_traicoes} voto(s)** de pessoas que davam ❤️ ao alvo.\n")
            print('<table class="table table-striped" style="font-size: 0.95rem;">')
            print('<thead><tr><th>Votante</th><th>Alvo</th><th style="text-align:center;">Deu</th><th>Mútuo?</th></tr></thead><tbody>')
            for b in betrayals:
                reciproca = f"Sim, {b['emoji_recebido']}" if b['tipo'] == 'aliados_mutuos' else "Não"
                print(f'<tr><td>{avatar_html(b["votante"], AVATARS, 28)}</td><td>{avatar_html(b["alvo"], AVATARS, 28)}</td>'
        ...
        if blind_spots:
            ...
            print(f"**{n_pontos_cegos} voto(s)** de inimigos disfarçados — a vítima dava ❤️ sem saber que era odiada.\n")
            print('<table class="table table-striped" style="font-size: 0.95rem;">')
            print('<thead><tr><th>Vítima</th><th>Votante</th><th style="text-align:center;">Vítima dava</th><th style="text-align:center;">Votante dava</th></tr></thead><tbody>')
            for bs in blind_spots:
                print(f'<tr><td>{avatar_html(bs["alvo"], AVATARS, 28)}</td><td>{avatar_html(bs["votante"], AVATARS, 28)}</td>'
        ...
        if has_temporal and n_daily >= 3:
            ...
            if recem_inimigos_v:
                print(f"**🚨 Mudança de última hora** — {len(recem_inimigos_v)} voto(s) de quem mudou nos últimos 2 dias:\n")
                for v in recem_inimigos_v:
                    print(f"- {avatar_html(v['votante'], AVATARS, 22)} → {avatar_html(v['alvo'], AVATARS, 22)}: {v['hist_narrative']}")
                print()

            if ex_amigos_v:
                print(f"**💔 Ex-amigos** — {len(ex_amigos_v)} voto(s) de quem já deu ❤️:\n")
            ...
            if always_friends_voted:
                print(f"**🗡️ Traição de longa data** — {len(always_friends_voted)} voto(s) contra quem sempre recebeu ❤️:\n")
                for v in always_friends_voted:
                    print(f"- {avatar_html(v['votante'], AVATARS, 22)} → {avatar_html(v['alvo'], AVATARS, 22)}: {v['days_as_friends']} dias de ❤️")
                print()

            # Temporal summary table
            pattern_counts = Counter([v['hist_pattern'] for v in vote_analysis])
            pattern_labels = {
                'sempre_amigos': ('💚 Sempre amigos', 'Sempre deu ❤️ — voto inesperado'),
                'sempre_inimigos': ('💀 Sempre inimigos', 'Nunca deu ❤️ — voto esperado'),
                'ex_amigos': ('💔 Ex-amigos', 'Davam ❤️, hoje não mais'),
                'recem_inimigos': ('⚡ Mudança recente', 'Mudou nos últimos 2 dias'),
                'instavel': ('🎭 Instáveis', 'Relação oscilou muito'),
            }
            print("| Padrão | Votos | Significado |")
            print("|--------|-------|-------------|")
        ...
        print(f"| Reações negativas recebidas | {neg_count_by_name.get(mais_votado, 0)} |")
        ...
        if mv_from_traitors > n_votos_mais / 2:
            ...
            print(f"> 💔 **{mais_votado} foi {apunhalado} pelas costas.** "
                  f"Mais da metade dos votos ({mv_from_traitors}/{n_votos_mais}) veio de quem dava ❤️.\n")
        elif mv_from_enemies == n_votos_mais:
            print(f"> ⚔️ **{mais_votado} era um alvo óbvio.** "
                  f"Todos os {n_votos_mais} votos vieram de quem já dava negativa.\n")
        elif mv_from_blind >= 2:
            print(f"> 🎯 **{mais_votado} não percebia seus inimigos.** "
                  f"Dava ❤️ para {mv_from_blind} pessoas que {'a' if gen_mv == 'f' else 'o'} detestavam.\n")

        # ── Leader nomination analysis ──
        ...
        if lider and indicado:
            ...
            print('<thead><tr><th>De</th><th>Para</th><th style="text-align:center;">Reação</th></tr></thead><tbody>')
            print(f'<tr><td>{avatar_html(lider, AVATARS, 28)}</td><td>{avatar_html(indicado, AVATARS, 28)}</td><td style="text-align:center; font-size:1.2em;">{emoji_lider}</td></tr>')
            print(f'<tr><td>{avatar_html(indicado, AVATARS, 28)}</td><td>{avatar_html(lider, AVATARS, 28)}</td><td style="text-align:center; font-size:1.2em;">{emoji_indicado}</td></tr>')
            print('</tbody></table>\n')

            if rel_type_l == 'aliados_mutuos':
                print(f"> 💔 **Indicação surpreendente!** Trocavam ❤️ mútuos.\n")
            elif rel_type_l == 'falso_amigo':
                print(f"> 🎭 **Indicação contraditória.** {lider} dava ❤️ para {indicado}.\n")
            elif rel_type_l == 'inimigos_declarados':
                print(f"> ⚔️ **Indicação coerente.** Hostilidade mútua no queridômetro.\n")
            elif rel_type_l == 'ponto_cego':
                print(f"> 🎯 **{indicado} não viu chegando.** Dava ❤️ para {lider}.\n")

        # ── Full vote table ──
        print("#### 📋 Tabela Completa de Votos\n")
        pattern_emoji = {
            'sempre_amigos': '💚', 'sempre_inimigos': '💀', 'ex_amigos': '💔',
            'recem_inimigos': '⚡', 'reconciliados': '🤝', 'instavel': '🎭', 'sem_dados': '❓',
        }
        ...
        print('<thead><tr><th>Votante</th><th>Votou em</th><th style="text-align:center;">Deu</th>'
              '<th style="text-align:center;">Recebeu</th><th>Tipo</th><th>Histórico</th></tr></thead><tbody>')
        for v in vote_analysis:
            hist_str = f"{pattern_emoji.get(v['hist_pattern'], '?')} {v['days_as_friends']}d❤️"
            print(f'<tr><td>{avatar_html(v["votante"], AVATARS, 28)}</td><td>{avatar_html(v["alvo"], AVATARS, 28)}</td>'
