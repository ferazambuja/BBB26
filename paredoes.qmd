...
if len(finalized_polls) >= 1:
    ...
    if backtest:
        ...
        print(f'<div style="min-width:150px; border-left:3px solid #00bc8c; padding-left:1rem;"><strong>🎯 Modelo Ponderado</strong><br><span style="font-size:2em; color:{_bt_color}; font-weight:bold;">{_bt_agg["model_mae"]:.1f}</span><br><span class="text-muted">p.p. (LOO) — {_bt_imp:+.0f}%</span></div>')
    ...
    if backtest:
        ...
        if model_errs:
            print(f'<tr style="background:rgba(0,188,140,0.2);"><td><strong>🎯 Modelo</strong></td>')
    ...
    print('</tbody></table>')
    print('<p class="text-muted small">Erro = diferença em pontos percentuais entre previsão e resultado real (Média Final). Modelo usa leave-one-out cross-validation.</p>\n')
    ...
    if len(finalized_polls) >= 2:
        ...
        if backtest:
            ...
            fig_lines.add_trace(go.Scatter(
                ...
                hovertemplate='Modelo: %{y:.1f} p.p.<extra></extra>',
                connectgaps=True,
            ))
    ...
    if backtest and len(backtest['per_paredao']) >= 3:
        ...
        print('<th class="tc">Consolidado (erro)</th>')
        print('<th class="tc">Modelo LOO (erro)</th>')
        ...
        print(f'<strong>🎯 Resultado:</strong> O modelo ponderado reduz o erro médio de <strong>{_bt_agg["consolidado_mae"]:.1f}</strong> para <strong>{_bt_agg["model_mae"]:.1f}</strong> p.p. (<strong>{_imp:+.0f}%</strong>), usando leave-one-out cross-validation.')
        print(f'<br><span class="text-muted">Acerto: Consolidado {_bt_agg["consolidado_correct"]}/{_bt_agg["n_paredoes"]} · Modelo {_bt_agg["model_correct"]}/{_bt_agg["n_paredoes"]}</span>')
        print(f'</div>\n')
...
for i, paredao in enumerate(reversed(paredoes_finalizados)):  # Most recent first
    ...
    if poll and poll.get('resultado_real'):
        ...
        def _arc_hl(val, all_vals, color_best, color_worst='#e74c3c'):
            best, worst = max(all_vals), min(all_vals)
            if _is_save_arc:
                if val == worst:
        ...
        for nome in participantes_poll:
            print(f'<th class="tc">{nome.split()[0]}</th>')
        print('</tr></thead><tbody>')
        ...
        for plat in platform_order:
            if plat in plataformas:
                ...
                for nome in participantes_poll:
                    ...
                    style = _arc_hl(val, _apvals, '#f39c12')
                    print(f'<td class="tc" style="{style}">{val:.1f}%</td>')
                print('</tr>')

        print(f'<tr style="background:rgba(155,89,182,0.2);"><td><strong>Consolidado</strong></td>')
        for nome in participantes_poll:
            print(f'<td class="tc" style="font-weight:bold;">{consolidado.get(nome, 0):.1f}%</td>')
        print('</tr>')
        ...
        if _tab_model:
            ...
            for nome in participantes_poll:
                ...
                style = _arc_hl(val, _amvals, '#00bc8c')
